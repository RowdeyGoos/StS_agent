...
internal static class SilentOracle
{
    public static void Run(Assembly asm, string digest)
    {
        ...
        var pool=Get("CardPool","CardPools.SilentCardPool");
        var originals=Items(Call(pool,"GetUnlockedCards",unlock,single)).Concat(new[]{Get("Card","Cards.Shiv")}).ToArray();
        ...
        foreach(var original in originals) {
         ...
         for(int level=0;level<=1;level++) {
          ...
          levels.Add(new { cost=Call(Prop(card,"EnergyCost"),"GetWithModifiers",Enum.ToObject(T("Entities.Cards.CostModifiers"),0)), x=Prop(Prop(card,"EnergyCost"),"CostsX"),keywords=Items(Prop(card,"Keywords")).Select(x=>x.ToString()),vars=Items(Prop(Prop(card,"DynamicVars"),"Values")).ToDictionary(v=>Prop(v,"Name").ToString()!,v=>Prop(v,"BaseValue"))});
         }
