...
internal static class EnemyInteractionOracle
{
        ...
        string seed, int fillers, bool answerLast, bool withTools, string scenario)
    {
        ...
        object[] Enemies()=>Items(P(combat,"Enemies")).Select(c=>(object)new{slot=P(c,"SlotName"),hp=P(c,"CurrentHp"),maxHp=P(c,"MaxHp"),block=P(c,"Block"),move=P(P(P(c,"Monster"),"NextMove"),"Id")}).ToArray();
        ...
        Require(pending.Length==(scenario=="block" ? (withTools ? 1 : 0) : withTools ? 2 : 1),"Unexpected deferred hook count.");
        ...
        foreach(var action in pending)
        {
            ...
            var pile=choiceId==0 && scenario!="block" ? "DrawPile" : "Hand";
        ...
        return new{seed,scenario,fillers,answerLast,withTools,before,enemiesBefore,checkpoint,enemiesCheckpoint,answers,after=State(),enemiesAfter=Enemies(),hits,
