...
if (args.Length is not (2 or 3) || (args.Length == 3 && args[2] is not ("generation" or "interactions" or "transforms" or "silent" or "regent" or "necrobinder" or "defect" or "foreign" or "hive" or "glory" or "reward-edges" or "ascensions"))) throw new ArgumentException("Usage: oracle <pinned-sts2.dll> <dependency-directory> [generation|interactions|transforms|silent|regent|necrobinder|defect|foreign|hive|glory|reward-edges|ascensions]");
...
if(args.Length==3 && args[2]=="ascensions") { AscensionOracle.Run(asm,digest); return; }
if(args.Length==3 && args[2]=="reward-edges") { RewardEdgeOracle.Run(asm,digest); return; }
if(args.Length==3 && args[2]=="foreign") { ForeignOracle.Run(asm,digest); return; }
