...
internal static class DefectOracle
{
    public static void Run(Assembly asm, string digest)
    {
        ...
        var pool=Get("CardPool","CardPools.DefectCardPool");
        var originals=Items(Call(pool,"GetUnlockedCards",unlock,single)).Concat(new[]{Get("Card","Cards.Fuel")}).ToArray();
