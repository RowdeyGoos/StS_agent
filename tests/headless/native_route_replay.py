...
from game.headless.run.actions import (
    ...
    Rest, OpenChest, ContinueAct, ChooseEventOption, LeaveEvent,
)
    ...
    if boosted:
        ...
        for native in expected['enemies']:
            ...
            enemies.append(dict(combatId=native['combatId'],
                                id=re.sub('[()]', '', name.upper()).replace(' ', '_'),
        ...
        enemies = [dict(id=re.sub('[()]', '', e.name.upper()).replace(' ', '_'), hp=e.hp, block=e.block)
...
def replay_route(row, *, boosted=False):
    ...
    for item in row['route']:
        ...
        if kind == 'ActTransition':
            ...
            assert run_boundary(run, boosted=boosted) == room['transition']
        elif kind == 'Victory':
            step(run, ContinueAct())
            assert run.state.pending['definition_id'] == 'the_architect'
            assert run_boundary(run, boosted=boosted) == room['entry']
            ...
            assert run_boundary(run, boosted=boosted) == room['state']
            continue
        node = next(n for n in run.graph.nodes if (n.row, n.column) == (room['row'], room['col']))
        step(run, ChooseNode(node.node_id))
        assert run_boundary(run, boosted=boosted) == room['entry'], (run.state.act_index, room['row'], 'entry')
        if boosted:
            assert run.state.act_index == item['actIndex']
        if kind == 'ActTransition':
            assert run.state.pending['definition_id'].upper() == room['ancient']
            assert [a.option_id.upper() for a in run.legal_actions()] == room['offers']
            ...
            if room['rewards'] is not None:
                ...
                assert run_boundary(run, boosted=boosted) == room['rewards']['state']
            ...
            step(run, LeaveShop())
        assert run_boundary(run, boosted=boosted) == room['state'], (run.state.act_index, room['row'])
    assert run_boundary(run, boosted=boosted) == row['state']
