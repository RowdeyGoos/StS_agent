...
def _config_without_mode(path: Path) -> dict:
    ...
    config["benchmark"]["batch"].pop("worker")
    return config
...
def test_serial_and_spawned_panels_have_equal_deterministic_artifacts(tmp_path: Path) -> None:
    serial_config = _write_panel(tmp_path / "serial.json")
    spawned_config = _write_panel(tmp_path / "spawned.json", process_safe=True)
...
def test_sigint_writes_received_and_pending_artifacts_without_lingering_cli_processes(tmp_path: Path) -> None:
    config = _panel(process_safe=True, repetitions=3, episodes=12)
    for episode in config["episodes"]:
        episode["transition_budget"] = 300
    ...
    process = subprocess.Popen(
        [
            ...
            "-c",
            (
                "import multiprocessing, sys; from game.cli.headless import main; "
                "print('READY', flush=True); main(sys.argv[1:]); "
        ...
        text=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE, env=_environment(),
    )
    try:
        assert process.stdout is not None
        assert process.stdout.readline() == "READY\n"
        # Start after all imports have completed, so SIGINT exercises collector
        # cleanup rather than module initialization.
        time.sleep(0.1)
