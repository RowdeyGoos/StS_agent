...
def _combat_data(*, hp: int = 61) -> dict[str, object]:
    enemy_ref = combat_enemy_reference("jaw_worm", 0)
    card_ref = combat_card_reference("strike", 0)
    return {
        "turn": 2,
        "player": {
            "hp": hp,
...
def _combat_observation(*, hp: int = 61) -> PublicObservation:
    return PublicObservation(DecisionPhase.COMBAT, _combat_data(hp=hp))


def _combat_candidates() -> tuple[CombatEndTurnCandidate, CombatPlayCardCandidate]:
    return (
        CombatEndTurnCandidate(),
        CombatPlayCardCandidate(
            card_ref=combat_card_reference("strike", 0),
            target_ref=combat_enemy_reference("jaw_worm", 0),
        ),
    )
...
def _reward_data(*, opened: bool = True) -> dict[str, object]:
    gold_ref = reward_reference(RewardKind.GOLD, 0)
    card_ref = reward_reference(RewardKind.CARD, 1)
    offers = []
    if opened:
        offers = [
            {
                "offer_ref": reward_offer_reference(card_ref, "strike", 0),
                "card_definition_id": "strike",
                "upgraded": False,
            }
        ]
...
def _map_data() -> dict[str, object]:
    combat_ref = map_node_reference(NodeKind.COMBAT, 0)
    rest_ref = map_node_reference(NodeKind.REST, 1)
    return {
        "player": _run_player(),
...
def _room_data(*, room_kind: RoomKind = RoomKind.REST) -> dict[str, object]:
    option_kind = (
    ...
    return {
        ...
        "options": [
            {
                "option_ref": room_option_reference(room_kind, option_kind, 0),
...
def _observation(phase: DecisionPhase) -> PublicObservation:
    data = {
        DecisionPhase.COMBAT: _combat_data(),
        DecisionPhase.REWARD: _reward_data(),
        DecisionPhase.MAP: _map_data(),
        DecisionPhase.ROOM: _room_data(),
        DecisionPhase.TERMINAL: {"outcome": "victory", "player": _run_player()},
        DecisionPhase.UNSUPPORTED: {"reason_code": "unsupported_phase"},
    }[phase]
    return PublicObservation(phase, data)
...
def _decision(
    *,
    ...
    selected_observation = observation or _observation(phase)
...
def _all_candidates() -> tuple:
    gold_ref = reward_reference(RewardKind.GOLD, 0)
    card_reward_ref = reward_reference(RewardKind.CARD, 1)
    offer_ref = reward_offer_reference(card_reward_ref, "strike", 0)
    rest_ref = room_option_reference(RoomKind.REST, RoomOptionKind.REST_HEAL, 0)
    event_ref = room_option_reference(RoomKind.EVENT, RoomOptionKind.EVENT_OPTION, 0)
    return (
        CombatPlayCardCandidate(
            combat_card_reference("strike", 0),
            combat_enemy_reference("jaw_worm", 0),
        ),
        CombatEndTurnCandidate(),
        RewardClaimGoldCandidate(gold_ref, 25),
        RewardOpenCardRewardCandidate(card_reward_ref),
        RewardChooseCardCandidate(card_reward_ref, offer_ref, "strike"),
        RewardSkipCardCandidate(card_reward_ref),
        RewardProceedCandidate(),
        MapChooseNodeCandidate(map_node_reference(NodeKind.REST, 1)),
        RoomRestHealCandidate(rest_ref, 18),
        RoomEventOptionCandidate(event_ref),
        RoomProceedCandidate(),
    )
...
def test_contract_identity_and_canonical_vectors_are_stable() -> None:
    ...
    assert CONTRACT_FINGERPRINT == (
        "9a8fca31c9671e2258d34294051cb387ef75e0cbb60ebc005aad636f6c23e056"
    )
    assert canonical_json({"z": 0, "a": "é", "list": [True, None]}) == (
        '{"a":"é","list":[true,null],"z":0}'
    )
    assert _decision().decision_hash == (
        "ecd7ad6ad97b88f1cdcb34061c596760e82b060c270b9e4f5be28b6989201976"
    )
...
def test_public_reference_factories_have_only_explicit_public_basis() -> None:
    ...
    with pytest.raises(TypeError):
        combat_card_reference("strike", 0, private_state="secret")
    assert combat_card_reference("strike", 0) == combat_card_reference("strike", 0)
    assert combat_card_reference("strike", 0) != combat_card_reference("defend", 0)
...
def test_raw_key_aliases_are_rejected(alias: str) -> None:
    ...
    with pytest.raises(ContractValidationError):
        PublicObservation(DecisionPhase.COMBAT, data)
...
def test_combat_cross_fields_and_canonical_references_fail_closed() -> None:
    ...
    with pytest.raises(ContractValidationError, match="inconsistent"):
        PublicObservation(DecisionPhase.COMBAT, data)
    data = _combat_data()
    data["hand"][0]["card_ref"] = combat_card_reference("defend", 0)
...
def test_actionable_combat_observation_cannot_be_empty_or_terminal() -> None:
    with pytest.raises(ContractValidationError):
        PublicObservation(DecisionPhase.COMBAT, {})
    data = _combat_data()
    data["enemies"] = []
    ...
    data["enemies"][0]["hp"] = 0
    data["enemies"][0]["alive"] = False
    observation = PublicObservation(DecisionPhase.COMBAT, data)
...
def test_map_reward_and_room_cross_links_fail_closed() -> None:
    ...
    reward_data["rewards"][1]["offers"][0]["offer_ref"] = reward_offer_reference(
        reward_data["rewards"][1]["reward_ref"], "defend", 0
    )
    with pytest.raises(ContractValidationError, match="canonically derived"):
        PublicObservation(DecisionPhase.REWARD, reward_data)
    map_data = _map_data()
    map_data["edges"][0]["target_node_ref"] = map_node_reference(NodeKind.EVENT, 2)
    with pytest.raises(ContractValidationError, match="unresolved"):
        PublicObservation(DecisionPhase.MAP, map_data)
    map_data = _map_data()
    ...
    with pytest.raises(ContractValidationError, match="canonical rest"):
        PublicObservation(DecisionPhase.ROOM, room_data)
...
def test_candidate_references_must_resolve_in_same_observation() -> None:
    with pytest.raises(ContractValidationError, match="card_ref does not resolve"):
        _decision(
            candidates=(
                CombatPlayCardCandidate(
                    combat_card_reference("defend", 0),
                    combat_enemy_reference("jaw_worm", 0),
                ),
            )
        )
    with pytest.raises(ContractValidationError, match="target_ref does not resolve"):
        _decision(
            candidates=(
                CombatPlayCardCandidate(
                    combat_card_reference("strike", 0),
                    combat_enemy_reference("jaw_worm", 1),
                ),
            )
        )
...
def test_reward_map_and_room_candidate_links_are_validated() -> None:
    reward_observation = PublicObservation(DecisionPhase.REWARD, _reward_data())
    reward_ref = reward_reference(RewardKind.CARD, 1)
    offer_ref = reward_offer_reference(reward_ref, "strike", 0)
    _decision(
        phase=DecisionPhase.REWARD,
        observation=reward_observation,
        candidates=(RewardChooseCardCandidate(reward_ref, offer_ref, "strike"),),
        events=(),
    )
    with pytest.raises(ContractValidationError, match="does not match its offer"):
        _decision(
            phase=DecisionPhase.REWARD,
            observation=reward_observation,
            candidates=(RewardChooseCardCandidate(reward_ref, offer_ref, "defend"),),
            events=(),
        )
    map_observation = PublicObservation(DecisionPhase.MAP, _map_data())
    with pytest.raises(ContractValidationError, match="not available"):
        _decision(
            phase=DecisionPhase.MAP,
            observation=map_observation,
            candidates=(MapChooseNodeCandidate(map_node_reference(NodeKind.COMBAT, 0)),),
            events=(),
        )
    room_observation = PublicObservation(DecisionPhase.ROOM, _room_data())
    with pytest.raises(ContractValidationError, match="does not match its option"):
        _decision(
            ...
            candidates=(
                RoomEventOptionCandidate(
                    room_option_reference(RoomKind.REST, RoomOptionKind.REST_HEAL, 0)
                ),
            ),
            events=(),
        )
...
def test_unpaired_surrogates_normalize_to_contract_validation_errors() -> None:
    ...
    with pytest.raises(ContractValidationError):
        combat_card_reference("strike" + surrogate, 0)
...
def test_hash_binds_public_candidate_event_decision_and_versions() -> None:
    ...
    variants = (
        ...
        _decision(candidates=(CombatEndTurnCandidate(),)),
        _decision(
            events=(
                PublicEvent(
                    0,
...
def test_hash_tampering_duplicate_candidates_unknown_and_duplicate_json_fail() -> None:
    ...
    duplicate = CombatEndTurnCandidate()
