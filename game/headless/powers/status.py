...
STATUS_DEFINITIONS = MappingProxyType({
    **{n: StatusDefinition(name=n, duration_tick_side='enemy' if n == 'debilitate' else None) for n in NECRO_STATUSES},
    ...
    CONQUEROR: StatusDefinition(name=CONQUEROR),
    FRAIL: StatusDefinition(name=FRAIL),
    **{name: StatusDefinition(name=name, duration_tick_side=None) for name in
       (ARTIFACT, CONSTRICT, TANGLED, RINGING, SLOW, PLOW, MINION, ILLUSION, INFESTED, MANGLE, DARK_SHACKLES, DEMISE, POISON, STRANGLE, CRUSH_UNDER, DYING_STAR, MONARCHS_GAZE)},
