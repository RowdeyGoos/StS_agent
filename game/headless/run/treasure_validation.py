...
def validate_treasure(state, graph):
    ...
    if (state.phase is not RunPhase.ROOM or pending["stage"] not in ("closed", "open", "claimed")
    ...
    elif (relic_id not in ORDINARY_CHEST.relic_pool or not state.treasure_relics_drawn
