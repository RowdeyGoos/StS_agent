"""Shared native A0 topology for Overgrowth and Underdocks."""
...
BASE_PROFILES = (BASE_PROFILE, UNDERDOCKS_BASE_PROFILE)
PRUNED_PROFILES = (PROFILE, UNDERDOCKS_PROFILE)
...
def generate_act1_map(rng, *, event_pool, act="overgrowth", profile=None):
    ...
    if profile not in (profile_for(act, base=True), profile_for(act)):
...
def validate_generated_map(graph):
    ...
    if graph.entry_node_ids != entries or len(entries) < (2 if graph.generation in BASE_PROFILES else 1):
