"""Play the restricted first slice using direct game commands and no RL stack."""
...
def play_slice(*, seed=0, rest_choice="smith", verify_restore=False):
    engine = RunEngine.ironclad_slice(seed=seed)
    trace = []
    for _ in range(500):
        if not engine.legal_actions():
            return engine, trace
        action = choose_demo_action(engine, rest_choice)
        if verify_restore:
...
def main(argv=None):
    ...
    engine, trace = play_slice(seed=args.seed, rest_choice=args.rest_choice, verify_restore=args.verify_restore)
    state = engine.state
    print(json.dumps({"scope": "restricted_ironclad_a0_two_combat_slice", "seed": state.seed,
