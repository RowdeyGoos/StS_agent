...
@dataclass(frozen=True, slots=True)
class _ActionSummaryContext:
    """Observation values shared by every action summarized in one state."""
    ...
    player_block: int
    player_strength: int
    player_statuses: dict[str, Any]
    hand: list[Any]
    enemies: list[dict[str, Any]]
    projected_incoming_hp_loss: int
...
def summarize_action(
    ...
    action: CombatAction,
    *,
    context: _ActionSummaryContext | None = None,
) -> ActionSummary:
    """Summarize the immediate tactical effect of one legal action."""
    validate_action(action)
    summary_context = (
        _prepare_action_summary_context(observation)
        if context is None
        else context
    )
    if summary_context.observation is not observation:
        raise ValueError("Action-summary context belongs to a different observation.")

    hand = summary_context.hand
    enemies = summary_context.enemies
    projected_before = summary_context.projected_incoming_hp_loss
    player_energy = summary_context.player_energy
    player_block = summary_context.player_block
    player_strength = summary_context.player_strength
    player_statuses = summary_context.player_statuses
    ...
    if action[0] == "end_turn":
        return ActionSummary(
            ...
            kills_target=False,
            wins_combat=False,
            living_enemy_count_after=summary_context.living_enemy_count,
        )
    ...
    copied_enemies = [dict(enemy) for enemy in enemies]
    ...
    projected_after = project_incoming_hp_loss_from_state(
        player_hp=summary_context.player_hp,
...
def _prepare_action_summary_context(
    observation: ObservationLike,
) -> _ActionSummaryContext:
    """Parse and simulate observation-wide values once for a feature matrix."""
    player = _require_mapping(observation, "player")
    hand = observation.get("hand")
    if not isinstance(hand, list):
        raise ValueError("Observation field 'hand' must be a list.")
    ...
    projected_incoming_hp_loss = project_incoming_hp_loss_from_state(
        player_hp=player_hp,
        player_block=player_block,
        player_statuses=player_statuses,
        enemies=enemies,
    )
    return _ActionSummaryContext(
        ...
        hand=hand,
        enemies=enemies,
        projected_incoming_hp_loss=projected_incoming_hp_loss,
...
def _action_summary_cache_key(
    context: _ActionSummaryContext,
    action: CombatAction,
) -> tuple[str, str | None, int | None]:
    """Identify actions whose encoded tactical effects are identical."""
    validate_action(action)
    if action[0] == "end_turn":
        return ("end_turn", None, None)

    hand_index = action[1]
    if hand_index < 0 or hand_index >= len(context.hand):
        raise ValueError(f"Hand index {hand_index} is out of range for observation hand.")
    card_name = str(context.hand[hand_index])
    card_spec = get_card_spec(card_name)
    target_index = (
        _resolve_target_index(action, context.enemies)
        if card_spec.uses_target
        else None
    )
    return ("play", card_name, target_index)
