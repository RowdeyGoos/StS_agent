...
| Embedded event combat | Non-resuming entry with no event-supplied extra rewards is implemented offline, with exact combat ownership and event/combat/rewards/map composition; Dense Vegetation Fight after Rest is the pending live case | Unreleased and not live-demonstrated; Battleworn Dummy resumption, extra rewards and arbitrary embedded branches remain unsupported |
