...
| Event card rewards and multiple reward entries | `card_reward_v1` has native/C#/Python fixtures for an ordinary singleton CardReward with 1–5 offers, exact choice or Skip plus dismissal; BrainLeech/Rip is the representative pinned caller. `item_set_v1` supports 2–8 potion/relic entries offline; Cheese uses a different add-grid surface | Live acceptance, multiple/mixed card-reward sets, repeated offers within one option, SpecialCardReward, reroll/multipick, substituted cards and nested pickup/selector ownership remain gaps |
