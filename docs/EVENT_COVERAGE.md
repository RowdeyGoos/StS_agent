...
| Repeated option pages | Fresh native controls plus completed callbacks permit identical keys/text and revisits; native-to-host fixtures cover two repeats through Exit/Proceed/map and separate completed item children | AbyssalBaths two-Linger live case remains open; EndlessConveyor and SlipperyBridge need caller evidence |
| Deck changes around selectors | Append-only additions before the first owned selector request now form its fixed baseline; native-to-host enchant/upgrade/transform shapes pass through Proceed/map | Grave/Confront and Trial verdicts need live caller evidence; changed original cards and removal-plus-addition after selection remain unsupported |
