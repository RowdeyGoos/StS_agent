...
| Item rewards | Singleton potion/relic children and Potion Courier/Ransack live; `item_set_v1` now has native/C#/Python fixtures for 2–8 potion/relic entries, exact ordered collection and retained prior effects | Multi-entry live acceptance, full-inventory handling, nested pickups and broader relic/caller evidence |
...
| Enchant cards | Single-card v1 has Sapphire Seed/Sown and Grave/SoulsPower live evidence; fixed multi-card v2 (2–8) now has native/C#/Python fixtures with exact original preview and per-card effects | Multi-card live acceptance and other callers remain open; candidates must be unenchanted; stacking/replacement and optional counts unsupported |
...
| Generic deck transformations | Fixed-one native transform-prompt `FromDeckGeneric` → `NDeckCardSelectScreen` uses original preview and exact `card_transform_v2` journal; Bird/Peck and Torus/ToricToughness shapes pass native/C#/Python fixtures | WoodCarvings live acceptance, selectorless automatic cases, optional/multiple generic selection and other prompt semantics remain open |
| Other card surfaces | Static pickup callers identify ChooseACard, bundle and result screens | These request/screen pairs are not covered by current generic card children |
