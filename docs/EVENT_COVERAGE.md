...
| Inactive combat layouts | Unreleased native `NCombatEventLayout` admission with stable embedded room and `HasCombatStarted=false`; ordinary options and existing children. PunchOff/Nab-shaped Injury plus relic reward and map return pass native/C#/Python fixtures | No live acceptance; combat execution, resumption and terminal branches remain unsupported; choices cannot all be classified as combat-starting in advance |
...
| Choose-one offered cards | Unreleased `card_offer_v1` uses the owned native ChooseACard request/screen; 1–3 offers, one click, exact chosen card addition. Optional `card_offer_v2` adds native Skip and zero/one separately observed appended grant; choose/Skip and map return pass native/C#/Python fixtures. LeadPaperweight/MassiveScroll supply v1 callers; HeftyTablet supplies v2 | No live acceptance; v2 effects/provenance remain unverified; multiple grants, substitution and nested pickups remain unsupported |
...
| Card-results acknowledgment | Unreleased `card_results_v1` acknowledges 1–64 displayed results through the owned SimpleCardsView capstone and native Confirm; exact unchanged post-show deck and Chosen completion; native/C#/Python fixtures | No live acceptance; automatic transformation correctness remains unverified. Darv/PandorasBox supplies the static caller, but pool eligibility remains unresolved. Nested/alternative results screens unsupported |
