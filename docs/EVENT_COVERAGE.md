...
| Deck changes around selectors | Append-only additions before the first owned selector request form its fixed baseline; Grave/Confront passed live; native-to-host enchant/upgrade/transform shapes pass through Proceed/map | Trial verdicts need live caller evidence; changed original cards and removal-plus-addition after selection remain unsupported; automatic addition provenance is not verified |
