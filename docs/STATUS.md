...
| Headless game engine | [Independent gameplay package](HEADLESS_ENGINE.md): playable restricted Ironclad A0 two-combat slice with rewards, rest/smith, Burning Blood, Fire/Block Potions and JSON continuation at every decision, including Armaments/True Grit combat hand choices. `sts-headless-play` exercises the loop. Legacy experiment APIs consume the same combat rules. Native full-run fidelity remains open; [evidence and limits](evidence/first_vertical_slice_2026_09_13.md), [remaining tasks](HEADLESS_FULL_GAME_IMPLEMENTATION.md). |
