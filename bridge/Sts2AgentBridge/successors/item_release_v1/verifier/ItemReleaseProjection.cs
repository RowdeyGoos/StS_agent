...
internal sealed record ItemReleaseProjection(
    string AssemblyName,
...
internal static partial class ItemReleaseProjectionBuilder
{
    private const string SensitiveType =
        "Sts2AgentBridge.Successors.ItemTransportV1.ItemTransportRuntime";
    private static readonly Regex RoutePattern = new(
        @"/probe/[A-Za-z0-9_./-]*",
        RegexOptions.CultureInvariant);

    internal static ItemReleaseProjection Build(byte[] image)
    {
        try
        {
            ...
            AddRoutes(Encoding.Latin1.GetString(image), routes);
            AddRoutes(Encoding.Unicode.GetString(image), routes);
            ...
            foreach (CustomAttributeHandle handle in reader.CustomAttributes)
            {
                ...
                inventory.Add($"attribute|{Token(value.Parent)}|{MethodOperand(reader, value.Constructor, names, methods)}|{Hex(reader.GetBlobBytes(value.Value))}");
            }
            ...
            foreach ((MethodDefinitionHandle handle, string owner) in methods)
            {
                ...
                foreach (IlInstruction instruction in instructions)
                {
                    ...
                    if (instruction.MetadataToken.HasValue && instruction.OpCode != 0x0072)
                    {
                        ...
                        if (targetIdentity.Contains(SensitiveType + ".StartForTests(", StringComparison.Ordinal) ||
                            targetIdentity.Contains(SensitiveType + ".IsAllowedTestEndpoint(", StringComparison.Ordinal))
                        {
            ...
            return new ItemReleaseProjection(
                assemblyName,
                assemblyVersion,
                Sha(image),
                image.LongLength,
