...
CAMPAIGN_ID = "ITEM-V1-COLLECTION-SMOKE-V1"
STATE_ROOT_NAME = "Sts2AgentBridgeCampaign-item-v1-collection-smoke-v1"
...
CONFIG_BYTES = (
    b'{"schema_version":"item_probe_v1_transport_config_v1","enabled":true,'
    b'"bind_address":"127.0.0.1","port":43117,"token_file":"credential.hex"}'
)
CONFIG_SHA256 = "240f57a37801fd13dedb4cc823163a50a9745fc8053f23318614d995749570b8"
...
OVERLAY_DLL_NAME = "Sts2AgentBridgeItemV1.dll"
OVERLAY_MANIFEST_NAME = "Sts2AgentBridgeItemV1.json"
...
CANONICAL_ARTIFACTS = ArtifactPolicy(
    dll=ArtifactSpec(
        OVERLAY_DLL_NAME,
        95_232,
        "09ea93cd86a4ca502c27357171f7a7efdaf2bcf91a0a3fcffec96ef79745b9f6",
    ),
    manifest=ArtifactSpec(
        OVERLAY_MANIFEST_NAME,
        340,
        "03163de389a7212c39f18c54ace292b8b105e3c7396bf08c871203f6c33978d4",
    ),
    archive=ArtifactSpec(
        "Sts2AgentBridgeItemV1-1.0.0.zip",
        95_936,
        "349c24fa02da4a11e19fa4dd6a08100b805d9c5dbe84b89cd4e3de120a412beb",
    ),
)
...
def _production_layout(user_profile: Path) -> CampaignLayout:
    ...
    overlay_root = mods_parent / "Sts2AgentBridgeItemV1"
    operator_parent = application_support / "Sts2AgentBridge"
    config_root = operator_parent / "item_v1"
...
def _validate_operator_unit(root: Path, uid: int) -> dict[Path, os.stat_result]:
    records: dict[Path, os.stat_result] = {}
    records[root] = _require_directory(root, uid, 0o700)
    _list_exact(root, {"item_v1"})
    records.update(_validate_config_tree(root / "item_v1", uid))
    return records
...
def _validate_operator_unit_metadata(
    ...
    records[root] = _require_directory(root, uid, 0o700)
    _list_exact(root, {"item_v1"})
    records.update(_validate_config_tree_metadata(root / "item_v1", uid))
    return records
...
def _validate_overlay_unit(
    ...
    _list_exact(unit_root, {"Sts2AgentBridgeItemV1"})
    records.update(
        _validate_overlay_tree(unit_root / "Sts2AgentBridgeItemV1", uid, policy)
    )
    return records
...
def _require_state_records(
    ...
    config_root = operator_root / "item_v1"
    overlay_root = (
        overlay_unit_root / "Sts2AgentBridgeItemV1"
        if state.mods_parent_created
...
def _install(
    ...
    try:
        ...
        with _private_umask():
            ...
            staged_config = layout.staged_operator / "item_v1"
            ...
            if mods_parent_created:
                staged_overlay_root = layout.staged_overlay / "Sts2AgentBridgeItemV1"
...
def _validate_purge_remainder(
    ...
    if progress < 4:
        ...
        expected_operator = {"item_v1"} if progress < 3 else set()
    ...
    if progress < overlay_removed_at:
        ...
        if state.mods_parent_created:
            expected_unit = {"Sts2AgentBridgeItemV1"} if progress < 7 else set()
