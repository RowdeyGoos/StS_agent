...
_POLICY = manager.ArtifactPolicy(
    ...
    archive=manager.ArtifactSpec(
        "Sts2AgentBridgeItemV1-0.1.0.zip",
        len(_ARCHIVE_DATA),
...
class FixtureTree:
    def __init__(self, root: Path, *, preexisting_mods: bool = False) -> None:
        ...
        self.overlay_root = self.mods_parent / "Sts2AgentBridgeItemV1"
        self.operator_parent = self.application_support / "Sts2AgentBridge"
        self.config_root = self.operator_parent / "item_v1"
...
def _assert_recorded_retention(
    ...
    for overlay_unit in present_overlays:
        overlay_root = (
            overlay_unit / "Sts2AgentBridgeItemV1"
...
def _verify_production_bindings() -> None:
    if manager.CAMPAIGN_ID != "ITEM-V1-COLLECTION-SMOKE-V1":
    ...
    if (
        len(manager.CONFIG_BYTES) != 139
        or hashlib.sha256(manager.CONFIG_BYTES).hexdigest()
        != "240f57a37801fd13dedb4cc823163a50a9745fc8053f23318614d995749570b8"
    ...
    if (
        ...
        / "Library"
        / "Application Support"
        / "Sts2AgentBridgeCampaign-item-v1-collection-smoke-v1"
        or manager.ARTIFACT_ROOT
        != Path("/private/tmp/sts-item-v1-release")
