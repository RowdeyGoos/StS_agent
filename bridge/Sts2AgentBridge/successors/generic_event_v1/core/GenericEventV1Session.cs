...
public sealed class GenericEventV1Session : IGenericEventV1Session
{
    private readonly IGenericEventV1NativeAdapter _native;
    ...
    private readonly List<GenericEventV1PriorResult> _history = new();
    private GenericEventV1NativeCapture? _published;
    ...
    public GenericEventV1Session(IGenericEventV1NativeAdapter adapter, string sessionNonce)
    {
    ...
    private GenericEventV1Observation ReadCore()
    {
        if (++_reads > GenericEventV1Limits.MaximumHostReads) _unsupported = true;
        ...
        GenericEventV1NativeCapture capture = _native.Capture();
        if (_unsupported || capture.Status == "unsupported") return Stop();
        if (_pending)
        {
            if (++_pendingReads > GenericEventV1Limits.MaximumPendingReads) return Stop();
            if (capture.Status == "child")
            {
                if (_proceed || _episodes >= GenericEventV1Limits.MaximumChildEpisodes ||
                    capture.ChildIdentity is null || !_screens.Add(capture.ChildIdentity) ||
                    capture.DomainCount < 2 || capture.DomainCount > 64) return Stop();
                _card = _native.CreateChild();
                _episodes++;
                _child = new GenericEventV1Child(_episodes, _pendingDecision,
                    _pendingAction, "upgrade", 1, 1, "preview_confirm", capture.DomainCount);
        ...
        foreach (var option in capture.Options)
            if (option.Identity is null || option.StableId.Length is < 1 or > 96 || option.StableId.Any(char.IsControl) ||
    ...
    public GenericEventV1ApplyResult Apply(string? decisionId, string? actionId)
    {
        ...
        try
        {
            ...
            GenericEventV1NativeOption selected = _published.Options[index];
    ...
    private void Reconcile(string result)
    {
        ...
        _history.Add(new GenericEventV1PriorResult(_pendingDecision,_pendingAction,result));
    }
    private GenericEventV1Observation Stop() { _unsupported = true; return Observation("unsupported","unsupported"); }
    private GenericEventV1ApplyResult Result(string? decision,string? action,string outcome) =>
        new(_nonce,decision ?? "",action ?? "",outcome);
    private GenericEventV1Observation Observation(string status,string phase)
    {
        var candidates = new List<GenericEventV1Candidate>(); var actions = new List<string>();
        if (status == "ready" && _published is not null)
            for (int i=0;i<_published.Options.Count;i++)
            {
                ...
                candidates.Add(new GenericEventV1Candidate(i,action,c.StableId,c.RenderedText,enabled,c.Dangerous,c.IsProceed));
                if (enabled && !c.Dangerous) actions.Add(action);
            }
        return new GenericEventV1Observation(_nonce,status,phase,status=="ready"?_decision:"",candidates,actions,
    ...
    private static string Stamp(GenericEventV1NativeCapture capture)
    {
    ...
    private static string DecisionStamp(GenericEventV1NativeCapture capture) => Stamp(capture) + string.Concat(capture.Options.Select(x => "|" + x.RenderedText.Length + ":" + x.RenderedText));
    public void Dispose()
    {
