...
PREVIOUS_SHA = '20de6604f790feca0e3651735d785aa30c523dcc72a4a831f72e962e624b81e7'
PREVIOUS_MANIFEST = '8e5d4ef4701929f2543a09ac41875c4479b59bf98e37286685cc86cbb5df053d'
PREVIOUS_INVENTORY = '97735acc3534bbe1841032dd10a361567b3b6f90e008992b2ab2fd3718fd7967'
...
def sources():
    path = ROOT.parent / 'event_card_operations_v1/check.py'
    ...
    components['event_card_operations_v1'], inventories['event_card_operations_v1'] = helper.source_snapshot(
        ROOT.parent / 'event_card_operations_v1', 'event_card_operations_v1', previous.CONTRACT,
        PREVIOUS_MANIFEST, PREVIOUS_INVENTORY)
    require(len(components) == 14, 'predecessor_count')
    own = {}
    ...
    contract_path = ROOT.parents[3] / 'docs/PHASE_1_GENERIC_EVENT_V1_CONTRACT.md'
    ...
    if identity.exists():
        ...
        require(expected == {'schema_version': 1, 'component': 'generic_event_v1',
            'contract_sha256': contract_hash, 'inventory_sha256': inventory, 'files': {name: sha(data) for name, data in own.items()}}, 'source_identity')
    components['generic_event_v1'] = own
    inputs = previous.inputs(components, previous.old_sources(helper))
    derivation = json.loads(own['native/derivation.json'])
    require(len(derivation['derived']) == 2, 'derivation_inventory')
...
def main():
    ...
    require(set(projects) == {
        'core/GenericEventV1.Core.csproj', 'native/GenericEventV1.Native.csproj',
        'native_tests/GenericEventV1.Native.Tests.csproj', 'wire/GenericEventV1.Wire.csproj',
        'wire_tests/Sts2AgentBridge.GenericEventV1.WireTests.csproj',
        'integration/Sts2AgentBridge.GenericEventV1.Integration.csproj',
        'integration/GenericEventV1.Native.Integration.csproj'}, 'project_inventory')
    require({'host_tests/test_generic_event_host.py', 'integration_tests/test_generic_event_integration.py'}.issubset(own), 'required_test_inventory')
    for project in projects:
        closure(inputs, PREFIX + 'generic_event_v1/' + project, project.startswith('native/'))
    ...
    def snapshot(name):
        ...
        component = source / PREFIX / 'generic_event_v1'
    ...
    for script in sorted((component / 'integration_tests').glob('test_*.py')):
        test_results[script.name] = run([sys.executable, '-B', str(script), '--dotnet', dotnet,
            '--fixture', str(outputs['integration/Sts2AgentBridge.GenericEventV1.Integration.csproj']),
            '--native-fixture', str(outputs['integration/GenericEventV1.Native.Integration.csproj']), '--host', str(component / 'host/generic_event_host.py')], component, env).strip()
