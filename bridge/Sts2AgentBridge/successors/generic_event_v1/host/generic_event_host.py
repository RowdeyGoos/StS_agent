...
DECISION_ROUTE = '/probe/generic-event-v1/public/decision'
ACTION_ROUTE = '/probe/generic-event-v1/public/action'
VERSION = 'generic_event_v1'
...
_PARENT = ('status', 'phase', 'decision_id', 'candidates', 'legal_actions',
           'prior_results', *_FIELDS, 'total_attempted', 'effects')
...
def _decode(body: Any) -> dict[str, Any]:
    ...
    try:
        ...
        if v['child'] is not None:
            ...
            _require(c['operation'] == 'upgrade' and c['commit_mode'] == 'preview_confirm')
            _require(type(c['min_select']) is int and type(c['max_select']) is int and
                     c['min_select'] == c['max_select'] == 1)
            _require(_integer(c['domain_count'], 64) and c['domain_count'] >= 2)
        return v
    except _Stop:
        raise
...
class _Controller:
    ...
    def parent_read(self, p: Any) -> str:
        ...
        _require(p['parent_reconciled'] == len(history))
    ...
    def card_parse(self, p: Any) -> None:
        ...
        try:
            self.card._validate_envelope(p)
        except Exception:
            raise _Stop('invalid_response') from None
    ...
    def child_read(self, p: Any) -> str:
        ...
        _require(len(selections) <= 1 and actions.count('preview') <= 1 and actions.count('confirm') <= 1)
        if 'confirm' in actions:
            _require(actions[-1] == 'confirm')
        if status == 'ready':
            _require('confirm' not in actions)
            _require((p['operation'], p['min_select'], p['max_select'], p['commit_mode']) == ('upgrade', 1, 1, 'preview_confirm'))
            _require(len(p['candidates']) == self.child['domain_count'])
            _require(p['selected_slots'] == selections)
            shape = [(c['slot'], c['key'], c['upgrade_level']) for c in p['candidates']]
            _require([s for s, _, _ in shape] == list(range(len(shape))))
            ...
            _require(self.child_shape is not None and self.preview_seen and len(selections) == 1 and actions[-1] == 'confirm')
            _require(p['operation'] == 'upgrade' and len(p['selected_cards']) == 1)
            for c in p['selected_cards']:
                _require(c['slot'] == selections[0] and c['selected'] is True and
    ...
    def summary(self, status: str, code: str | None) -> dict[str, Any]:
        return {'schema_version': 1, 'status': status, **self.counts,
                ...
                'reads': self.reads, 'effects': self.effects, 'code': code}
...
def run_event(request: Callable, *, provider: Callable, clock: Callable = time.monotonic,
    ...
    empty = {'schema_version': 1, 'status': 'failed', **dict.fromkeys(_FIELDS, 0),
             'total_attempted': 0, 'reads': 0, 'effects': 'none_attempted'}
