...
// Explicit, exclusive three-method observational patch scope. Originals always run.
public sealed class GenericEventV1Hooks : IDisposable
{
    private const string Owner = "sts2agent.generic_event_v1";
    private static GenericEventV1Hooks? _installed;
    private static GenericEventV1Binding? _armed;
    private static readonly AsyncLocal<GenericEventV1Binding?> Dispatch = new();
    private static readonly AsyncLocal<GenericEventV1Binding?> Parent = new();
    private static readonly AsyncLocal<GenericEventV1Binding?> Request = new();
    ...
    public GenericEventV1Hooks() : this(null,null) { }
    internal GenericEventV1Hooks(Action<int>? afterPatch,Action? cleanupProbe)
    {
    ...
    private static MethodInfo Hook(string name) => typeof(GenericEventV1Hooks).GetMethod(name,BindingFlags.Static|BindingFlags.NonPublic)!;
    internal static void Arm(GenericEventV1Binding binding)
    {
    ...
    internal static bool Owns(GenericEventV1Binding binding) => ReferenceEquals(_armed,binding) &&
        _installed is not null && !_installed._disposed && _installed.ExactPatches();
    ...
    internal static void Close(GenericEventV1Binding binding)
    {
        binding.Closed = true;
        if (ReferenceEquals(_armed,binding)) _armed = null;
    }
    internal static IDisposable EnterDispatch(GenericEventV1Binding binding)
    {
    ...
    {
        private readonly GenericEventV1Binding _binding;
        internal DispatchScope(GenericEventV1Binding binding)=>_binding=binding;
        public void Dispose()
        {
    ...
    {
        internal GenericEventV1Binding? Binding;
        internal GenericEventV1Binding? Previous;
    ...
    private static void UpgradePrefix(Player __0, CardSelectorPrefs __1, out State __state)
    {
        ...
        try
        {
            ...
            b.RequestSeen=true; b.Prefs=__1;
    ...
    private static void ScreenPrefix(IReadOnlyList<CardModel> __0, CardSelectorPrefs __1, IRunState __2, out State __state)
    {
        ...
        try
        {
            if (!Owns(b) || b.Closed || b.ScreenSeen || !ReferenceEquals(Parent.Value,b) ||
