...
internal static class GenericEventV1WireCodec
{
    internal static byte[] Decision(string nonce, GenericEventV1Observation p, byte[]? payload) =>
        Encode(w =>
        {
    ...
    internal static byte[] Action(string nonce, GenericEventV1Child? child,
        GenericEventV1ApplyResult? receipt, byte[]? payload) => Encode(w =>
    {
    ...
    private static void Start(Utf8JsonWriter w, string nonce, string kind)
    {
        ...
        w.WriteString("protocol", GenericEventV1Limits.Version); w.WriteString("session_nonce", nonce);
        w.WriteString("kind", kind);
    }
    private static void Child(Utf8JsonWriter w, GenericEventV1Child? c)
    {
