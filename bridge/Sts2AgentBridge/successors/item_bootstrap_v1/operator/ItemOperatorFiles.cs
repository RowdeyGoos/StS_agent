...
internal static class ItemOperatorFiles
{
    ...
    private static ItemOperatorConfiguration? OpenBelowHome(DarwinDescriptorLease descriptors)
    {
        ...
        try
        {
            if (!descriptors.TryOpenDirectory("Library", DarwinDescriptorPolicy.OwnedDirectory) ||
                ...
                !descriptors.TryOpenDirectory("item_v1", DarwinDescriptorPolicy.PrivateDirectory) ||
    ...
    private static bool IsExactEnabled(byte[] bytes)
    {
        ...
        if (bytes.Length != enabled.Length)
            return false;
        int difference = 0;
        for (int index = 0; index < bytes.Length; index++)
            difference |= bytes[index] ^ enabled[index];
        return difference == 0;
    }
    ...
    private static bool IsSyntheticHome(string path)
    {
        ...
        if (leaf.Length == 0 || leaf.Length > 128 || leaf.IndexOf('/') >= 0 ||
            leaf == "." || leaf == ".." || leaf.IndexOf('\0') >= 0 ||
            leaf.IndexOf('\n') >= 0 || leaf.IndexOf('\r') >= 0)
            return false;
        return true;
    }
