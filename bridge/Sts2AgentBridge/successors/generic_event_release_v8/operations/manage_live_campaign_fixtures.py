...
_POLICY = manager.ArtifactPolicy(
    ...
    archive=manager.ArtifactSpec(
        "Sts2AgentBridgeGenericEventV8-0.1.0.zip",
        len(_ARCHIVE_DATA),
...
class FixtureTree:
    def __init__(self, root: Path, *, preexisting_mods: bool = False, flow_kind: str = "generic") -> None:
        ...
        self.overlay_root = self.mods_parent / "Sts2AgentBridgeGenericEventV8"
        self.operator_parent = self.application_support / "Sts2AgentBridge"
        self.config_root = self.operator_parent / "generic_event_v8"
...
def _assert_recorded_retention(
    ...
    for operator_root in present_operators:
        config_root = operator_root / "generic_event_v8"
    ...
    for overlay_unit in present_overlays:
        overlay_root = (
            overlay_unit / "Sts2AgentBridgeGenericEventV8"
...
def _verify_production_bindings() -> None:
    if manager.CAMPAIGN_ID != "GENERIC-EVENT-V8-SMOKE-V1":
    ...
    expected_configs = {"generic": b'{"schema_version":"generic_event_v8_transport_config_v1","enabled":true,"flow_kind":"generic","bind_address":"127.0.0.1","port":43117,"token_file":"credential.hex"}'}
    if manager.CONFIGURATIONS != expected_configs or manager.CONFIG_HASHES != {"generic": "745880a427ea75faadd603b6f554c07ddab0eb2d4fe26b09fd060629271156bf"}:
    ...
    if (
        ...
        / "Library"
        / "Application Support"
        / "Sts2AgentBridgeCampaign-generic-event-v8-smoke-v1"
        or manager.ARTIFACT_ROOT
        != Path("/private/tmp/sts-generic-event-v8-release")
