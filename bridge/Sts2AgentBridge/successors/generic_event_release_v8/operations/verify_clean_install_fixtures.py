...
_MANIFEST_SHA256 = "55636a2930ca9888229ecc7c7c4c8d57c11825a05c29bb4e28023a6c0f725568"
...
def _write_package(path: Path, dll: bytes, manifest: bytes) -> bytes:
    with zipfile.ZipFile(
        ...
        for name, data in (
            (
                "Sts2AgentBridgeGenericEventV8/Sts2AgentBridgeGenericEventV8.dll",
                dll,
            ),
            (
                "Sts2AgentBridgeGenericEventV8/Sts2AgentBridgeGenericEventV8.json",
                manifest,
            ),
        ):
...
class _Fixture:
    ...
    def install_overlay(self, manifest: bytes | None = None) -> None:
        ...
        (directory / "Sts2AgentBridgeGenericEventV8.dll").write_bytes(self.dll)
        (directory / "Sts2AgentBridgeGenericEventV8.json").write_bytes(
...
def operation() -> dict[str, object]:
    ...
    with tempfile.TemporaryDirectory(
        ...
        mutated[mutated.index(b"Generic Event V8")] ^= 1
