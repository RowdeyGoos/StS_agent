...
public sealed class GenericEventV7TransformAdapter : ICardTransformV2NativeAdapter
{
        ...
        out GenericEventDiagnosticCode diagnostic)
    {
        ...
        if (!GridGeometry.TryBind(grid, bindings, out geometry)) return false;
        candidates=geometry.Mask(candidates);
        if(!InitialCandidatesValid(candidates))return false;
    ...
    private sealed class GridGeometry
    {
            ...
            out GridGeometry geometry)
        {
            ...
            if(!ProbeGeometry.TryBind(grid,candidates,out var probe))return false;
    ...
    private sealed class ProbeGeometry
    {
        ...
        internal static bool TryBind(NCardGrid grid,CandidateBinding[] candidates,out ProbeGeometry result)
        {
            result=null!;
            ...
            {
                if(!Valid(cursor)||!seen.Add(cursor))return false;
                if(cursor is Control control&&control.ClipContents){clip=control;break;}
                if(depth==32)return false;
                cursor=cursor.GetParent();
            }
            if(clip is null||!TryRect(clip,out var clipRect))return false;
            Rid canvas=clip.GetCanvas();if(!canvas.IsValid)return false;
            var chains=new List<NodeGeometry[]>();
            if(!TryChain(grid,clip,canvas,out var gridChain))return false;chains.Add(gridChain);
            ...
            for(int index=0;index<candidates.Length;index++)
            {
                ...
                {
                    if(!TryChain(control,clip,canvas,out var chain)||!TryRect(control,out var rect))return false;
        ...
        internal bool Matches()=>Valid(_clip)&&_clip.ClipContents&&TryRect(_clip,out var rect)&&Same(rect,_clipRect)&&
            _chains.All(chain=>chain.All(node=>node.Matches()));
        internal CardSelectionV1NativeCandidate[] Mask(CardSelectionV1NativeCandidate[] candidates)
        {
            if(candidates.Length!=_below.Length||!Matches())throw new InvalidOperationException("Offscreen geometry changed.");
        ...
        private static bool TryChain(Node start,Control clip,Rid canvas,out NodeGeometry[] chain)
        {
            ...
            for(int depth=0;depth<=32&&current is not null;depth++)
            {
                ...
                if(!Valid(current)||current is not CanvasItem||!seen.Add(current)||(!atClip&&depth==32))return false;
                Node? parent=atClip?null:current.GetParent();
                if(!NodeGeometry.TryBind(current,canvas,parent,!atClip,out var snapshot))return false;
        ...
        private sealed class NodeGeometry
        {
            ...
            internal static bool TryBind(Node node,Rid canvas,Node? parent,bool trackParent,out NodeGeometry result)
            {
                result=null!;Transform2D transform=default;Rect2 rectangle=default;bool clip=false;
                if(node is CanvasItem item)
                {
                    if(item.IsSetAsTopLevel()||item.GetCanvas()!=canvas)return false;
                    transform=item.GetGlobalTransform();if(!AxisAligned(transform))return false;
                }
                if(node is Control control){rectangle=control.GetGlobalRect();if(!Finite(rectangle.Position)||!Finite(rectangle.Size))return false;clip=control.ClipContents;}
                result=new NodeGeometry(node,parent,trackParent,canvas,transform,rectangle,clip);return true;
            }
            internal bool Matches()
            {
                if(!Valid(_node)||(_trackParent&&!ReferenceEquals(_node.GetParent(),_parent)))return false;
                if(_node is CanvasItem item&&(item.IsSetAsTopLevel()||item.GetCanvas()!=_canvas||!Same(item.GetGlobalTransform(),_transform)))return false;
                if(_node is Control control&&(control.ClipContents!=_clip||!Same(control.GetGlobalRect(),_rectangle)))return false;
                return true;
            }
        }
    }
