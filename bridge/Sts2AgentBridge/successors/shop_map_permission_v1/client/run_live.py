#!/usr/bin/env python3
"""One fixed campaign read and one frozen selected room controller invocation."""
...
def verify_sources() -> None:
    for component in (*FROZEN, 'shop_map_permission_v1'):
        ...
        if manifest.get('schema_version') != 1 or manifest.get('component') != component:
            raise ValueError()
        if component == 'shop_map_permission_v1' and manifest.get('contract_sha256') != CONTRACT:
            raise ValueError()
        names = set()
...
def main() -> int:
    result = {'schema_version': 1, 'status': 'failed', 'code': 'room_client_preflight_failed'}
    try:
        args = sys.argv[1:]
        ...
        from room_transport import run_authenticated_flow
        result = run_once(args[1], manager.validate_installed_for_client, read_credential,
                          run_authenticated_flow, manager.require_no_granting_acl_fd)
