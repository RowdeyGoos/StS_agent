...
_POLICY = manager.ArtifactPolicy(
    ...
    archive=manager.ArtifactSpec(
        "Sts2AgentBridgeRoomFlowsV1-0.1.0.zip",
        len(_ARCHIVE_DATA),
...
class FixtureTree:
    def __init__(self, root: Path, *, preexisting_mods: bool = False, flow_kind: str = "shop") -> None:
        ...
        self.overlay_root = self.mods_parent / "Sts2AgentBridgeRoomFlowsV1"
        self.operator_parent = self.application_support / "Sts2AgentBridge"
        self.config_root = self.operator_parent / "room_flows_v1"
...
def _assert_recorded_retention(
    ...
    for overlay_unit in present_overlays:
        overlay_root = (
            overlay_unit / "Sts2AgentBridgeRoomFlowsV1"
...
def _verify_production_bindings() -> None:
    if manager.CAMPAIGN_ID != "SHOP-MAP-PERMISSION-V1-SMOKE-V1":
    ...
    if manager.CONFIGURATIONS != expected_configs or manager.CONFIG_HASHES != {
        "shop": "febe931cbb5b2864744d27858e93e2a809b30ba58e121e47071b6040c0bca55c",
        "event": "22533f994afda1a269c0583dc13c3ca1cfc9b53c35eb30d299b1cef4293bb36f",
    }:
    ...
    if (
        ...
        / "Library"
        / "Application Support"
        / "Sts2AgentBridgeCampaign-shop-map-permission-v1-smoke-v1"
        or manager.ARTIFACT_ROOT
        != Path("/private/tmp/sts-shop-map-permission-v1-release")
...
def _exercise_flow_and_config_immutability(root: Path) -> None:
    ...
    changed = replace(
        current,
        flow_kind="event",
        config_sha256=manager.CONFIG_HASHES["event"],
        phase="quarantining",
    ...
    document = json.loads(state_path.read_text(encoding="ascii"))
    document["flow_kind"] = "event"
    document["config_sha256"] = manager.CONFIG_HASHES["event"]
    ...
    config_path.write_bytes(manager.configuration_bytes("event"))
    config_path.chmod(0o600)
    _expect_failure(
...
def _exercise_client_validation_api(root: Path) -> None:
    ...
    config_path.unlink()
    config_path.write_bytes(manager.configuration_bytes("shop"))
    config_path.chmod(0o600)
    with (
        mock.patch.object(manager.sys, "platform", "darwin"),
        mock.patch.object(manager.os, "geteuid", return_value=501),
