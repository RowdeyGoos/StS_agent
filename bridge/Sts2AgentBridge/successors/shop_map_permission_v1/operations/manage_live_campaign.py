...
CAMPAIGN_ID = "SHOP-MAP-PERMISSION-V1-SMOKE-V1"
STATE_ROOT_NAME = "Sts2AgentBridgeCampaign-shop-map-permission-v1-smoke-v1"
LEGACY_STATE_ROOT_NAME = "Sts2AgentBridgeCampaign-r0i-batched-bridge-smoke-v1"
LEGACY_OVERLAY_ROOT_NAME = "Sts2AgentBridge"
ARTIFACT_ROOT = Path("/private/tmp/sts-shop-map-permission-v1-release")
CONFIGURATIONS = {
    "shop": b'{"schema_version":"room_flows_v1_transport_config_v1","enabled":true,"flow_kind":"shop","bind_address":"127.0.0.1","port":43117,"token_file":"credential.hex"}',
    "event": b'{"schema_version":"room_flows_v1_transport_config_v1","enabled":true,"flow_kind":"event","bind_address":"127.0.0.1","port":43117,"token_file":"credential.hex"}',
}
...
OVERLAY_DLL_NAME = "Sts2AgentBridgeRoomFlowsV1.dll"
OVERLAY_MANIFEST_NAME = "Sts2AgentBridgeRoomFlowsV1.json"
...
CANONICAL_ARTIFACTS = ArtifactPolicy(
    ...
    archive=ArtifactSpec(
        "Sts2AgentBridgeRoomFlowsV1-1.0.0.zip",
        177900,
...
def parse_args(
    ...
    flow_kind = None
    if mode == "install":
        if len(values) != 10 or values[8] != "--flow-kind" or values[9] not in ("shop", "event"):
            fail(EXIT_INVALID_INVOCATION, "invalid_invocation")
        flow_kind = values[9]
    elif len(values) != 8:
        fail(EXIT_INVALID_INVOCATION, "invalid_invocation")
...
def _production_layout(user_profile: Path) -> CampaignLayout:
    ...
    overlay_root = mods_parent / "Sts2AgentBridgeRoomFlowsV1"
    operator_parent = application_support / "Sts2AgentBridge"
    config_root = operator_parent / "room_flows_v1"
...
def _validate_operator_unit(root: Path, uid: int, flow_kind: str) -> dict[Path, os.stat_result]:
    records: dict[Path, os.stat_result] = {}
    records[root] = _require_directory(root, uid, 0o700)
    _list_exact(root, {"room_flows_v1"})
    records.update(_validate_config_tree(root / "room_flows_v1", uid, flow_kind))
    return records
...
def _validate_operator_unit_metadata(
    ...
    records[root] = _require_directory(root, uid, 0o700)
    _list_exact(root, {"room_flows_v1"})
    records.update(_validate_config_tree_metadata(root / "room_flows_v1", uid, flow_kind))
    return records
...
def _validate_overlay_unit(
    ...
    _list_exact(unit_root, {"Sts2AgentBridgeRoomFlowsV1"})
    records.update(
        _validate_overlay_tree(unit_root / "Sts2AgentBridgeRoomFlowsV1", uid, policy)
    )
    return records
...
def _require_state_records(
    ...
    config_root = operator_root / "room_flows_v1"
    overlay_root = (
        overlay_unit_root / "Sts2AgentBridgeRoomFlowsV1"
        if state.mods_parent_created
...
def _install(
    ...
    try:
        ...
        with _private_umask():
            ...
            staged_config = layout.staged_operator / "room_flows_v1"
            ...
            if mods_parent_created:
                staged_overlay_root = layout.staged_overlay / "Sts2AgentBridgeRoomFlowsV1"
...
def _validate_purge_remainder(
    ...
    if progress < 4:
        ...
        expected_operator = {"room_flows_v1"} if progress < 3 else set()
    ...
    if progress < overlay_removed_at:
        ...
        if state.mods_parent_created:
            expected_unit = {"Sts2AgentBridgeRoomFlowsV1"} if progress < 7 else set()
...
def _run(
    ...
    *,
    ...
    if mode == "install":
        if flow_kind not in ("shop", "event"):
