...
internal sealed record ShopMapPermissionReleaseProjection(
    string AssemblyName,
...
internal static partial class ShopMapPermissionReleaseProjectionBuilder
{
    private static readonly Regex RoutePattern = new(
        @"/probe/[A-Za-z0-9_./-]*",
        RegexOptions.CultureInvariant);

    internal static ShopMapPermissionReleaseProjection Build(byte[] image)
    {
        try
        {
            ...
            return new ShopMapPermissionReleaseProjection(
                assemblyName,
                assemblyVersion,
                Sha(image),
                image.LongLength,
