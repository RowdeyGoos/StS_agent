...
internal static class Program
{
    public static int Main(string[] args)
    {
        try
        {
            ...
            string policySha = ShopMapPermissionReleaseProjectionBuilder.Sha(policyBytes);
            if (policySha != ShopMapPermissionReleasePolicy.ExpectedPolicySha256)
            {
                throw new VerificationException("policy_identity_mismatch");
            }
            ShopMapPermissionReleasePolicy policy = ShopMapPermissionReleasePolicy.Parse(policyBytes);
            byte[] candidate = SafeFiles.ReadBoundedRegularFile(parsed.Assembly, 64 * 1024 * 1024, "candidate_boundary");
            ShopMapPermissionReleaseVerificationReport report = ShopMapPermissionReleaseVerifier.Verify(
            ...
            Emit(new SortedDictionary<string, object?>(StringComparer.Ordinal)
            {
                ...
                ["status"] = "passed",
                ["suite"] = "shop_map_permission_v1_release_surface",
            });
            return 0;
        }
