...
internal static class FactoryProgram
{
    ...
    public static int Main()
    {
        try
        {
            ...
            Console.WriteLine(JsonSerializer.Serialize(new { schema_version = 1, status = "passed",
                suite = "event_orchestrator_v1_native_factories", check_count = _checks }));
            return 0;
        }
        catch
        {
    ...
    private sealed class Surface
    {
        ...
        internal static Surface Create(EventModel model, string key)
        {
            ...
            value.Option = new EventOption { TextKey = key };
            value.Button = new NEventOptionButton { Event = model, Option = value.Option };
            value.Button.Named["%Text"] = new MegaRichTextLabel { Text = key };
            layout.OptionButtons.Add(value.Button);
