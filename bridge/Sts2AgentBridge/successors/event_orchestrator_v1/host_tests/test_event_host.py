...
def parent(key='INITIAL', *, prior=None, policy='item_reward', text='Choose', proceed=False):
    ...
    c = dict(candidate_index=0, action_id='choose:0', stable_id='PROCEED' if proceed else key, rendered_text=text, enabled=True, is_dangerous=False, is_proceed=proceed, child_policy='' if proceed else policy)
    p = dict(schema_version=1, kind='parent_observation', version='event_orchestrator_v1', session_nonce=N, parent_ordinal=1, status='ready', phase='proceed' if proceed else 'choose_option', decision_id='', candidates=[c], legal_actions=['choose:0'], child=None, prior_result=prior)
...
def resolved(p):
    return dict(schema_version=1, kind='parent_resolved', version='event_orchestrator_v1', session_nonce=N, parent_ordinal=1, status='resolved', phase='complete', decision_id=p['decision_id'], action_id='choose:0', result='map_handoff', prior_result=prior(p, 'map_handoff'))
...
def envelope(p, c=None):
    return dict(schema_version=1, protocol='event_orchestrator_v1', session_nonce=N, child=c, payload=p)
...
class Transport:
    ...
    def __call__(self, method, route, body):
        if method == 'GET':
            ...
            c = v['child']
            if c is None:
                p = dict(schema_version=1, kind='parent_receipt', version='event_orchestrator_v1', session_nonce=N, parent_ordinal=1, status='accepted', decision_id=v['decision_id'], action_id=v['action_id'])
