    ...
    try:
        ...
        for header in (b"Authorization: Bearer " + b"d" * 64,
            ...
            with socket.create_connection(('127.0.0.1', port), timeout=1) as probe:
                probe.sendall(b"GET /probe/generic-event-v6/public/decision HTTP/1.1\r\nHost: 127.0.0.1:43117\r\n" + header + b"\r\nAccept: application/json\r\nConnection: close\r\n\r\n")
        ...
        with mock.patch.object(transport, '_perform_exchange', observe):
            ...
            def collect(owned):
                ...
                return transport._run_with_socket_factory(owned, Redirect,
                    clock=time.monotonic, sleep=time.sleep, provider=transport.first_legal)
        ...
        lost = scenario.startswith('lost_') or scenario == 'native_lost_item'
        unsupported = scenario in ('native_binding_wait', 'native_selector_wait', 'native_unsupported', 'native_candidate_card_type', 'native_item_failure', 'native_item_late_effect')
        expected_diagnostic = {'native_derived_hitbox':'map_ready','native_candidate_card_type':'candidate_card_type','native_binding_wait':'pending_screen','native_selector_wait':'prepare_geometry','native_unsupported':'pending_binding_failed','native_normal':'map_ready','native_selector_release':'map_ready','provider_throw':'diagnostic_unavailable','provider_invalid':'diagnostic_unavailable'}.get(scenario, 'child_ready' if scenario in ('native_item_failure', 'native_item_late_effect', 'native_lost_item') else 'map_ready' if scenario.startswith('native_') else 'none')
        ...
        if unsupported and not scenario.startswith('native_item') and (result['parent_accepted'] != 1 or result['parent_reconciled'] != 0 or result['child_accepted'] != 0):
        ...
        cards = (2 if scenario == 'native_item_mixed' else 0) if items else (episodes if not lost and not unsupported else 0)
