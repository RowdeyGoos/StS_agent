...
internal sealed class ProductionGenericEventRuntimeFactory : IGenericEventBootstrapRuntimeFactory
{
    ...
    private static GenericEventV6WireService CreateService(GenericEventReleaseSelection selection, string nonce, Runtime owner)
    {
    ...
    private static GenericEventV6WireService CreateVerifiedService(string nonce, Runtime owner)
    {
        IGenericEventV6NativeAdapter? adapter = null;
        GenericEventV6Session? session = null;
        try
        {
            var pinned = new PinnedGenericEventV6NativeAdapter();
            adapter = pinned;
            owner.BindDiagnostic(pinned);
            session = new GenericEventV6Session(adapter, nonce);
            adapter = null;
            var service = new GenericEventV6WireService(nonce, session);
            session = null;
            return service;
        }
        catch
        {
            ...
            throw new GenericEventTransportFactoryFailure(() =>
            {
                ...
                else GenericEventV6Hooks.RecoverFailedInstallation();
            });
        }
    }
    ...
    private sealed class Runtime : IGenericEventBootstrapRuntime
    {
        ...
        private PinnedGenericEventV6NativeAdapter? _diagnosticTarget;
        private bool _diagnosticBound;
        internal void BindDiagnostic(PinnedGenericEventV6NativeAdapter target)
        {
