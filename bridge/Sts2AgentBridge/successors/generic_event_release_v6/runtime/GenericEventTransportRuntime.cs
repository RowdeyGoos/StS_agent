...
public sealed class GenericEventTransportRuntime : IDisposable
{
    ...
    private GenericEventV6WireService? _service;
    private Action? _ownerFrameCleanup;
    private Func<GenericEventDiagnosticCode>? _diagnosticProvider;
    ...
    public static GenericEventTransportRuntime? Create(
        ...
        Func<GenericEventReleaseSelection, string, GenericEventV6WireService> factory,
        Func<GenericEventDiagnosticCode>? diagnosticProvider = null)
    {
        ...
        try
        {
            ...
            try
            {
                ...
                authenticator = null;
                GenericEventV6WireService service;
                try
                {
    ...
    public bool DisposeServiceOnOwnerFrame()
    {
        ...
        try
        {
            ...
            lock (_gate) { _serviceDisposed = true; _service = null; _ownerFrameCleanup = null; _diagnosticProvider = null; }
            return true;
        }
        catch
        {
    ...
    private async Task HandleSocketAsync(Socket socket, CancellationToken stopToken)
    {
        ...
        try
        {
            ...
            string route = parsed.IsPost ? GenericEventV6WireService.ActionRoute : GenericEventV6WireService.DecisionRoute;
            OwnedByteDispatchResult dispatch = _frameQueue.Submit(() =>
            {
...
#endif
                GenericEventV6WireService service = _service ??
                ...
                try
                {
                    ...
                    return new OwnedServiceResponse(200, result, CaptureDiagnostic());
                }
                finally
                {
...
#endif
            response = GenericEventTransportHttpEncoder.Wrap(body, dispatch.Diagnostic);
    ...
    private GenericEventDiagnosticCode CaptureDiagnostic()
    {
        try { return GenericEventDiagnosticCodec.Normalize(_diagnosticProvider?.Invoke() ?? GenericEventDiagnosticCode.NotCaptured); }
        catch { return GenericEventDiagnosticCode.DiagnosticUnavailable; }
    }
...
internal static class GenericEventTerminalClassifier
{
        ...
        string nonce, int statusCode, byte[] body)
    {
        ...
        try
        {
            ...
            if (!Bounded(root) || !Keys(root, "schema_version", "protocol", "session_nonce", "kind", "parent", "child", "payload") ||
                root.GetProperty("schema_version").GetInt32() != 1 || Text(root,"protocol") != "generic_event_v6" || Text(root,"session_nonce") != nonce)
            ...
            if (kind == "action")
            {
                ...
                if (route == GenericEventTransportRoute.ParentPost)
                {
                    if (!Null(child) || !Keys(payload,"version","session_nonce","decision_id","action_id","outcome") ||
                        Text(payload,"version") != "generic_event_v6") return TerminalClassification.Invalid;
                }
    ...
    private static bool Child(JsonElement value) {
        ...
        return item ? Text(value,"contract_version")=="item_v1"&&value.GetProperty("offer_count").GetInt32()==1 :
            Text(value,"kind")=="card_selection"&&Text(value,"contract_version")==GenericEventV6Families.ContractVersion(Text(value,"operation")??"")&&
            GenericEventV6Families.Supports(Text(value,"operation")??"",value.GetProperty("min_select").GetInt32(),value.GetProperty("max_select").GetInt32(),Text(value,"commit_mode")??"",value.GetProperty("domain_count").GetInt32());
    }
