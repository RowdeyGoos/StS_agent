...
class Fixture:
    ...
    def __enter__(self):
        ...
        self.item = self.home / 'Library' / 'Application Support' / 'Sts2AgentBridge' / 'generic_event_v6'
...
class ClientTests(unittest.TestCase):
    ...
    def test_config_failures_no_credential_read(self):
        for flow in ("generic",):
            values=[reader.CONFIGURATIONS[flow].replace(b'true',b'false'),b'{}',reader.CONFIGURATIONS[flow]+b'\n',reader.CONFIGURATIONS[flow].replace(b'generic_event_v6',b'live_probe_v0'),reader.CONFIGURATIONS[flow].replace(b'generic',b'smith')]
            for data in values:
