...
internal static class ReleaseNativeTests
{
    ...
    private static byte[] Configuration() => Encoding.UTF8.GetBytes("{\"schema_version\":\"generic_event_v6_transport_config_v1\",\"enabled\":true,\"flow_kind\":\"generic\",\"bind_address\":\"127.0.0.1\",\"port\":43117,\"token_file\":\"credential.hex\"}");
    ...
    internal static int Main()
    {
        try
        {
            ...
            Console.WriteLine("{\"status\":\"passed\",\"suite\":\"generic_event_v6_native_release\",\"check_count\":" + _checks + "}"); return 0;
        }
    ...
    private static void FactoryDefaultDeny()
    {
        ...
        Check(Harmony.GetAllPatchedMethods().Count(m=>Harmony.GetPatchInfo(m)!.Owners.Contains("sts2agent.generic_event_v6"))==18);
    }
    private static GenericEventV6WireService InvokeVerified(string nonce,object? owner=null)
    {
        try { return (GenericEventV6WireService)typeof(ProductionGenericEventRuntimeFactory).GetMethod("CreateVerifiedService",BindingFlags.NonPublic|BindingFlags.Static)!.Invoke(null,new object[]{nonce,owner??Activator.CreateInstance(typeof(ProductionGenericEventRuntimeFactory).GetNestedType("Runtime",BindingFlags.NonPublic)!,true)!})!; }
        catch(TargetInvocationException error) { throw error.InnerException!; }
    }
    private static void RetainedFailedConstruction()
    {
        bool rejectCleanup=true;int cleanupCalls=0;bool failed=false;
        try { _=new GenericEventV6Hooks(_=>throw new InvalidOperationException(),()=> {cleanupCalls++;if(rejectCleanup)throw new InvalidOperationException();}); }
    ...
    private static void FactoryDiagnosticOwnership()
    {
        ...
        var target=(PinnedGenericEventV6NativeAdapter)ownerType.GetField("_diagnosticTarget",BindingFlags.Instance|BindingFlags.NonPublic)!.GetValue(owner)!;
        ...
        var prefix=typeof(GenericEventV6Hooks).GetMethod("ChosenPrefix",BindingFlags.Static|BindingFlags.NonPublic)!;
