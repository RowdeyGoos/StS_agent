...
import argparse,difflib,hashlib,json,os,stat,subprocess,sys,types
from pathlib import Path
import xml.etree.ElementTree as ET
ROOT=Path(__file__).absolute().parent
PREFIX='bridge/Sts2AgentBridge/successors/'
PREVIOUS_CHECK='633e4bc3a826f78d2f8ecd97e5305359e24bce08abba7fb7714f49cff44dadd0'
PREVIOUS_MANIFEST='d6cc289c5722ff3f4c4501f728718a9562ebe60f2ef24eb79df0f185c0d064b8'
PREVIOUS_INVENTORY='042fd7a640980db8e971de022a5653f22942d9a86a288a2f83783c4581cf29bb'
HARMONY_BYTES=2328064
HARMONY_SHA='ef1898322c9f5c86dc1b0758b272a9c440823b4a41ca9a0b82a3aa6b3d206387'
PRODUCTION='production/Sts2AgentBridgeGenericEventV6.csproj'
def sha(data):return hashlib.sha256(data).hexdigest()
def require(value,code):
    if not value:raise ValueError(code)
def read_own_source(path):
    # The larger G6 policy/derivation are repository metadata, never target inputs.
...
def sources():
    path=ROOT.parent/'generic_event_v6/check.py';require(not path.is_symlink(),'predecessor_link')
    ...
    inventories=dict(inventories,generic_event_v6=prior);require(len(inventories)==26,'predecessor_count')
    ...
    contract=ROOT.parents[3]/'docs/PHASE_1_GENERIC_EVENT_RELEASE_V6_CONTRACT.md';helper.reject_links(contract);contract_hash=sha(helper.read_regular(contract))
    identity=ROOT/'source_identity.json'
    if identity.exists():
        require(json.loads(identity.read_bytes())==dict(schema_version=1,component='generic_event_release_v6',contract_sha256=contract_hash,inventory_sha256=inventory,files={name:sha(data) for name,data in own.items()}),'source_identity')
    inputs=dict(inputs);inputs.update({PREFIX+'generic_event_release_v6/'+name:data for name,data in own.items()})
...
def main():
    ...
    require(set(projects)=={PRODUCTION,'runtime/Sts2AgentBridge.GenericEventV6.Transport.csproj',
        'runtime_tests/GenericEventReleaseV6.RuntimeTests.csproj','integration/GenericEventReleaseV6.SocketFixture.csproj',
        'operator_tests/Sts2AgentBridge.GenericEventV6.Operator.Tests.csproj',
        'lifecycle_tests/Sts2AgentBridge.GenericEventV6.Bootstrap.Tests.csproj',
        'native_tests/Sts2AgentBridge.GenericEventV6.Native.Tests.csproj',
        'verifier/Sts2AgentBridge.GenericEventV6.Release.Verifier.csproj',
        'verifier_tests/Sts2AgentBridge.GenericEventV6.Release.Verifier.Tests.csproj'},'project_inventory')
    production_sources=closure(inputs,PREFIX+'generic_event_release_v6/'+PRODUCTION,True)
    gameplay=closure(inputs,PREFIX+'generic_event_v6/native/GenericEventV6.Native.csproj',True)|closure(inputs,PREFIX+'generic_event_v6/wire/GenericEventV6.Wire.csproj',False)
    require(gameplay.issubset(production_sources),'frozen_g6_gameplay_closure')
    ...
    for p in production_sources:
        if p.startswith(PREFIX+'generic_event_release_v6/native/'):
    ...
    native_fixture_sources=closure(inputs,PREFIX+'generic_event_release_v6/native_tests/Sts2AgentBridge.GenericEventV6.Native.Tests.csproj',False)
    ...
    compiled=set()
    for p in projects:compiled.update(closure(inputs,PREFIX+'generic_event_release_v6/'+p,p==PRODUCTION))
    require({PREFIX+'generic_event_release_v6/'+p for p in own if p.endswith('.cs')}.issubset(compiled),'uncompiled_source')
    ...
    def snapshot(label):
        ...
        doc=source/'docs/PHASE_1_GENERIC_EVENT_RELEASE_V6_CONTRACT.md';doc.parent.mkdir(exist_ok=True);doc.write_bytes((ROOT.parents[3]/'docs/PHASE_1_GENERIC_EVENT_RELEASE_V6_CONTRACT.md').read_bytes())
        if (ROOT/'source_identity.json').exists():(source/PREFIX/'generic_event_release_v6/source_identity.json').write_bytes((ROOT/'source_identity.json').read_bytes())
        ...
        component=source/PREFIX/'generic_event_release_v6';nuget=dest/'NuGet.Config';nuget.write_text('<configuration><packageSources><clear /></packageSources></configuration>\n')
    ...
    g6_scratch=args.scratch.with_name(args.scratch.name+'-g6')
    g6_run=run([sys.executable,'-B',str(ROOT.parent/'generic_event_v6/check.py'),'--dotnet',dotnet,'--game-data-dir',str(args.game_data_dir),'--scratch',str(g6_scratch)],ROOT,env)
    g6=json.loads((g6_scratch/'result.json').read_bytes())
    require(g6['status']=='passed' and g6['source_inventory_sha256']==PREVIOUS_INVENTORY,'frozen_g6_regression')
    results['frozen_g6_regression']=dict(result_sha256=sha((g6_scratch/'result.json').read_bytes()),tests=g6['tests'])
    for p in projects:
        outputs[p]=build(p)
        if p.startswith(('runtime_tests/','lifecycle_tests/','operator_tests/','native_tests/')):
    ...
    verifier=outputs['verifier/Sts2AgentBridge.GenericEventV6.Release.Verifier.csproj']
    ...
    vp='verifier_tests/Sts2AgentBridge.GenericEventV6.Release.Verifier.Tests.csproj'
