...
_MANIFEST_SHA256 = "c7480a29c041269b9b0991826fa2fd4408cc8673a4427dac9ca299c9396ece62"
...
def _write_package(path: Path, dll: bytes, manifest: bytes) -> bytes:
    with zipfile.ZipFile(
        ...
        for name, data in (
            (
                "Sts2AgentBridgeGenericEventV6/Sts2AgentBridgeGenericEventV6.dll",
                dll,
            ),
            (
                "Sts2AgentBridgeGenericEventV6/Sts2AgentBridgeGenericEventV6.json",
                manifest,
            ),
        ):
...
class _Fixture:
    ...
    def install_overlay(self, manifest: bytes | None = None) -> None:
        ...
        (directory / "Sts2AgentBridgeGenericEventV6.dll").write_bytes(self.dll)
        (directory / "Sts2AgentBridgeGenericEventV6.json").write_bytes(
...
def operation() -> dict[str, object]:
    ...
    with tempfile.TemporaryDirectory(
        ...
        mutated[mutated.index(b"Generic Event V6")] ^= 1
