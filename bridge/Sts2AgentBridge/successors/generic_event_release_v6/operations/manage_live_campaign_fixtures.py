...
_EXPECTED_CANONICAL_ARTIFACTS = (
    (
        "Sts2AgentBridgeGenericEventV6.dll",
        379392,
        "96723ba26f9f64a74cd0f1d8ae4217191a13410c5629a1feedc26a44f3890c2f",
    ),
    (
        "Sts2AgentBridgeGenericEventV6.json",
        352,
...
_POLICY = manager.ArtifactPolicy(
    ...
    archive=manager.ArtifactSpec(
        "Sts2AgentBridgeGenericEventV6-0.1.0.zip",
        len(_ARCHIVE_DATA),
...
class FixtureTree:
    def __init__(self, root: Path, *, preexisting_mods: bool = False, flow_kind: str = "generic") -> None:
        ...
        self.overlay_root = self.mods_parent / "Sts2AgentBridgeGenericEventV6"
        self.operator_parent = self.application_support / "Sts2AgentBridge"
        self.config_root = self.operator_parent / "generic_event_v6"
...
def _assert_recorded_retention(
    ...
    for overlay_unit in present_overlays:
        overlay_root = (
            overlay_unit / "Sts2AgentBridgeGenericEventV6"
...
def _verify_production_bindings() -> None:
    if manager.CAMPAIGN_ID != "GENERIC-EVENT-V6-SMOKE-V1":
    ...
    expected_configs = {"generic": b'{"schema_version":"generic_event_v6_transport_config_v1","enabled":true,"flow_kind":"generic","bind_address":"127.0.0.1","port":43117,"token_file":"credential.hex"}'}
    if manager.CONFIGURATIONS != expected_configs or manager.CONFIG_HASHES != {"generic": "bd90152d7eb5f700c272cb0aad2d87973f7acc727027bc9f176cd4759a56dfb0"}:
    ...
    if (
        ...
        / "Library"
        / "Application Support"
        / "Sts2AgentBridgeCampaign-generic-event-v6-smoke-v1"
        or manager.ARTIFACT_ROOT
        != Path("/private/tmp/sts-generic-event-v6-release")
