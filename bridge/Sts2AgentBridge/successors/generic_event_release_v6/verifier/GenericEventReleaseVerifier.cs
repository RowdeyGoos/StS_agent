...
{
    private const string ExpectedAssemblyName = "Sts2AgentBridgeGenericEventV6";
    private const string ExpectedAssemblyVersion = "1.0.0.0";
    private static readonly string[] ExpectedRoutes =
    {
        "/probe/generic-event-v6/public/action",
        "/probe/generic-event-v6/public/decision",
    };
    private static readonly string[] ExpectedRequestLines =
    {
        "GET /probe/generic-event-v6/public/decision HTTP/1.1",
        "POST /probe/generic-event-v6/public/action HTTP/1.1",
    };
    ...
    private const string ExpectedInitializer =
        "type:Sts2AgentBridge.Successors.GenericEventReleaseV6.GenericEventModEntry|" +
    ...
    private const string ExpectedEntryType =
        "type_def|Sts2AgentBridge.Successors.GenericEventReleaseV6.GenericEventModEntry|1048961|System.Object|0|0";
    private const string ExpectedEntryMethodSuffix =
        "|Sts2AgentBridge.Successors.GenericEventReleaseV6.GenericEventModEntry.Initialize()|System.Void|0|0|0|150|0";
    ...
    internal static void VerifySemanticRules(GenericEventReleaseProjection projection)
    {
        ...
        if (!projection.InitializerAttributes.SequenceEqual(new[] { ExpectedInitializer }, StringComparer.Ordinal) ||
            ...
            projection.Inventory.Count(value => value.StartsWith(
                ...
                "|Sts2AgentBridge.Successors.GenericEventReleaseV6.GenericEventModEntry.",
    ...
    private static void VerifyCriticalFlow(string[] references, string[] inventory)
    {
        ...
        const string guardedFactory = "Sts2AgentBridge.Successors.GenericEventReleaseV6.ProductionGenericEventRuntimeFactory.CreateService(Sts2AgentBridge.Successors.GenericEventReleaseV6.GenericEventReleaseSelection,System.String,Sts2AgentBridge.Successors.GenericEventReleaseV6.ProductionGenericEventRuntimeFactory+Runtime)";
        ...
        if (guards.Length != 1 || entries.Length != 1 || Owner(guards[0]) != guardedFactory ||
            ...
            throw new VerificationException("critical_flow_forbidden");
        string factory = "critical|Sts2AgentBridge.Successors.GenericEventReleaseV6.ProductionGenericEventRuntimeFactory.CreateVerifiedService(";
        string runtime = "critical|Sts2AgentBridge.Successors.GenericEventReleaseV6.GenericEventTransportRuntime+";
        ...
        string[] dispatch = references.Where(value => value.StartsWith(runtime, StringComparison.Ordinal) &&
            Target(value).Contains(".GenericEventV6WireService.Handle(", StringComparison.Ordinal)).ToArray();
        ...
        if (construction.Length != 3 || dispatch.Length != 1 || submissions.Length != 1 || callbacks.Length != 1 ||
            construction.Count(value => Target(value).Contains(".PinnedGenericEventV6NativeAdapter..ctor(", StringComparison.Ordinal)) != 1 ||
            construction.Count(value => Target(value).Contains(".GenericEventV6Session..ctor(", StringComparison.Ordinal)) != 1 ||
            construction.Count(value => Target(value).Contains(".GenericEventV6WireService..ctor(", StringComparison.Ordinal)) != 1)
        {
            throw new VerificationException("critical_flow_forbidden");
        }
        int pinned = Offset(construction.Single(value => Target(value).Contains(
            ".PinnedGenericEventV6NativeAdapter..ctor(", StringComparison.Ordinal)));
        int session = Offset(construction.Single(value => Target(value).Contains(
            ".GenericEventV6Session..ctor(", StringComparison.Ordinal)));
        int service = Offset(construction.Single(value => Target(value).Contains(
            ".GenericEventV6WireService..ctor(", StringComparison.Ordinal)));
