...
internal static partial class GenericEventReleaseProjectionBuilder
{
    private static readonly Regex RequestLinePattern = new(
        @"(?:GET|POST) /probe/generic-event-v6/[a-z0-9/-]+ HTTP/1\.1",
        ...
        try
        {
            ...
            foreach ((MethodDefinitionHandle handle, string owner) in methods)
            {
                ...
                for (int instructionIndex = 0; instructionIndex < instructions.Count; instructionIndex++)
                {
                    ...
                    if (owner.StartsWith("Sts2AgentBridge.Successors.GenericEventReleaseV6.ProductionGenericEventRuntimeFactory.CreateVerifiedService(",StringComparison.Ordinal) &&
    ...
    private static bool IsCriticalReference(string identity) =>
        ...
        identity.Contains(".PinnedGenericEventV6NativeAdapter..ctor(", StringComparison.Ordinal) ||
        identity.Contains(".GenericEventV6Session..ctor(", StringComparison.Ordinal) ||
        identity.Contains(".GenericEventV6WireService..ctor(", StringComparison.Ordinal) ||
        identity.Contains(".GenericEventV6WireService.Handle(", StringComparison.Ordinal) ||
        ...
        IReadOnlyDictionary<FieldDefinitionHandle, string> fields)
    {
        ...
        int inputIndex = target == "method:Sts2AgentBridge.Successors.GenericEventV6.GenericEventV6Session..ctor(Sts2AgentBridge.Successors.GenericEventV6.IGenericEventV6NativeAdapter,System.String)"
            ? index - 2 : index - 1;
    ...
    {
        const string hook = "Sts2AgentBridge.Successors.GenericEventV6.Native.GenericEventV6Hooks";
        ...
        bool guardOwner = owner.StartsWith(
            "Sts2AgentBridge.Successors.GenericEventReleaseV6.PinnedGenericEventHarmonyGuard.",
        ...
        bool allowedExit = owner == "Sts2AgentBridge.Successors.GenericEventReleaseV6.GenericEventBootstrapHost.Initialize()" &&
