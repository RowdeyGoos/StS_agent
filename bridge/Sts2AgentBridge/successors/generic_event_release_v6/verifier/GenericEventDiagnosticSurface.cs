...
{
    internal const string Prefix = "Sts2AgentBridge.Successors.GenericEventReleaseV6.";
    internal const string FrozenPrefix = "Sts2AgentBridge.Successors.GenericEventReleaseV5.";
    internal const string Native = "Sts2AgentBridge.Successors.GenericEventV6.Native.PinnedGenericEventV6NativeAdapter";
    ...
    internal static void Verify(string[] inventory, string[] critical)
    {
        ...
        var handle = critical.Where(x => x.Contains(".GenericEventV6WireService.Handle(",StringComparison.Ordinal)).Select(x => x.Split('|')).ToArray();
