...
internal static class Program
{
    ...
    private static readonly byte[] Config=Encoding.UTF8.GetBytes("{\"schema_version\":\"generic_event_v6_transport_config_v1\",\"enabled\":true,\"flow_kind\":\"generic\",\"bind_address\":\"127.0.0.1\",\"port\":43117,\"token_file\":\"credential.hex\"}");
    ...
    private static GenericEventTransportRuntime Create(InertFixture.InertEvent? adapter=null)=>GenericEventTransportRuntime.Create((byte[])Config.Clone(),()=>Encoding.ASCII.GetBytes(new string('c',64)),(_,nonce)=>new GenericEventV6WireService(nonce,new GenericEventV6Session(adapter??new("FOREST_ARCHIVE"),nonce)))!;
    ...
    private static void DiagnosticCleanup()
    {
        ...
        var runtime=GenericEventTransportRuntime.Create((byte[])Config.Clone(),()=>Encoding.ASCII.GetBytes(new string('c',64)),
            (_,nonce)=>new GenericEventV6WireService(nonce,new GenericEventV6Session(adapter,nonce)),()=>{samples++;return GenericEventDiagnosticCode.ParentReady;})!;
    ...
    private static void TypedClassifiers()
    {
        ...
        TerminalClassification Classify(object payload,bool apply=false,object? child=null) {
            byte[] body=JsonSerializer.SerializeToUtf8Bytes(new{schema_version=1,protocol="generic_event_v6",session_nonce=Nonce,kind=apply?"action":"decision",
        ...
        var transform=new{ordinal=1,parent_decision_id=Id,parent_action_id="choose:0",kind="card_selection",contract_version="card_transform_v1",operation="transform",min_select=2,max_select=2,commit_mode="preview_confirm",domain_count=3};
        ...
        Check(Classify(Card("card_transform_v1"),child:transform)==TerminalClassification.NonTerminal);
        Check(Classify(Card("card_selection_v1"),child:transform)==TerminalClassification.Invalid);
        Check(Classify(Card("card_transform_v1","child_observation","unsupported"),child:transform)==TerminalClassification.Terminal);
        Check(Classify(Resolved(),child:transform)==TerminalClassification.Invalid);
        Check(Classify(Card("card_transform_v1"))==TerminalClassification.Invalid);
    }
    ...
    private static void Classifiers()
    {
        ...
        {
            var adapter=new InertFixture.InertEvent(scenario);using var service=new GenericEventV6WireService(Nonce,new GenericEventV6Session(adapter,Nonce));int childResolved=0;
            for(int i=0;i<150;i++)
            {
                adapter.Advance();byte[] response=service.Handle("GET","/probe/generic-event-v6/public/decision",null);using var doc=JsonDocument.Parse(response);var root=doc.RootElement;var parent=root.GetProperty("parent");
                ...
                var receipt=service.Handle("POST","/probe/generic-event-v6/public/action",body);Check(GenericEventTerminalClassifier.Classify(child?GenericEventTransportRoute.ChildPost:GenericEventTransportRoute.ParentPost,GenericEventReleaseSelection.Generic,Nonce,200,receipt)==TerminalClassification.NonTerminal);
            }
        }
        foreach(string outcome in new[]{"unsupported","uncertain","rejected"})
        {
            byte[] response=JsonSerializer.SerializeToUtf8Bytes(new {schema_version=1,protocol="generic_event_v6",session_nonce=Nonce,kind="action",parent=(object?)null,
        ...
        {
            byte[] body=Encoding.UTF8.GetBytes("{\"schema_version\":1,\"protocol\":\"generic_event_v6\",\"session_nonce\":\""+Nonce+"\",\"kind\":\"action\",\"parent\":null,\"child\":null,\"payload\":{\"version\":\"generic_event_v6\",\"session_nonce\":\""+Nonce+"\",\"decision_id\":\""+Id+"\",\"action_id\":\"choose:0\",\"outcome\":\""+outcome+"\"}}");
