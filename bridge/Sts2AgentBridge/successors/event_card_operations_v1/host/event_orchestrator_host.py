...
DECISION_ROUTE = '/probe/event-card-operations-v1/public/decision'
...
def _projection(v: dict[str, Any], *, text: bool) -> str:
    ...
    for c in v['candidates']:
        ...
        string(c['child_policy']); integer(c['child_domain_count'])
    if text:
...
class _Controller:
    def __init__(self, request: Callable, provider: Callable, clock: Callable, sleep: Callable, policies: Mapping[str, _CardPolicy]):
        self.request, self.provider, self.clock, self.sleep = request, provider, clock, sleep
        self.policies = _catalog_copy(policies)
    ...
    def prior(self, p: dict[str, Any], *, resolved: bool = False) -> None:
        ...
        if result == 'option_transition':
            _require(self.parent_policy not in self.policies)
            self.child_window = False
    ...
    def parent_read(self, p: dict[str, Any]) -> str:
        ...
        for i, c in enumerate(p['candidates']):
            _keys(c, ('candidate_index', 'action_id', 'stable_id', 'rendered_text', 'enabled', 'is_dangerous', 'is_proceed', 'child_policy', 'child_domain_count'))
            ...
            name, domain = c['child_policy'], c['child_domain_count']
    ...
    def child_read(self, p: dict[str, Any]) -> str:
        ...
        _require(self.parent_policy in self.policies)
        ...
        if status == 'ready':
            _require('confirm' not in actions and
            ...
            _require(self.child_shape is not None and p['operation'] == policy.operation and
    ...
    def choose(self, p: dict[str, Any]) -> None:
        ...
        if self.child is None:
            ...
            self.parent_domain = candidate['child_domain_count']
        ...
        while True:
            ...
            if c is None:
                ...
                if self.child is None:
                    ...
                    _require((self.parent_policy == 'item_reward' and c['kind'] == 'item') or
                             (self.parent_policy in self.policies and c['kind'] == 'card_selection'))
                    self.child = c; self.child_window = False; self.child_done = False
                    self.child_accepted = []; self.child_history = 0; self.child_shape = None; self.item_offer = None; self.child_preview_seen = False
...
def run_event(request: Callable, *, provider: Callable, clock: Callable = time.monotonic, sleep: Callable = time.sleep) -> dict[str, Any]:
    """Drive one event using only the reviewed production policy catalog."""
...
def _run_with_catalog(request: Callable, *, provider: Callable, clock: Callable, sleep: Callable,
    ...
    try:
        if not callable(provider):
            raise _Stop('invalid_provider')
        controller = _Controller(request, provider, clock, sleep, policies)
