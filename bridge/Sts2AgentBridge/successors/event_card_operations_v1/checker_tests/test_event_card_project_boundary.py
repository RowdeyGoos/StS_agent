#!/usr/bin/env python3
"""Synthetic regressions for the event-card-operations build-input boundary."""
...
def _configure(components, project_hashes) -> None:
    ...
    gate.PURE_TESTS = (
        ("event_card_operations_v1/tests/Pure.Tests.csproj", "fixture_pure", 1),
    )
...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="event-card-operations-freeze-") as temporary:
    ...
    # Authority documents are exact pinned inputs, and oversize input must stop
    ...
    print(json.dumps({
        "schema_version": 1,
        "status": "passed",
        "suite": "event_card_operations_v1_project_boundary",
        "check_count": checks,
    }, separators=(",", ":")))
    return 0
