...
{
    public const string DecisionRoute="/probe/event-card-operations-v1/public/decision";
    public const string ActionRoute="/probe/event-card-operations-v1/public/action";
    ...
    public EventOrchestratorV1WireService(string nonce,IEventOrchestratorV1Session session)
        : this(nonce,session,new EventOrchestratorV1WireValidation(nonce)) { }
#if EVENT_CARD_OPERATIONS_TEST_SEAM
    internal EventOrchestratorV1WireService(string nonce,IEventOrchestratorV1Session session,
        IReadOnlyList<EventOrchestratorV1CardPolicyDefinition> definitions)
        : this(nonce,session,new EventOrchestratorV1WireValidation(nonce,definitions)) { }
#endif
    private EventOrchestratorV1WireService(string nonce,IEventOrchestratorV1Session session,
        EventOrchestratorV1WireValidation validation)
    {
        if(!RoomFlowIdentity.IsNonce(nonce))throw new ArgumentException("Invalid nonce.");
        _nonce=nonce;_validation=validation;_session=session??throw new ArgumentNullException(nameof(session));
    ...
    private void ValidateHistory(JsonElement x,string status)
    {
        ...
        // Exact operation, domain, selection and commit history are validated
        // against the accepted parent policy by _validation.Child.
    }
