...
internal sealed class OwnedByteFrameQueue : IDisposable
{
    ...
    public OwnedByteDispatchResult Submit(Func<OwnedServiceResponse> operation)
    {
        ...
        lock (_gate)
        {
            if (!_accepting)
            {
                return new OwnedByteDispatchResult(
                    OwnedByteDispatchStatus.Unavailable, 0, null);
            }
        }
        if (!_slots.Wait(QueueWaitMilliseconds))
        {
            return new OwnedByteDispatchResult(OwnedByteDispatchStatus.Busy, 0, null);
        }
    ...
    {
        private readonly Func<OwnedServiceResponse> _operation;
        private readonly object _gate = new();
        ...
        private byte[]? _value;
        private GenericEventDiagnosticCode _diagnostic;

        public WorkItem(Func<OwnedServiceResponse> operation)
        {
        ...
        public void Execute()
        {
            ...
            try
            {
                OwnedServiceResponse response = _operation();
                _statusCode = response.StatusCode;
                value = response.Body;
                diagnostic = response.Diagnostic;
                success = value is not null &&
                    value.Length is >= 1 and <= GenericEventTransportLimits.MaximumBody;
            }
            catch
            {
            }
            ...
            lock (_gate)
            {
                ...
                if (!discard)
                {
                    ...
                    _diagnostic = success ? diagnostic : GenericEventDiagnosticCode.NotCaptured;
                    SignalLocked();
                }
            }
            if (!success || discard)
        ...
        public OwnedByteDispatchResult TakeResult()
        {
            lock (_gate)
            {
                ...
                {
                    return new OwnedByteDispatchResult(_status, 0, null);
                }
                _phase = WorkPhase.Transferred;
                byte[]? value = _value;
                _value = null;
                GenericEventDiagnosticCode diagnostic = _diagnostic;
                _diagnostic = GenericEventDiagnosticCode.NotCaptured;
                return new OwnedByteDispatchResult(_status, _statusCode, value, diagnostic);
            }
        }
