...
def main():
    ...
    frozen['card_selection_release_v1'] = own
    gate.verify_projects(frozen)
    cases = []
    production = gate.PRODUCTION
    runtime = 'runtime/Sts2AgentBridge.CardSelectionV1.Transport.csproj'
    def edited(name, old, new):
        def apply(c):
            assert old in c['card_selection_release_v1'][name]
            c['card_selection_release_v1'][name] = c['card_selection_release_v1'][name].replace(old, new, 1)
        return apply
    def added(name, content):
        return lambda c: c['card_selection_release_v1'].__setitem__(name, content)
    cases += [added('unknown.csproj', own[runtime]),
        ...
        edited(runtime, b'CardSelectionTransportRuntime.cs', b'*.cs'),
        edited(runtime, b'CardSelectionTransportRuntime.cs', b'/synthetic/foreign.cs'),
        ...
        lambda c: c['card_selection_release_v1'].pop('runtime/CardSelectionTransportRuntime.cs')]
    def policy_change(kind):
        def apply(c):
            name = 'policy/card_selection_release_policy.json'
            value = json.loads(c['card_selection_release_v1'][name])
            if kind == 'order':
            ...
            c['card_selection_release_v1'][name] = json.dumps(value).encode()
        return apply
    ...
    print(json.dumps({'schema_version':1,'status':'passed','suite':'card_selection_release_project_boundary','check_count':1+len(cases)}, separators=(',', ':')))
