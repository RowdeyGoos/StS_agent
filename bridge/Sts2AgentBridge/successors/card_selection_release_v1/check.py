...
CONTRACT = "0d66520d5f7f14a78f9d6e3b35fd4438f7811661a5e8a399b33920da4f2fd0f6"
...
PROJECT_HASHES: dict[str, str] = {
    "lifecycle_tests/Sts2AgentBridge.CardSelectionV1.Bootstrap.Tests.csproj": "de18bb4a0bd0ac433723714a22ec9c45d28fe7b26990544567eb72bf7a170e9b",
    "operator_tests/Sts2AgentBridge.CardSelectionV1.Operator.Tests.csproj": "a4e6db77811950ba1e9982813ce318408186513cfd9d9dcb8af3215d555d9477",
    "production/Sts2AgentBridgeCardSelectionV1.csproj": "b617e0e89f1b255b64b28291de8669d1e1afe072388243fbb3d08f15dcaef914",
    "runtime/Sts2AgentBridge.CardSelectionV1.Transport.csproj": "9392c2a4ab3f5b3f358f608f02d54288ebd8f566e3fa962652258d520e3fdc1f",
    "runtime_tests/Sts2AgentBridge.CardSelectionV1.Transport.Tests.csproj": "4d8100503bc6161013bedbd1d64d75932dfb4f44f38d9439a2ef6603465dad66",
...
PURE_TESTS: tuple[tuple[str, str, int], ...] = (
    ("operator_tests/Sts2AgentBridge.CardSelectionV1.Operator.Tests.csproj", "card_selection_v1_operator", 5),
    ("lifecycle_tests/Sts2AgentBridge.CardSelectionV1.Bootstrap.Tests.csproj", "card_selection_v1_bootstrap", 14),
    ("runtime_tests/Sts2AgentBridge.CardSelectionV1.Transport.Tests.csproj", "card_selection_v1_transport", 13),
)
...
def verify_projects(components: dict[str, dict[str, bytes]]) -> None:
    own = components["card_selection_release_v1"]
    if len(PROJECT_HASHES) != 7 or len(PURE_TESTS) != 3:
    ...
    for name, digest in PROJECT_HASHES.items():
        ...
        for node in tree.iter():
            ...
            full = os.path.normpath("card_selection_release_v1/" + str(Path(name).parent / value))
    ...
    if {"card_selection_release_v1/" + p for p in own if p.endswith(".cs")} != {p for p in compiled if p.startswith("card_selection_release_v1/")}:
        fail("uncompiled_new_source")
    for name in PROJECT_HASHES:
        if name == PRODUCTION:
            continue
        pending = ["card_selection_release_v1/" + name]
        visited = set()
        while pending:
            path = pending.pop()
            if path in visited:
            ...
            for node in tree.iter("ProjectReference"):
                ...
                if reference not in all_inputs or reference == "card_selection_release_v1/" + PRODUCTION:
...
def passed(output, suite, count=None):
    ...
    if suite == "card_selection_release_package":
...
def main():
    ...
    components["card_selection_release_v1"], inventory = helper.source_snapshot(ROOT, "card_selection_release_v1", CONTRACT)
    ...
    def snapshot_at(destination):
        ...
        own = siblings / "card_selection_release_v1"
    ...
    for project, suite, count in PURE_TESTS:
        assembly = build("card_selection_release_v1/" + project)
        assemblies[suite] = assembly
        arguments = [dotnet, str(assembly)]
        if project.startswith("operator_tests/"):
    ...
    summaries["socket_composition"] = passed(run([sys.executable, "-B", "-I", "-S",
        str(own / "integration/test_card_selection_socket_composition.py"), "--dotnet", dotnet,
    ...
    for relative, suite, count, unit in (
        ("checker_tests/test_card_selection_release_project_boundary.py", "card_selection_release_project_boundary", 15, False),
        ("client_tests/client_fixtures.py", "card_selection_release_client", 13, True),
        ...
        ("operations/predecessor_conflict_fixtures.py", "card_selection_release_predecessor_conflicts", 18, False),
        ("operations/verify_source_derivation.py", "card_selection_release_operations_derivation", 13, False)):
        summaries[suite] = passed(run([sys.executable, "-B", "-I", "-S", str(own / relative)], own, env,
                                     unittest_count=count if unit else None), suite, count)
    output = run([sys.executable, "-B", "-I", "-S", str(own / "transport_tests/test_card_selection_transport.py")], own, env, unittest_count=14)
    if output:
        fail("transport_fixture_stdout")
    summaries["python_transport"] = {"schema_version": 1, "status": "passed", "suite": "card_selection_release_python_transport", "check_count": 14}
    candidate = build("card_selection_release_v1/" + PRODUCTION, native=True)
    _, _, _, _, second_build = snapshot_at(args.scratch / "second")
    second = second_build("card_selection_release_v1/" + PRODUCTION, native=True)
    if candidate.read_bytes() != second.read_bytes():
        fail("candidate_reproduction")
    verifier = build("card_selection_release_v1/verifier/Sts2AgentBridge.CardSelectionV1.Release.Verifier.csproj")
    mutations = build("card_selection_release_v1/verifier_tests/Sts2AgentBridge.CardSelectionV1.Release.Verifier.Tests.csproj")
    ...
    summaries["verifier_cli"]["suite"] = "card_selection_release_cli"
    summaries["package"] = passed(run([sys.executable, "-B", "-I", "-S", str(own / "package/package_fixtures.py"), str(candidate)], own, env, unittest_count=5), "card_selection_release_package", 5)
    ...
    result = {"schema_version": 1, "status": "passed", "suite": "card_selection_release_v1_offline",
