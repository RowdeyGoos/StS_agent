...
internal static class ReleaseNativeTests
{
    ...
    private static byte[] Configuration() => Encoding.UTF8.GetBytes("{\"schema_version\":\"generic_event_v10_transport_config_v1\",\"enabled\":true,\"flow_kind\":\"generic\",\"bind_address\":\"127.0.0.1\",\"port\":43117,\"token_file\":\"credential.hex\"}");
    ...
    internal static int Main()
    {
        try
        {
            ...
            Console.WriteLine("{\"status\":\"passed\",\"suite\":\"generic_event_v10_native_release\",\"check_count\":" + _checks + "}"); return 0;
        }
    ...
    private static void FactoryDefaultDeny()
    {
        ...
        config=Encoding.UTF8.GetBytes(Encoding.UTF8.GetString(Configuration()).Replace("generic_event_v10_transport_config_v1","generic_event_v7_transport_config_v1",StringComparison.Ordinal));credential=Credential();
        Check(factory.Create(config,credential) is null);Check(config.All(x=>x==0) && credential.All(x=>x==0));
        config=Configuration();credential=Credential();
        Check(factory.Create(config,credential) is null);Check(config.All(x=>x==0) && credential.All(x=>x==0));
        using var hooks=new GenericEventV7Hooks();
        Check(Harmony.GetAllPatchedMethods().Count(m=>Harmony.GetPatchInfo(m)!.Owners.Contains("sts2agent.generic_event_v7"))==18);
    }
    private static GenericEventV7WireService InvokeVerified(string nonce,object? owner=null)
    {
        try { return (GenericEventV7WireService)typeof(ProductionGenericEventRuntimeFactory).GetMethod("CreateVerifiedService",BindingFlags.NonPublic|BindingFlags.Static)!.Invoke(null,new object[]{nonce,owner??Activator.CreateInstance(typeof(ProductionGenericEventRuntimeFactory).GetNestedType("Runtime",BindingFlags.NonPublic)!,true)!})!; }
        catch(TargetInvocationException error) { throw error.InnerException!; }
    }
    private static void RetainedFailedConstruction()
    {
        bool rejectCleanup=true;int cleanupCalls=0;bool failed=false;
        try { _=new GenericEventV7Hooks(_=>throw new InvalidOperationException(),()=> {cleanupCalls++;if(rejectCleanup)throw new InvalidOperationException();}); }
    ...
    private static void FactoryDiagnosticOwnership()
    {
        ...
        var target=(PinnedGenericEventV7NativeAdapter)ownerType.GetField("_diagnosticTarget",BindingFlags.Instance|BindingFlags.NonPublic)!.GetValue(owner)!;
        ...
        var prefix=typeof(GenericEventV7Hooks).GetMethod("ChosenPrefix",BindingFlags.Static|BindingFlags.NonPublic)!;
