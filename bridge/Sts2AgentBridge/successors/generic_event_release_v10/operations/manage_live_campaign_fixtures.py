...
_POLICY = manager.ArtifactPolicy(
    ...
    archive=manager.ArtifactSpec(
        "Sts2AgentBridgeGenericEventV10-0.1.0.zip",
        len(_ARCHIVE_DATA),
...
class FixtureTree:
    def __init__(self, root: Path, *, preexisting_mods: bool = False, flow_kind: str = "generic") -> None:
        ...
        self.overlay_root = self.mods_parent / "Sts2AgentBridgeGenericEventV10"
        self.operator_parent = self.application_support / "Sts2AgentBridge"
        self.config_root = self.operator_parent / "generic_event_v10"
...
def _assert_recorded_retention(
    ...
    for operator_root in present_operators:
        config_root = operator_root / "generic_event_v10"
    ...
    for overlay_unit in present_overlays:
        overlay_root = (
            overlay_unit / "Sts2AgentBridgeGenericEventV10"
...
def _verify_production_bindings() -> None:
    if manager.CAMPAIGN_ID != "GENERIC-EVENT-V10-SMOKE-V1":
    ...
    expected_configs = {"generic": b'{"schema_version":"generic_event_v10_transport_config_v1","enabled":true,"flow_kind":"generic","bind_address":"127.0.0.1","port":43117,"token_file":"credential.hex"}'}
    if manager.CONFIGURATIONS != expected_configs or manager.CONFIG_HASHES != {"generic": "4ad1b741be4e89ebce759ec80fc3796cacb00eb06177ca71d0106deddb6e532c"}:
    ...
    expected_overlay = (
        ...
        / "MacOS"
        / "mods"
        / "Sts2AgentBridgeGenericEventV10"
    )
    expected_config = (
        profile
        / "Library"
    ...
    if (
        ...
        / "Library"
        / "Application Support"
        / "Sts2AgentBridgeCampaign-generic-event-v10-smoke-v1"
        or manager.ARTIFACT_ROOT
        != Path("/private/tmp/sts-generic-event-v10-release")
