...
CAMPAIGN_ID = "GENERIC-EVENT-V10-SMOKE-V1"
STATE_ROOT_NAME = "Sts2AgentBridgeCampaign-generic-event-v10-smoke-v1"
...
CONFIGURATIONS = {
    "generic": b'{"schema_version":"generic_event_v10_transport_config_v1","enabled":true,"flow_kind":"generic","bind_address":"127.0.0.1","port":43117,"token_file":"credential.hex"}',
}
...
OVERLAY_DLL_NAME = "Sts2AgentBridgeGenericEventV10.dll"
OVERLAY_MANIFEST_NAME = "Sts2AgentBridgeGenericEventV10.json"
...
def _production_layout(user_profile: Path) -> CampaignLayout:
    ...
    overlay_root = mods_parent / "Sts2AgentBridgeGenericEventV10"
    operator_parent = application_support / "Sts2AgentBridge"
    config_root = operator_parent / "generic_event_v10"
...
def _validate_operator_unit(root: Path, uid: int, flow_kind: str) -> dict[Path, os.stat_result]:
    records: dict[Path, os.stat_result] = {}
    records[root] = _require_directory(root, uid, 0o700)
    _list_exact(root, {"generic_event_v10"})
    records.update(_validate_config_tree(root / "generic_event_v10", uid, flow_kind))
    return records
...
def _validate_operator_unit_metadata(
    ...
    records[root] = _require_directory(root, uid, 0o700)
    _list_exact(root, {"generic_event_v10"})
    records.update(_validate_config_tree_metadata(root / "generic_event_v10", uid, flow_kind))
    return records
...
def _validate_overlay_unit(
    ...
    _list_exact(unit_root, {"Sts2AgentBridgeGenericEventV10"})
    records.update(
        _validate_overlay_tree(unit_root / "Sts2AgentBridgeGenericEventV10", uid, policy)
    )
    return records
...
def _require_state_records(
    ...
    overlay_root = (
        overlay_unit_root / "Sts2AgentBridgeGenericEventV10"
        if state.mods_parent_created
...
def _install(
    ...
    try:
        ...
        with _private_umask():
            ...
            staged_config = layout.staged_operator / "generic_event_v10"
            ...
            if mods_parent_created:
                staged_overlay_root = layout.staged_overlay / "Sts2AgentBridgeGenericEventV10"
...
def _validate_purge_remainder(
    ...
    if progress < 4:
        ...
        expected_operator = {"generic_event_v10"} if progress < 3 else set()
    ...
    if progress < overlay_removed_at:
        ...
        if state.mods_parent_created:
            expected_unit = {"Sts2AgentBridgeGenericEventV10"} if progress < 7 else set()
