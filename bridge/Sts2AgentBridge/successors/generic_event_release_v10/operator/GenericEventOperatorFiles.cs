...
internal static class GenericEventOperatorFiles
{
    ...
    private static ItemOperatorConfiguration? OpenBelowHome(DarwinDescriptorLease descriptors)
    {
        ...
        try
        {
            if (!descriptors.TryOpenDirectory("Library", DarwinDescriptorPolicy.OwnedDirectory) ||
                ...
                !descriptors.TryOpenDirectory("generic_event_v10", DarwinDescriptorPolicy.PrivateDirectory) ||
    ...
    private static bool IsExactEnabled(byte[] bytes) => FixedEquals(bytes, GenericEnabled);

    private static ReadOnlySpan<byte> GenericEnabled =>
        "{\"schema_version\":\"generic_event_v10_transport_config_v1\",\"enabled\":true,\"flow_kind\":\"generic\",\"bind_address\":\"127.0.0.1\",\"port\":43117,\"token_file\":\"credential.hex\"}"u8;
