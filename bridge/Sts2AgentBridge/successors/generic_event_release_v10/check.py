...
PREVIOUS_CHECK='3c89b7b9d2a032ada30ff4358f5cd0652f039baad6ce37d63d05d6d4d6fa5750'
PREVIOUS_MANIFEST='53a6cc7186f13f12cb3a9f4d0437c85be99ed600b0162deca31f2deee8aebb2d'
PREVIOUS_INVENTORY='733316f29243300256be3fe1bc60330900a5f4d3048078d11480eeb1aeffcaa0'
HARMONY_BYTES=2328064
HARMONY_SHA='ef1898322c9f5c86dc1b0758b272a9c440823b4a41ca9a0b82a3aa6b3d206387'
PRODUCTION='production/Sts2AgentBridgeGenericEventV10.csproj'
...
def sources():
    path=ROOT.parent/'generic_event_release_v9/check.py';require(not path.is_symlink(),'predecessor_link')
    ...
    inventories=dict(inventories,generic_event_release_v9=prior);require(len(inventories)==31,'predecessor_count')
    ...
    contract=ROOT.parents[3]/'docs/PHASE_1_GENERIC_EVENT_RELEASE_V10_CONTRACT.md';helper.reject_links(contract);contract_hash=sha(helper.read_regular(contract))
    identity=ROOT/'source_identity.json'
    if identity.exists():
        require(json.loads(identity.read_bytes())==dict(schema_version=1,component='generic_event_release_v10',contract_sha256=contract_hash,inventory_sha256=inventory,files={name:sha(data) for name,data in own.items()}),'source_identity')
    inputs=dict(inputs);inputs.update({PREFIX+'generic_event_release_v10/'+name:data for name,data in own.items()})
...
def validate_provenance_closure(inputs,own,derivation):
    own_prefix=PREFIX+'generic_event_release_v10/'
...
def provenance_mutations(inputs,own):
    ...
    for key,code in (('derived','derived_complete_closure'),('reused','reused_complete_closure')):
        for duplicate in (False,True):
            ...
            count+=1
    changed=copy.deepcopy(original);changed['created']=[dict(target=original['derived'][0]['target'],sha256=original['derived'][0]['target_sha256'])]
    try:validate_provenance_closure(inputs,own,changed)
    except ValueError as error:require(str(error)=='created_complete_closure','created_mutation_rejection')
    else:raise ValueError('created_mutation_accepted')
    count+=1
...
def main():
    ...
    require(set(projects)=={PRODUCTION,'runtime/Sts2AgentBridge.GenericEventV10.Transport.csproj',
        'runtime_tests/GenericEventReleaseV10.RuntimeTests.csproj','integration/GenericEventReleaseV10.SocketFixture.csproj',
        'operator_tests/Sts2AgentBridge.GenericEventV10.Operator.Tests.csproj',
        'lifecycle_tests/Sts2AgentBridge.GenericEventV10.Bootstrap.Tests.csproj',
        'native_tests/Sts2AgentBridge.GenericEventV10.Native.Tests.csproj',
        'verifier/Sts2AgentBridge.GenericEventV10.Release.Verifier.csproj',
        'verifier_tests/Sts2AgentBridge.GenericEventV10.Release.Verifier.Tests.csproj','gameplay_tests/GenericEventReleaseV10.Offscreen.Tests.csproj'},'project_inventory')
    production_sources=closure(inputs,PREFIX+'generic_event_release_v10/'+PRODUCTION,True)
    ...
    new_adapter=PREFIX+'generic_event_release_v10/gameplay/GenericEventV7TransformAdapter.cs'
    require(old_adapter not in production_sources and new_adapter in production_sources and (gameplay-{old_adapter}-{PREFIX+'generic_event_release_v5/runtime/'+n+'.cs' for n in ('GenericEventDiagnosticCode','GenericEventDiagnosticCodec')}).issubset(production_sources),'exact_g7_adapter_substitution')
    require({p for p in production_sources if p.startswith(PREFIX+'generic_event_release_v10/gameplay/')}=={new_adapter},'only_offscreen_gameplay_replacement')
    ...
    old_diagnostic={PREFIX+'generic_event_release_v5/runtime/'+name+'.cs' for name in ('GenericEventDiagnosticCode','GenericEventDiagnosticCodec')}
    new_diagnostic={PREFIX+'generic_event_release_v10/runtime/'+name+'.cs' for name in ('GenericEventDiagnosticCode','GenericEventDiagnosticCodec')}
    require(not old_diagnostic.intersection(production_sources) and new_diagnostic.issubset(production_sources),'diagnostic_substitution_closure')
    for p in production_sources:
        if p.startswith(PREFIX+'generic_event_release_v10/native/'):
    ...
    native_fixture_sources=closure(inputs,PREFIX+'generic_event_release_v10/native_tests/Sts2AgentBridge.GenericEventV10.Native.Tests.csproj',False)
    ...
    compiled=set()
    for p in projects:compiled.update(closure(inputs,PREFIX+'generic_event_release_v10/'+p,p==PRODUCTION))
    require({PREFIX+'generic_event_release_v10/'+p for p in own if p.endswith('.cs')}.issubset(compiled),'uncompiled_source')
    ...
    def snapshot(label):
        ...
        doc=source/'docs/PHASE_1_GENERIC_EVENT_RELEASE_V10_CONTRACT.md';doc.parent.mkdir(exist_ok=True);doc.write_bytes((ROOT.parents[3]/'docs/PHASE_1_GENERIC_EVENT_RELEASE_V10_CONTRACT.md').read_bytes())
        if (ROOT/'source_identity.json').exists():(source/PREFIX/'generic_event_release_v10/source_identity.json').write_bytes((ROOT/'source_identity.json').read_bytes())
        ...
        component=source/PREFIX/'generic_event_release_v10';nuget=dest/'NuGet.Config';nuget.write_text('<configuration><packageSources><clear /></packageSources></configuration>\n')
    ...
    results['frozen_g7_regression']=dict(status='retained_evidence_not_rerun',source_inventory_sha256=inventories['generic_event_v7'],evidence='PHASE_1_GENERIC_EVENT_RELEASE_V9_ACCEPTANCE.md')
    for p in projects:
        outputs[p]=build(p)
    ...
    verifier=outputs['verifier/Sts2AgentBridge.GenericEventV10.Release.Verifier.csproj']
    ...
    vp='verifier_tests/Sts2AgentBridge.GenericEventV10.Release.Verifier.Tests.csproj'
