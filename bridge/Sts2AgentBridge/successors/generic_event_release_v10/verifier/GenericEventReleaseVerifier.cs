...
{
    private const string ExpectedAssemblyName = "Sts2AgentBridgeGenericEventV10";
    private const string ExpectedAssemblyVersion = "1.0.0.0";
    private static readonly string[] ExpectedRoutes =
    {
        "/probe/generic-event-v7/public/action",
        "/probe/generic-event-v7/public/decision",
    };
    private static readonly string[] ExpectedRequestLines =
    {
        "GET /probe/generic-event-v7/public/decision HTTP/1.1",
        "POST /probe/generic-event-v7/public/action HTTP/1.1",
    };
    ...
    private const string ExpectedInitializer =
        "type:Sts2AgentBridge.Successors.GenericEventReleaseV10.GenericEventModEntry|" +
    ...
    private const string ExpectedEntryType =
        "type_def|Sts2AgentBridge.Successors.GenericEventReleaseV10.GenericEventModEntry|1048961|System.Object|0|0";
    private const string ExpectedEntryMethodSuffix =
        "|Sts2AgentBridge.Successors.GenericEventReleaseV10.GenericEventModEntry.Initialize()|System.Void|0|0|0|150|0";
    ...
    internal static void VerifySemanticRules(GenericEventReleaseProjection projection)
    {
        ...
        GenericEventHitboxSurface.Verify(projection.Inventory);
        GenericEventOffscreenSurface.Verify(projection.Inventory);
        ...
        if (!projection.InitializerAttributes.SequenceEqual(new[] { ExpectedInitializer }, StringComparer.Ordinal) ||
            ...
            projection.Inventory.Count(value => value.StartsWith(
                ...
                "|Sts2AgentBridge.Successors.GenericEventReleaseV10.GenericEventModEntry.",
    ...
    private static void VerifyMethodTokenLinks(string[] inventory, int checkedBodies)
    {
        ...
        foreach (string row in inventory.Where(value => value.StartsWith("method_def|", StringComparison.Ordinal)))
        {
            ...
            if (fields.Length != 9 || !IsMethodToken(fields[1]) || !methods.Add(fields[1]) ||
                !IsMetadataCell(fields[2]) || !IsMetadataCell(fields[3]))
        ...
        {
            string[] fields = row.Split('|');
            if (fields.Length != 5 || !methods.Contains(fields[1]) || !IsMetadataCell(fields[3]))
                throw new VerificationException("metadata_token_linkage");
        }
    }

    private static bool IsMetadataCell(string value)
    {
        for (int index = 0; index < value.Length; index++)
        {
            if (value[index] is '|' or '\r' or '\n') return false;
            if (value[index] != '%') continue;
            if (index + 2 >= value.Length || value.Substring(index + 1, 2) is not ("25" or "7C" or "0D" or "0A")) return false;
            index += 2;
        }
        return true;
    }
    ...
    private static void VerifyCriticalFlow(string[] references, string[] inventory)
    {
        ...
        const string guardedFactory = "Sts2AgentBridge.Successors.GenericEventReleaseV10.ProductionGenericEventRuntimeFactory.CreateService(Sts2AgentBridge.Successors.GenericEventReleaseV10.GenericEventReleaseSelection,System.String,Sts2AgentBridge.Successors.GenericEventReleaseV10.ProductionGenericEventRuntimeFactory+Runtime)";
        ...
        if (guards.Length != 1 || entries.Length != 1 || Owner(guards[0]) != guardedFactory ||
            ...
            throw new VerificationException("critical_flow_forbidden");
        string factory = "critical|Sts2AgentBridge.Successors.GenericEventReleaseV10.ProductionGenericEventRuntimeFactory.CreateVerifiedService(";
        string runtime = "critical|Sts2AgentBridge.Successors.GenericEventReleaseV10.GenericEventTransportRuntime+";
        ...
        string[] dispatch = references.Where(value => value.StartsWith(runtime, StringComparison.Ordinal) &&
            Target(value).Contains(".GenericEventV7WireService.Handle(", StringComparison.Ordinal)).ToArray();
        ...
        if (construction.Length != 3 || dispatch.Length != 1 || submissions.Length != 1 || callbacks.Length != 1 ||
            construction.Count(value => Target(value).Contains(".PinnedGenericEventV7NativeAdapter..ctor(", StringComparison.Ordinal)) != 1 ||
            construction.Count(value => Target(value).Contains(".GenericEventV7Session..ctor(", StringComparison.Ordinal)) != 1 ||
            construction.Count(value => Target(value).Contains(".GenericEventV7WireService..ctor(", StringComparison.Ordinal)) != 1)
        {
            throw new VerificationException("critical_flow_forbidden");
        }
        int pinned = Offset(construction.Single(value => Target(value).Contains(
            ".PinnedGenericEventV7NativeAdapter..ctor(", StringComparison.Ordinal)));
        int session = Offset(construction.Single(value => Target(value).Contains(
            ".GenericEventV7Session..ctor(", StringComparison.Ordinal)));
        int service = Offset(construction.Single(value => Target(value).Contains(
            ".GenericEventV7WireService..ctor(", StringComparison.Ordinal)));
