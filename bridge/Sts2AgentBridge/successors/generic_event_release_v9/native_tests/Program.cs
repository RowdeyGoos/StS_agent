...
internal static class ReleaseNativeTests
{
    ...
    private static byte[] Configuration() => Encoding.UTF8.GetBytes("{\"schema_version\":\"generic_event_v9_transport_config_v1\",\"enabled\":true,\"flow_kind\":\"generic\",\"bind_address\":\"127.0.0.1\",\"port\":43117,\"token_file\":\"credential.hex\"}");
    ...
    internal static int Main()
    {
        try
        {
            ...
            Console.WriteLine("{\"status\":\"passed\",\"suite\":\"generic_event_v9_native_release\",\"check_count\":" + _checks + "}"); return 0;
        }
    ...
    private static void FactoryDefaultDeny()
    {
        ...
        config=Encoding.UTF8.GetBytes(Encoding.UTF8.GetString(Configuration()).Replace("generic_event_v9_transport_config_v1","generic_event_v7_transport_config_v1",StringComparison.Ordinal));credential=Credential();
        Check(factory.Create(config,credential) is null);Check(config.All(x=>x==0) && credential.All(x=>x==0));
