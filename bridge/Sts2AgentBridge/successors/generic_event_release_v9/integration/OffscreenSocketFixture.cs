...
internal static class OffscreenSocketFixture
{
    ...
    {
        string mode=scenario switch {
            "native_offscreen_missing_clip"=>"missing_clip", "native_offscreen_partial"=>"partial",
            "native_offscreen_scroll_height" or "native_offscreen_transform" or
            "native_offscreen_success" or "native_offscreen_rectangle" or "native_offscreen_clip" or
            "native_offscreen_reassign" or "native_offscreen_deferred_reassign" or "native_offscreen_lost"=>"probe",
            _=>throw new ArgumentException("Unknown offscreen fixture.")
        };
        ...
        var runtime=GenericEventTransportRuntime.Create((byte[])SocketFixture.Configuration.Clone(),
            ()=>Encoding.ASCII.GetBytes(new string('c',64)),(_,nonce)=>{
                fixture=new global::Program.TransformFixture("OFFSCREEN_HELD_OUT",1,20,substitute:true,nonce:nonce);
                ...
                NDeckTransformSelectScreen.Factory=(cards,factory,prefs)=>{
                    var screen=original(cards,factory,prefs);
                    geometry=OffscreenGeometry.Configure(screen,mode);
                    if(scenario=="native_offscreen_scroll_height")geometry.Grid.GetNodeOrNull<Godot.Control>("%ScrollContainer")!.Size=new Godot.Vector2(1000,1);
                    if(scenario=="native_offscreen_transform")geometry.Mutate("rotation");
                    if(scenario=="native_offscreen_deferred_reassign")geometry.Holders[2].FixtureGuiInputDispatch=action=>deferred=action;
                    return screen;
                };
        ...
        {
            runtime.DrainFrame();
            if(!changed&&runtime.ReadSubmissionCount>=2&&scenario is "native_offscreen_rectangle" or "native_offscreen_clip" or "native_offscreen_reassign")
            {
                changed=true;
                geometry!.Mutate(scenario=="native_offscreen_rectangle"?"rectangle":scenario=="native_offscreen_clip"?"clip":"holder");
            }
        ...
        bool valid=geometry is not null&&geometry.DispatchCount==(dispatched?1:0)&&geometry.AllDispatchesBelow&&
            geometry.DispatchedSlots.SequenceEqual(dispatched?new[]{2}:Array.Empty<int>())&&
            fixture!.SelectCalls==(dispatched?1:0)&&fixture.ConfirmCalls==(positive?1:0)&&
            (!positive||(fixture.CompletionValid&&fixture.Originals.Count==1&&ReferenceEquals(fixture.Originals[0],fixture.Cards[2])&&fixture.Map.IsOpen));
