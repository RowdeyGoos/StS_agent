...
class GenericEventTransportTests(unittest.TestCase):
    ...
    def test_all_parent_child_actions_and_lineage(self):
        for child, actions in ((False, ["choose:" + str(i) for i in range(8)]),
                              (True, ["select:" + str(i) for i in range(64)] + ["preview", "confirm"] + ["collect:" + str(i) for i in range(256)])):
    ...
    def test_collect_bounds_and_maximum_request_witness(self):
        for action in ('collect:256', 'collect:00', 'collect:01', 'collect:-1', 'collect:+1', 'collect:', 'collect:1x'):
            body = action_body(action)
            with self.assertRaises(ValueError):
                transport._build_request('POST', transport.ACTION_ROUTE, body, bytearray(TOKEN))
            self.assertFalse(any(body))
        body = action_body('collect:255')
        body[:] = body.replace(b'"ordinal":1', b'"ordinal":4').replace(b'"choose:0"', b'"choose:7"')
        self.assertEqual(len(body), 248)
        request = transport._build_request('POST', transport.ACTION_ROUTE, body, bytearray(TOKEN))
        self.assertEqual(len(request), 481)
        self.assertFalse(any(body))
        transport._zero(request)

    def test_live_policy_requires_two_initial_options_then_first_legal(self):
        def view(kind='parent', count=2, legal=('choose:0', 'choose:0')):
            return SimpleNamespace(kind=kind, payload={'candidates':[{}] * count, 'legal_actions':legal})
        policy = transport._LivePolicy()
        self.assertEqual(policy(view()), 'choose:0')
        self.assertEqual(policy(view('item', 1, ('collect:255',))), 'collect:255')
        self.assertEqual(policy(view('parent', 1, ('choose:0',))), 'choose:0')
        for value in (view(count=0), view(count=1), view(count=3), view(legal=('choose:1',)), view(kind='card')):
            with self.assertRaises(ValueError):
                transport._LivePolicy()(value)
        self.assertEqual(transport._LivePolicy()(view()), 'choose:0')

    def test_public_entry_has_fresh_fixed_policy_and_no_provider_override(self):
        policies = []
        def host(exchange, *, provider, clock, sleep):
            policies.append(provider)
            self.assertEqual(provider(SimpleNamespace(kind='parent', payload={'candidates':[{},{}], 'legal_actions':['choose:0','choose:1']})), 'choose:0')
            return {'status':'resolved'}
        with mock.patch.object(transport, 'run_event', host), mock.patch.object(transport, '_new_socket', side_effect=AssertionError('unexpected socket')):
            for _ in range(2):
                token = bytearray(TOKEN)
                result = transport.run_authenticated_generic_event(token, clock=Clock(), sleep=lambda _:None)
                self.assertEqual(result['status'], 'resolved')
                self.assertFalse(any(token))
        self.assertIsNot(policies[0], policies[1])
        self.assertEqual(transport._failure()['completed_card_children'], 0)
        self.assertEqual(transport._failure()['completed_item_children'], 0)
    ...
    def test_diagnostic_allowlist_preserves_old_codes_and_exact_append(self):
        expected = (
            ...
            "geometry_scroll_missing",
            "geometry_scroll_invalid",
            "geometry_scroll_invisible",
            "geometry_scroll_size",
            "geometry_scroll_position",
            "geometry_grid_size",
            "geometry_card_size",
            "geometry_y_offset",
            "geometry_columns",
            "geometry_scroll_height_mismatch",
            "geometry_scroll_position_mismatch",
            "geometry_clip_search_invalid",
            "geometry_clip_search_cycle",
            "geometry_clip_search_depth",
            "geometry_clip_missing",
            "geometry_clip_rect",
            "geometry_canvas_invalid",
            "geometry_chain_invalid",
            "geometry_chain_non_canvas",
            "geometry_chain_cycle",
            "geometry_chain_depth",
            "geometry_chain_unreached",
            "geometry_top_level",
            "geometry_canvas_mismatch",
            "geometry_transform",
            "geometry_node_rect",
            "geometry_control_rect",
            "geometry_clip_changed",
            "geometry_parent_changed",
            "geometry_transform_changed",
            "geometry_node_clip_changed",
            "geometry_rect_changed",
            "geometry_mask_count",
            "geometry_initially_selected",
            "geometry_candidate_invisible",
            "geometry_none_eligible",
        )
        self.assertEqual(len(expected), 115)
        self.assertEqual(transport._DIAGNOSTICS, {code.encode("ascii"): code for code in expected})
        self.assertTrue(all(1 <= len(code) <= 33 for code in expected))

    def test_geometry_diagnostic_header_boundary(self):
        longest = b"geometry_scroll_position_mismatch"
        self.assertEqual(len(longest), 33)
        for code in (longest + b"x", b"geometry_unknown", longest.upper(), longest + b" "):
            clock = Clock()
            factory = Factory([FakeSocket(http(b"{}", code), clock=clock)])
