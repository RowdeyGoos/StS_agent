...
_MANIFEST_SHA256 = "c2df30bf27f9c381305f73893b77f5813a8b8c48fb257161b479cfda4bd31112"
...
def _write_package(path: Path, dll: bytes, manifest: bytes) -> bytes:
    with zipfile.ZipFile(
        ...
        for name, data in (
            (
                "Sts2AgentBridgeGenericEventV9/Sts2AgentBridgeGenericEventV9.dll",
                dll,
            ),
            (
                "Sts2AgentBridgeGenericEventV9/Sts2AgentBridgeGenericEventV9.json",
                manifest,
            ),
        ):
...
class _Fixture:
    ...
    def install_overlay(self, manifest: bytes | None = None) -> None:
        ...
        (directory / "Sts2AgentBridgeGenericEventV9.dll").write_bytes(self.dll)
        (directory / "Sts2AgentBridgeGenericEventV9.json").write_bytes(
...
def operation() -> dict[str, object]:
    ...
    with tempfile.TemporaryDirectory(
        ...
        mutated[mutated.index(b"Generic Event V9")] ^= 1
