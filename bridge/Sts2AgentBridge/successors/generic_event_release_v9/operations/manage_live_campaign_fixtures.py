...
_POLICY = manager.ArtifactPolicy(
    ...
    archive=manager.ArtifactSpec(
        "Sts2AgentBridgeGenericEventV9-0.1.0.zip",
        len(_ARCHIVE_DATA),
...
class FixtureTree:
    def __init__(self, root: Path, *, preexisting_mods: bool = False, flow_kind: str = "generic") -> None:
        ...
        self.overlay_root = self.mods_parent / "Sts2AgentBridgeGenericEventV9"
        self.operator_parent = self.application_support / "Sts2AgentBridge"
        self.config_root = self.operator_parent / "generic_event_v9"
...
def _assert_recorded_retention(
    ...
    for operator_root in present_operators:
        config_root = operator_root / "generic_event_v9"
    ...
    for overlay_unit in present_overlays:
        overlay_root = (
            overlay_unit / "Sts2AgentBridgeGenericEventV9"
...
def _verify_production_bindings() -> None:
    if manager.CAMPAIGN_ID != "GENERIC-EVENT-V9-SMOKE-V1":
    ...
    expected_configs = {"generic": b'{"schema_version":"generic_event_v9_transport_config_v1","enabled":true,"flow_kind":"generic","bind_address":"127.0.0.1","port":43117,"token_file":"credential.hex"}'}
    if manager.CONFIGURATIONS != expected_configs or manager.CONFIG_HASHES != {"generic": "281398d3fb4db046b983a5e90615aee0c51d10646cd52952eb80e4c8175a07c6"}:
    ...
    if (
        ...
        / "Library"
        / "Application Support"
        / "Sts2AgentBridgeCampaign-generic-event-v9-smoke-v1"
        or manager.ARTIFACT_ROOT
        != Path("/private/tmp/sts-generic-event-v9-release")
