...
internal static class PeMutations
{
    ...
    internal static byte[] ChangeDiagnosticMappingString(byte[] source, string literal = "parent_ready")
    {
        byte[] result = source.ToArray();
        using var pe = Open(result, out MetadataReader reader);
        var owner = reader.TypeDefinitions.Single(h => reader.GetString(reader.GetTypeDefinition(h).Name) == "GenericEventDiagnosticCodec");
        var handle = reader.GetTypeDefinition(owner).GetMethods().Single(h => reader.GetString(reader.GetMethodDefinition(h).Name) == "Encode");
        var method = reader.GetMethodDefinition(handle);
        var instructions = IlDecoder.Decode(pe.GetMethodBody(method.RelativeVirtualAddress).GetILBytes()!);
        var load = instructions.Where(i => i.OpCode == 0x0072 && i.MetadataToken is int token &&
            reader.GetUserString(MetadataTokens.UserStringHandle(token)) == literal)
            .Select(i => i.MetadataToken!.Value).Distinct().ToArray();
        if (load.Length != 1 || literal.Length > 63)
            throw new InvalidOperationException("diagnostic literal not unique");
        int heapOffset = MetadataTokens.GetHeapOffset(MetadataTokens.UserStringHandle(load[0]));
        int offset = MetadataOffset(pe) + reader.GetHeapMetadataOffset(HeapIndex.UserString) + heapOffset;
        if (result[offset] != literal.Length * 2 + 1)
            throw new InvalidOperationException("diagnostic literal length shape");
        result[offset + 1] = (byte)'q';
        return result;
    }
    ...
    internal static byte[] ChangeFactorySessionAdapterLocal(byte[] source)
    {
        ...
        int pinnedCall = FindMethodInstruction(reader, instructions,
            ".PinnedGenericEventV7NativeAdapter..ctor(");
        int sessionCall = FindMethodInstruction(reader, instructions,
            ".GenericEventV7Session..ctor(");
