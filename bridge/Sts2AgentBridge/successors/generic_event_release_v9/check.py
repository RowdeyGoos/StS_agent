...
PREVIOUS_CHECK='222e6f7222850801511e10417a1da8fd50d66c32fda00252357483db7ce02598'
PREVIOUS_MANIFEST='3854ed2a09d664a29bed8b079e831cb2db96d2cc5e7aee79e400d201b310a52b'
PREVIOUS_INVENTORY='860b8b1062a4668e4ae5df70399a88e2c770606794148a933149b0316700e377'
HARMONY_BYTES=2328064
HARMONY_SHA='ef1898322c9f5c86dc1b0758b272a9c440823b4a41ca9a0b82a3aa6b3d206387'
PRODUCTION='production/Sts2AgentBridgeGenericEventV9.csproj'
...
def sources():
    path=ROOT.parent/'generic_event_release_v8/check.py';require(not path.is_symlink(),'predecessor_link')
    ...
    inventories=dict(inventories,generic_event_release_v8=prior);require(len(inventories)==30,'predecessor_count')
    ...
    contract=ROOT.parents[3]/'docs/PHASE_1_GENERIC_EVENT_RELEASE_V9_CONTRACT.md';helper.reject_links(contract);contract_hash=sha(helper.read_regular(contract))
    identity=ROOT/'source_identity.json'
    if identity.exists():
        require(json.loads(identity.read_bytes())==dict(schema_version=1,component='generic_event_release_v9',contract_sha256=contract_hash,inventory_sha256=inventory,files={name:sha(data) for name,data in own.items()}),'source_identity')
    inputs=dict(inputs);inputs.update({PREFIX+'generic_event_release_v9/'+name:data for name,data in own.items()})
...
def validate_provenance_closure(inputs,own,derivation):
    own_prefix=PREFIX+'generic_event_release_v9/'
...
def main():
    ...
    require(set(projects)=={PRODUCTION,'runtime/Sts2AgentBridge.GenericEventV9.Transport.csproj',
        'runtime_tests/GenericEventReleaseV9.RuntimeTests.csproj','integration/GenericEventReleaseV9.SocketFixture.csproj',
        'operator_tests/Sts2AgentBridge.GenericEventV9.Operator.Tests.csproj',
        'lifecycle_tests/Sts2AgentBridge.GenericEventV9.Bootstrap.Tests.csproj',
        'native_tests/Sts2AgentBridge.GenericEventV9.Native.Tests.csproj',
        'verifier/Sts2AgentBridge.GenericEventV9.Release.Verifier.csproj',
        'verifier_tests/Sts2AgentBridge.GenericEventV9.Release.Verifier.Tests.csproj','gameplay_tests/GenericEventReleaseV9.Offscreen.Tests.csproj'},'project_inventory')
    production_sources=closure(inputs,PREFIX+'generic_event_release_v9/'+PRODUCTION,True)
    ...
    new_adapter=PREFIX+'generic_event_release_v9/gameplay/GenericEventV7TransformAdapter.cs'
    ...
    require({p for p in production_sources if p.startswith(PREFIX+'generic_event_release_v9/gameplay/')}=={new_adapter},'only_offscreen_gameplay_replacement')
    ...
    new_diagnostic={PREFIX+'generic_event_release_v9/runtime/'+name+'.cs' for name in ('GenericEventDiagnosticCode','GenericEventDiagnosticCodec')}
    ...
    for p in production_sources:
        if p.startswith(PREFIX+'generic_event_release_v9/native/'):
    ...
    native_fixture_sources=closure(inputs,PREFIX+'generic_event_release_v9/native_tests/Sts2AgentBridge.GenericEventV9.Native.Tests.csproj',False)
    ...
    compiled=set()
    for p in projects:compiled.update(closure(inputs,PREFIX+'generic_event_release_v9/'+p,p==PRODUCTION))
    require({PREFIX+'generic_event_release_v9/'+p for p in own if p.endswith('.cs')}.issubset(compiled),'uncompiled_source')
    ...
    def snapshot(label):
        ...
        doc=source/'docs/PHASE_1_GENERIC_EVENT_RELEASE_V9_CONTRACT.md';doc.parent.mkdir(exist_ok=True);doc.write_bytes((ROOT.parents[3]/'docs/PHASE_1_GENERIC_EVENT_RELEASE_V9_CONTRACT.md').read_bytes())
        if (ROOT/'source_identity.json').exists():(source/PREFIX/'generic_event_release_v9/source_identity.json').write_bytes((ROOT/'source_identity.json').read_bytes())
        ...
        component=source/PREFIX/'generic_event_release_v9';nuget=dest/'NuGet.Config';nuget.write_text('<configuration><packageSources><clear /></packageSources></configuration>\n')
    ...
    verifier=outputs['verifier/Sts2AgentBridge.GenericEventV9.Release.Verifier.csproj']
    ...
    vp='verifier_tests/Sts2AgentBridge.GenericEventV9.Release.Verifier.Tests.csproj'
