...
_MANIFEST_SHA256 = "e814b2354922698f6346ec9c9d4009445459714a2cbd3ea7864326fa9d8c4bd6"
...
def _write_package(path: Path, dll: bytes, manifest: bytes) -> bytes:
    with zipfile.ZipFile(
        ...
        for name, data in (
            (
                "Sts2AgentBridgeGenericEventV7/Sts2AgentBridgeGenericEventV7.dll",
                dll,
            ),
            (
                "Sts2AgentBridgeGenericEventV7/Sts2AgentBridgeGenericEventV7.json",
                manifest,
            ),
        ):
...
class _Fixture:
    ...
    def install_overlay(self, manifest: bytes | None = None) -> None:
        ...
        (directory / "Sts2AgentBridgeGenericEventV7.dll").write_bytes(self.dll)
        (directory / "Sts2AgentBridgeGenericEventV7.json").write_bytes(
...
def operation() -> dict[str, object]:
    ...
    with tempfile.TemporaryDirectory(
        ...
        mutated[mutated.index(b"Generic Event V7")] ^= 1
