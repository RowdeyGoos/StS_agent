...
PREVIOUS_CHECK='5ebe65a68f3444fed2c68035ad7776d952666c538dc4e14fdeb272d61655f5c8'
PREVIOUS_MANIFEST='83d2d6f21223de32370205198838d80282c48718ed7cb23b656449de845876d4'
PREVIOUS_INVENTORY='1f263d7916cb9bb2cd973cce2e72eb4e21ba04e92ff3554ae7e1e0ca490d66df'
HARMONY_BYTES=2328064
HARMONY_SHA='ef1898322c9f5c86dc1b0758b272a9c440823b4a41ca9a0b82a3aa6b3d206387'
PRODUCTION='production/Sts2AgentBridgeGenericEventV7.csproj'
...
def sources():
    path=ROOT.parent/'generic_event_v7/check.py';require(not path.is_symlink(),'predecessor_link')
    ...
    inventories=dict(inventories,generic_event_v7=prior);require(len(inventories)==28,'predecessor_count')
    ...
    contract=ROOT.parents[3]/'docs/PHASE_1_GENERIC_EVENT_RELEASE_V7_CONTRACT.md';helper.reject_links(contract);contract_hash=sha(helper.read_regular(contract))
    identity=ROOT/'source_identity.json'
    if identity.exists():
        require(json.loads(identity.read_bytes())==dict(schema_version=1,component='generic_event_release_v7',contract_sha256=contract_hash,inventory_sha256=inventory,files={name:sha(data) for name,data in own.items()}),'source_identity')
    inputs=dict(inputs);inputs.update({PREFIX+'generic_event_release_v7/'+name:data for name,data in own.items()})
...
def validate_provenance_closure(inputs,own,derivation):
    own_prefix=PREFIX+'generic_event_release_v7/'
    expected_derived={own_prefix+name for name in own if name not in ('README.md','derivation.json')}
...
def main():
    ...
    require(set(projects)=={PRODUCTION,'runtime/Sts2AgentBridge.GenericEventV7.Transport.csproj',
        'runtime_tests/GenericEventReleaseV7.RuntimeTests.csproj','integration/GenericEventReleaseV7.SocketFixture.csproj',
        'operator_tests/Sts2AgentBridge.GenericEventV7.Operator.Tests.csproj',
        'lifecycle_tests/Sts2AgentBridge.GenericEventV7.Bootstrap.Tests.csproj',
        'native_tests/Sts2AgentBridge.GenericEventV7.Native.Tests.csproj',
        'verifier/Sts2AgentBridge.GenericEventV7.Release.Verifier.csproj',
        'verifier_tests/Sts2AgentBridge.GenericEventV7.Release.Verifier.Tests.csproj'},'project_inventory')
    production_sources=closure(inputs,PREFIX+'generic_event_release_v7/'+PRODUCTION,True)
    ...
    require(gameplay.issubset(production_sources),'frozen_g7_gameplay_closure')
    ...
    for p in production_sources:
        if p.startswith(PREFIX+'generic_event_release_v7/native/'):
    ...
    native_fixture_sources=closure(inputs,PREFIX+'generic_event_release_v7/native_tests/Sts2AgentBridge.GenericEventV7.Native.Tests.csproj',False)
    ...
    compiled=set()
    for p in projects:compiled.update(closure(inputs,PREFIX+'generic_event_release_v7/'+p,p==PRODUCTION))
    require({PREFIX+'generic_event_release_v7/'+p for p in own if p.endswith('.cs')}.issubset(compiled),'uncompiled_source')
    ...
    def snapshot(label):
        ...
        doc=source/'docs/PHASE_1_GENERIC_EVENT_RELEASE_V7_CONTRACT.md';doc.parent.mkdir(exist_ok=True);doc.write_bytes((ROOT.parents[3]/'docs/PHASE_1_GENERIC_EVENT_RELEASE_V7_CONTRACT.md').read_bytes())
        if (ROOT/'source_identity.json').exists():(source/PREFIX/'generic_event_release_v7/source_identity.json').write_bytes((ROOT/'source_identity.json').read_bytes())
        ...
        component=source/PREFIX/'generic_event_release_v7';nuget=dest/'NuGet.Config';nuget.write_text('<configuration><packageSources><clear /></packageSources></configuration>\n')
    ...
    require(g7['status']=='passed' and g7['source_inventory_sha256']==PREVIOUS_INVENTORY,'frozen_g7_regression')
    ...
    verifier=outputs['verifier/Sts2AgentBridge.GenericEventV7.Release.Verifier.csproj']
    ...
    vp='verifier_tests/Sts2AgentBridge.GenericEventV7.Release.Verifier.Tests.csproj'
