...
internal static class Program
{
    ...
    internal static int Main(string[] arguments)
    {
        ...
        try
        {
            ...
            Console.WriteLine("{\"schema_version\":1,\"status\":\"passed\",\"suite\":\"shop_diagnostic_v1_bootstrap\",\"check_count\":" + _checks + "}");
            return 0;
        }
        catch
        {
            Console.Error.WriteLine("shop_diagnostic_v1_bootstrap_tests_failed");
            return 1;
        }
    }
    ...
    private sealed class Fixture
    {
        ...
        internal bool BuildResult = true; internal RoomFlowBootstrapLifecycle Lifecycle { get; }
        internal Fixture(FakeLease? lease, List<string>? log = null, FakeRuntime? runtime = null)
        {
            ...
            Lifecycle = new RoomFlowBootstrapLifecycle(Open, Build, Frames, Factory, Clock, Timers);
        }
        private IRoomFlowBootstrapOperatorLease? Open() { OpenCalls++; _lease?.Log?.Add("open"); return _lease; }
        private bool Build() { BuildCalls++; _lease?.Log?.Add("build"); return BuildResult; }
    }

    private sealed class FakeLease : IRoomFlowBootstrapOperatorLease
    {
    ...
    private sealed class FakeClock : IRoomFlowBootstrapClock { internal long Now; public long Timestamp => Now; public long Frequency => 1000; }
    private sealed class FakeTimers : IRoomFlowBootstrapTimerFactory
    {
        internal FakeTimer? Last; public IRoomFlowBootstrapTimer Arm(TimeSpan due, Action callback) { Last=new FakeTimer(callback); return Last; }
    }
    private sealed class FakeTimer : IRoomFlowBootstrapTimer
    {
    ...
    private sealed class FakeFrames : IRoomFlowBootstrapFrameConnector
    {
        ...
        public IRoomFlowBootstrapFrameAttachment? Attach(Action onFrame) { AttachCalls++;_log?.Add("attach");_frame=onFrame;return Attachment; }
        internal void Fire() => _frame!();
    }
    private sealed class FakeAttachment : IRoomFlowBootstrapFrameAttachment
    {
    ...
    private sealed class FakeFactory : IRoomFlowBootstrapRuntimeFactory
    {
    ...
    private sealed class FakeRuntime : IRoomFlowBootstrapRuntime
    {
