#!/usr/bin/env python3
"""Offline passive shop diagnostic gate; no installation, live endpoint or target execution."""
...
CONTRACT="4919241452060ba9cb452683f29dc518c9455860b5bfa5eb0ffe0460ff01c68a"
...
PROJECT_HASHES = {'core/Sts2AgentBridge.ShopDiagnosticV1.Core.csproj': 'd02423544b60db0db089e85c3ed3f8b5b9ef8f036cbd68594272aa4ea695a6c1', 'lifecycle_tests/Sts2AgentBridge.ShopDiagnosticV1.Bootstrap.Tests.csproj': '6d3db698080f66daca494b01a93ca6a4e852dd0ba9ba43ed3a82db24157cbf12', 'operator_tests/Sts2AgentBridge.ShopDiagnosticV1.Operator.Tests.csproj': 'd4c914262458d2bd6aa966dc6bac4600464a1055ad61ba44fdce399e0f67674f', 'production/Sts2AgentBridgeShopDiagnosticV1.csproj': 'e384e9929024d07a6853969cc10ef13bba78a7e95b7e07b07eed56836890cfe6', 'reader/Sts2AgentBridge.ShopDiagnosticV1.Reader.csproj': '95557d76d21ade83d0a00a5b86ad9eac26274d27e95b9f7aeac506033a37d92a', 'reader_tests/Sts2AgentBridge.ShopDiagnosticV1.Reader.Tests.csproj': 'b121135a48a6aa459d5c25a794fe0487bd34ccd8c9e9e544476366e789e45796', 'runtime/Sts2AgentBridge.ShopDiagnosticV1.Transport.csproj': '90214fe092d1bb7e00ac68b2fe12a26c638b44524c9c77462f03a32a0a5b06de', 'runtime_tests/Sts2AgentBridge.ShopDiagnosticV1.Transport.Tests.csproj': '9c33caa5867633db4388af8f42ea087807f712f21c7d7edf2bf38e4de4e230f0', 'verifier/Sts2AgentBridge.ShopDiagnosticV1.Release.Verifier.csproj': '22ff1ce6b931fa4165c934f938609847cf96699f5a2cde3dd8c7497f588497bb', 'verifier_tests/Sts2AgentBridge.ShopDiagnosticV1.Release.Verifier.Tests.csproj': 'b1d93630dd9a7a04e07b9e6570c53debb294f7a99d6484be34a60529230ad871'}
PROJECTS=set(PROJECT_HASHES)
PURE_DEPENDENCIES={"Sts2AgentBridge.ShopDiagnosticV1.Core", "Sts2AgentBridge.RoomFlowsV1.Shop.Core", "Sts2AgentBridge.RoomFlowsV1.Common", "Sts2AgentBridge.ItemV1.Core"}
...
def verify_projects(components):
    own=components["shop_diagnostic_v1"]
    ...
    for name,expected in PROJECT_HASHES.items():
        ...
        for n in tree.iter():
            ...
            full=os.path.normpath("shop_diagnostic_v1/"+str(Path(name).parent/value))
    ...
    policy=json.loads(own["policy/shop_diagnostic_policy.json"])
    if sorted(production)!=[p["path"] for p in policy["source_files"]] or len(production)!=24:
        fail("production_policy_source_closure")
    if {p for p in own if p.endswith(".cs")}!={p.split("/",1)[1] for p in compiled if p.startswith("shop_diagnostic_v1/")}:
        fail("uncompiled_new_source")

def verify_values(own):
    spec=json.loads(own["core/diagnostic_values.json"])
    expected={(status,stage,reason) for status,stages in spec["combinations"].items()
              for stage,reasons in stages.items() for reason in reasons}
    scope={};exec(compile(own["transport/diagnostic_values.py"],"diagnostic_values.py","exec"),scope)
    if scope["ALLOWED_RESULTS"]!=expected:fail("diagnostic_values_parity")
    return {"schema_version":1,"status":"passed","suite":"shop_diagnostic_values","check_count":len(expected)}
...
def passed(output,suite=None,count=None):
    ...
    extras={"manage_live_campaign_fixtures":{"checks"},"check_live_runtime_fixtures":{"checks"},
            "shop_diagnostic_package":{"mutation_cases"},"shop_diagnostic_project_boundary":{"candidate_executed"}}
    if suite=="shop_diagnostic_v1_release_surface":
...
def main():
    ...
    contracts={"item_v1":helper.CORE_CONTRACT,"item_wire_v1":helper.WIRE_CONTRACT,
               ...
               "item_release_v1":release.CONTRACT,"room_flows_v1":ROOM_CONTRACT,"room_release_v1":RELEASE_CONTRACT}
    ...
    components["shop_diagnostic_v1"],inventory=helper.source_snapshot(ROOT,"shop_diagnostic_v1",CONTRACT)
    ...
    def snapshot_at(destination):
        ...
        own=siblings/"shop_diagnostic_v1";nuget=destination/"NuGet.Config"
        ...
        def build(relative,native=False):
            ...
            elif names-{name}-PURE_DEPENDENCIES or any(p.name in ("sts2.dll","GodotSharp.dll","Sts2AgentBridgeShopDiagnosticV1.dll") for p in directory.glob("*.dll")):
    ...
    summaries={"values":verify_values(components["shop_diagnostic_v1"])}
    tests=[
        ("reader_tests/Sts2AgentBridge.ShopDiagnosticV1.Reader.Tests.csproj","reader","shop_diagnostic_v1_reader",13),
        ("operator_tests/Sts2AgentBridge.ShopDiagnosticV1.Operator.Tests.csproj","operator","shop_diagnostic_v1_operator",4),
        ("lifecycle_tests/Sts2AgentBridge.ShopDiagnosticV1.Bootstrap.Tests.csproj","bootstrap","shop_diagnostic_v1_bootstrap",12),
        ("runtime_tests/Sts2AgentBridge.ShopDiagnosticV1.Transport.Tests.csproj","runtime","shop_diagnostic_v1_transport",112),
    ]
    assemblies={}
    for relative,key,suite,count in tests:
        assembly=build("shop_diagnostic_v1/"+relative);assemblies[key]=assembly
        ...
        if key=="operator":
            ...
            fixture_root=Path("/private/tmp")/("shop-diagnostic-operator-"+sha(str(args.scratch).encode("utf-8"))[:24])
            arguments.extend(["--fixture-root",str(fixture_root)])
        summaries[key]=passed(run(arguments,own,env),suite,count)
    summaries["socket_composition"]=passed(run([sys.executable,"-B","-I","-S",str(own/"integration/test_shop_diagnostic_socket_composition.py"),
        "--dotnet",dotnet,"--fixture-assembly",str(assemblies["runtime"])],own,env),"shop_diagnostic_socket_composition",51)
    for relative,key,suite,count in (
        ("operations/verify_source_derivation.py","operations_derivation","shop_diagnostic_operations_derivation",10),
        ("operations/predecessor_conflict_fixtures.py","predecessor_conflicts","shop_diagnostic_predecessor_conflicts",9),
        ("derivation/check_derivation.py","reader_derivation","shop_diagnostic_v1_derivation",22),
        ("checker_tests/test_shop_diagnostic_project_boundary.py","project_boundary","shop_diagnostic_project_boundary",15),
        ("transport_tests/test_diagnostic_transport.py","python_transport","shop_diagnostic_transport",8),
        ("client_tests/client_fixtures.py","client","shop_diagnostic_client",11),
        ("operations/manage_live_campaign_fixtures.py","campaign_manager","manage_live_campaign_fixtures",38),
    ...
    candidate=build("shop_diagnostic_v1/production/Sts2AgentBridgeShopDiagnosticV1.csproj",native=True)
    second_source,_,_,_,second_build=snapshot_at(args.scratch/"second")
    second=second_build("shop_diagnostic_v1/production/Sts2AgentBridgeShopDiagnosticV1.csproj",native=True)
    if candidate.read_bytes()!=second.read_bytes():fail("candidate_reproduction")
    verifier=build("shop_diagnostic_v1/verifier/Sts2AgentBridge.ShopDiagnosticV1.Release.Verifier.csproj")
    mutations=build("shop_diagnostic_v1/verifier_tests/Sts2AgentBridge.ShopDiagnosticV1.Release.Verifier.Tests.csproj")
    policy=own/"policy/shop_diagnostic_policy.json"
    summaries["surface"]=passed(run([dotnet,str(verifier),"--assembly",str(candidate),"--source-root",str(source),"--policy",str(policy)],own,env),"shop_diagnostic_v1_release_surface")
    ...
    summaries["verifier_mutations"]=passed(run([dotnet,str(mutations),"--candidate",str(candidate),"--source-root",str(source),"--policy",str(policy)],own,env),"shop_diagnostic_v1_release_verifier",38)
    summaries["verifier_cli"]=release.verify_cli_negatives(dotnet,verifier,candidate,source,policy,args.scratch,own,env)
    summaries["verifier_cli"]["suite"]="shop_diagnostic_cli"
    summaries["package"]=passed(run([sys.executable,"-B","-I","-S",str(own/"package/package_fixtures.py"),str(candidate)],own,env,unittest=True),"shop_diagnostic_package",5)
    ...
    result={"schema_version":1,"status":"passed","suite":"shop_diagnostic_v1_offline","source_inventory_sha256":inventory,
