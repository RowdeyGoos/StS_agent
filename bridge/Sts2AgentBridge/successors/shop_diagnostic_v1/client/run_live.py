#!/usr/bin/env python3
"""One fixed campaign validation and one passive diagnostic observation."""
...
def verify_sources() -> None:
    for component in (*FROZEN, 'shop_diagnostic_v1'):
        ...
        if manifest.get('schema_version') != 1 or manifest.get('component') != component:
            raise ValueError()
        if component == 'shop_diagnostic_v1' and manifest.get('contract_sha256') != CONTRACT:
            raise ValueError()
        names = set()
...
def main() -> int:
    result = {'schema_version': 1, 'status': 'failed', 'code': 'diagnostic_client_preflight_failed'}
    try:
        args = sys.argv[1:]
        ...
        from diagnostic_transport import run_authenticated_diagnostic
        result = run_once(args[1], manager.validate_installed_for_client, read_credential,
                          run_authenticated_diagnostic, manager.require_no_granting_acl_fd)
