"""Descriptor-bound read lease for the one fixed item operator tree."""
...
ENABLED = b'{"schema_version":"shop_diagnostic_v1_transport_config_v1","enabled":true,"bind_address":"127.0.0.1","port":43117,"token_file":"credential.hex"}'
...
def read_credential(home: Path, uid: int, state: Any,
    ...
    try:
        ...
        item = open_entry('shop_diagnostic_v1', private=True, owned=True)
        ...
        for entry in fds:
            validate(entry)
        # Both leaves are relative to the retained item directory, never the config FD.
        parent = item[0]
