...
_EXPECTED_CANONICAL_ARTIFACTS = (
    (
        "Sts2AgentBridgeShopDiagnosticV1.dll",
        83_968,
        "c4151b5277715e605417e064bd249d7d8ad9bc0a9d2d0be6408a56be2b65771b",
    ),
    (
        "Sts2AgentBridgeShopDiagnosticV1.json",
        353,
...
_POLICY = manager.ArtifactPolicy(
    ...
    archive=manager.ArtifactSpec(
        "Sts2AgentBridgeShopDiagnosticV1-0.1.0.zip",
        len(_ARCHIVE_DATA),
...
class FixtureTree:
    def __init__(self, root: Path, *, preexisting_mods: bool = False) -> None:
        self.root = root
        ...
        self.overlay_root = self.mods_parent / "Sts2AgentBridgeShopDiagnosticV1"
        self.operator_parent = self.application_support / "Sts2AgentBridge"
        self.config_root = self.operator_parent / "shop_diagnostic_v1"
...
def _assert_recorded_retention(
    ...
    for overlay_unit in present_overlays:
        overlay_root = (
            overlay_unit / "Sts2AgentBridgeShopDiagnosticV1"
...
def _assert_install(tree: FixtureTree, mods_created: bool) -> None:
    state, _ = manager._read_state(tree.layout, os.geteuid())
    if state.phase != "installed" or state.mods_parent_created != mods_created:
...
def _verify_production_bindings() -> None:
    if manager.CAMPAIGN_ID != "SHOP-DIAGNOSTIC-V1-SMOKE-V1":
    ...
    if (
        len(manager.CONFIG_BYTES) != 144
        or hashlib.sha256(manager.CONFIG_BYTES).hexdigest()
        != "02962b9519c77959461b1cafb4dd40c1432309072a8fa03d8480c41059d8eece"
        or manager.CONFIG_SHA256
        != "02962b9519c77959461b1cafb4dd40c1432309072a8fa03d8480c41059d8eece"
    ):
    ...
    expected_overlay = (
        ...
        / "MacOS"
        / "mods"
        / "Sts2AgentBridgeShopDiagnosticV1"
    )
    expected_config = (
        profile
        / "Library"
    ...
    if (
        ...
        / "Library"
        / "Application Support"
        / "Sts2AgentBridgeCampaign-shop-diagnostic-v1-smoke-v1"
        or manager.ARTIFACT_ROOT
        != Path("/private/tmp/sts-shop-diagnostic-v1-release")
...
def _exercise_client_validation_api(root: Path) -> None:
    ...
    config_path.unlink()
    config_path.write_bytes(manager.CONFIG_BYTES)
    config_path.chmod(0o600)
    with (
        mock.patch.object(manager.sys, "platform", "darwin"),
        mock.patch.object(manager.os, "geteuid", return_value=501),
...
def operation() -> dict[str, object]:
    ...
    with tempfile.TemporaryDirectory(
        ...
        existing_mods = FixtureTree(root / "full-existing-mods", preexisting_mods=True)
