...
_MANIFEST_SHA256 = "3c6d1ae87fd339868c3cc52a5fa8115968402aa8a6c981fa8d64a5c7a909946d"
...
def _release_package() -> ModuleType:
    ...
    spec = importlib.util.spec_from_file_location("verified_shop_diagnostic_package", path)
...
def _write_package(path: Path, dll: bytes, manifest: bytes) -> bytes:
    with zipfile.ZipFile(
        ...
        for name, data in (
            (
                "Sts2AgentBridgeShopDiagnosticV1/Sts2AgentBridgeShopDiagnosticV1.dll",
                dll,
            ),
            (
                "Sts2AgentBridgeShopDiagnosticV1/Sts2AgentBridgeShopDiagnosticV1.json",
                manifest,
            ),
        ):
...
class _Fixture:
    def __init__(
        ...
        target = {
            "manifest_id": "synthetic-shop-diagnostic-target",
            "identity": {
                "installation_tree": {
    ...
    def patch(self, arguments: list[str]) -> object:
        values = {
            ...
            "TARGET_MANIFEST_ID": "synthetic-shop-diagnostic-target",
    ...
    def install_overlay(self, manifest: bytes | None = None) -> None:
        ...
        (directory / "Sts2AgentBridgeShopDiagnosticV1.dll").write_bytes(self.dll)
        (directory / "Sts2AgentBridgeShopDiagnosticV1.json").write_bytes(
...
def operation() -> dict[str, object]:
    ...
    with tempfile.TemporaryDirectory(
        prefix="shop-diagnostic-clean-install.", dir="/private/tmp"
    ) as temporary:
        fixture = _Fixture(
    ...
    with tempfile.TemporaryDirectory(
        ...
        mutated[mutated.index(b"Shop Diagnostic V1")] ^= 1
