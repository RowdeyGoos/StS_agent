...
CAMPAIGN_ID = "SHOP-DIAGNOSTIC-V1-SMOKE-V1"
STATE_ROOT_NAME = "Sts2AgentBridgeCampaign-shop-diagnostic-v1-smoke-v1"
...
ARTIFACT_ROOT = Path("/private/tmp/sts-shop-diagnostic-v1-release")
CONFIG_BYTES = (
    b'{"schema_version":"shop_diagnostic_v1_transport_config_v1","enabled":true,'
    b'"bind_address":"127.0.0.1","port":43117,"token_file":"credential.hex"}'
)
CONFIG_SHA256 = "02962b9519c77959461b1cafb4dd40c1432309072a8fa03d8480c41059d8eece"
...
OVERLAY_DLL_NAME = "Sts2AgentBridgeShopDiagnosticV1.dll"
OVERLAY_MANIFEST_NAME = "Sts2AgentBridgeShopDiagnosticV1.json"
...
CANONICAL_ARTIFACTS = ArtifactPolicy(
    dll=ArtifactSpec(
        OVERLAY_DLL_NAME,
        83_968,
        "c4151b5277715e605417e064bd249d7d8ad9bc0a9d2d0be6408a56be2b65771b",
    ),
    manifest=ArtifactSpec(
        OVERLAY_MANIFEST_NAME,
        353,
        "3c6d1ae87fd339868c3cc52a5fa8115968402aa8a6c981fa8d64a5c7a909946d",
    ),
    archive=ArtifactSpec(
        "Sts2AgentBridgeShopDiagnosticV1-1.0.0.zip",
        84_765,
        "e249e180f88937ade8d64428b5708f6080749f754e100408ea902afb6a2454e0",
    ),
)
...
) -> tuple[str, Path, int, Path | None, str | None]:
...
def _production_layout(user_profile: Path) -> CampaignLayout:
    ...
    overlay_root = mods_parent / "Sts2AgentBridgeShopDiagnosticV1"
    operator_parent = application_support / "Sts2AgentBridge"
    config_root = operator_parent / "shop_diagnostic_v1"
...
def _validate_config_tree(root: Path, uid: int) -> dict[Path, os.stat_result]:
    records: dict[Path, os.stat_result] = {}
    records[root] = _require_directory(root, uid, 0o700)
    ...
    config_data = _read_stable_file(config_path, config_metadata, len(CONFIG_BYTES))
    if config_data != CONFIG_BYTES or hashlib.sha256(config_data).hexdigest() != CONFIG_SHA256:
        fail(EXIT_MISMATCH, "generated_content_mismatch")
...
def _validate_operator_unit(root: Path, uid: int) -> dict[Path, os.stat_result]:
    records: dict[Path, os.stat_result] = {}
    records[root] = _require_directory(root, uid, 0o700)
    _list_exact(root, {"shop_diagnostic_v1"})
    records.update(_validate_config_tree(root / "shop_diagnostic_v1", uid))
    return records


def _validate_config_tree_metadata(root: Path, uid: int) -> dict[Path, os.stat_result]:
...
def _validate_operator_unit_metadata(
    ...
    records[root] = _require_directory(root, uid, 0o700)
    _list_exact(root, {"shop_diagnostic_v1"})
    records.update(_validate_config_tree_metadata(root / "shop_diagnostic_v1", uid))
    return records
...
def _validate_overlay_unit(
    ...
    _list_exact(unit_root, {"Sts2AgentBridgeShopDiagnosticV1"})
    records.update(
        _validate_overlay_tree(unit_root / "Sts2AgentBridgeShopDiagnosticV1", uid, policy)
    )
    return records
...
def _require_state_records(
    ...
    config_root = operator_root / "shop_diagnostic_v1"
    overlay_root = (
        overlay_unit_root / "Sts2AgentBridgeShopDiagnosticV1"
        if state.mods_parent_created
...
def _validate_installed_sources(
    ...
    operator = _validate_operator_unit(layout.operator_parent, uid)
...
def _validate_quarantined_units(
    ...
    operator = _validate_operator_unit(layout.quarantine_operator, uid)
    overlay = _validate_overlay_unit(
...
def _validate_activation_units(
    ...
    operator = _validate_operator_unit(operator_root, uid)
    overlay = _validate_overlay_unit(
        overlay_unit_root,
        state.mods_parent_created,
        uid,
...
def _install(
    ...
    try:
        ...
        with _private_umask():
            ...
            staged_config = layout.staged_operator / "shop_diagnostic_v1"
            ...
            if mods_parent_created:
                staged_overlay_root = layout.staged_overlay / "Sts2AgentBridgeShopDiagnosticV1"
        ...
        state = bound
        staged_operator_records = _validate_operator_unit(layout.staged_operator, uid)
...
def _validate_purge_remainder(
    ...
    if progress < 4:
        ...
        expected_operator = {"shop_diagnostic_v1"} if progress < 3 else set()
    ...
    if progress < overlay_removed_at:
        ...
        if state.mods_parent_created:
            expected_unit = {"Sts2AgentBridgeShopDiagnosticV1"} if progress < 7 else set()
...
def validate_installed_for_client(
    ...
    if state.phase != "installed":
        fail(EXIT_MISMATCH, "wrong_phase")
    _validate_live_parents(layout, uid, state)
    operator = _validate_operator_unit_metadata(layout.operator_parent, uid)
    overlay_unit = _active_overlay_unit(layout, state)
    overlay = _validate_overlay_unit(
        overlay_unit,
        state.mods_parent_created,
