...
internal static class ShopDiagnosticOperatorFiles
{
    internal static ItemOperatorConfiguration? OpenEnabled()
    {
        if (!DarwinReadOnly.IsSupportedPlatform()) return null;
        ...
        string[] homeComponents)
    {
        ...
        try
        {
            ...
            if (!descriptors.TryOpenRoot()) return CloseAndNull(descriptors);
    ...
    private static ItemOperatorConfiguration? OpenBelowHome(DarwinDescriptorLease descriptors)
    {
        ...
        try
        {
            if (!descriptors.TryOpenDirectory("Library", DarwinDescriptorPolicy.OwnedDirectory) ||
                ...
                !descriptors.TryOpenDirectory("shop_diagnostic_v1", DarwinDescriptorPolicy.PrivateDirectory) ||
    ...
    private static bool IsExactEnabled(byte[] bytes) => FixedEquals(bytes, EnabledDocument);
