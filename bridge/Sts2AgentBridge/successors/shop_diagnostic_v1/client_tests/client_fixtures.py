...
from test_diagnostic_transport import READY, http
...
class Fixture:
    def __enter__(self):
        ...
        self.item = self.home / 'Library' / 'Application Support' / 'Sts2AgentBridge' / 'shop_diagnostic_v1'
...
class ClientTests(unittest.TestCase):
    ...
    def test_composition_one_call_and_zero(self):
        for error in (None,RuntimeError('synthetic'),KeyboardInterrupt()):
            ...
            def collect(value):
                ...
                return {'schema_version':1,'status':'failed','code':'diagnostic_transport_failed'}
    ...
    def test_cli_invalid_inputs_sanitized(self):
        for args in ([],['--path','/synthetic-secret'],['--expected-state-sha256','BAD']):
            ...
            self.assertEqual(json.loads(result.stdout),{'schema_version':1,'status':'failed','code':'diagnostic_client_preflight_failed'})

    def test_ephemeral_single_diagnostic_and_lost_delivery(self):
        for uncertain in (False,True):
            with self.subTest(uncertain=uncertain), Fixture() as f:
                ...
                responses=[None] if uncertain else [READY]
                def serve():
                    try:
                ...
                def collect(token):
                    import time
                    return transport._run_with_socket_factory(token,Redirect,clock=time.monotonic)
                ...
                self.assertEqual(sum(r.startswith(b'POST ') for r in requests),0)
...
if __name__=='__main__':
    ...
    if result.wasSuccessful(): print(json.dumps({'schema_version':1,'status':'passed','suite':'shop_diagnostic_client','check_count':result.testsRun},separators=(',',':')))
