...
internal static class Program
{
    ...
    internal static int Main(string[] arguments)
    {
        if (arguments.Length != 2 || arguments[0] != "--fixture-root" ||
            !ShopDiagnosticOperatorFiles.IsSyntheticHomeForTests(arguments[1]) ||
            Directory.Exists(arguments[1]) || File.Exists(arguments[1]))
            return 2;
        _root = arguments[1];
        try
        {
            Run(() => ExactEnabled(Enabled));
            Run(DefaultDenyAndIsolation);
            Run(CredentialReservationAndBinding);
            Run(LeaseCleanupAndSurface);
            Console.WriteLine("{\"schema_version\":1,\"status\":\"passed\",\"suite\":\"shop_diagnostic_v1_operator\",\"check_count\":" + _checks + "}");
            return 0;
        }
        catch
        {
            Console.Error.WriteLine("shop_diagnostic_v1_operator_tests_failed");
            return 1;
        }
        finally
        {
            Array.Clear(Enabled); Array.Clear(Credential);
    ...
    private static void ExactEnabled(byte[] expected)
    {
        ...
        using ItemOperatorConfiguration lease = Present(ShopDiagnosticOperatorFiles.OpenEnabledForTests(_root));
    ...
    private static void DefaultDenyAndIsolation()
    {
        byte[][] rejected =
        {
            Encoding.ASCII.GetBytes("{\"schema_version\":\"shop_diagnostic_v1_transport_config_v1\",\"enabled\":false,\"bind_address\":\"127.0.0.1\",\"port\":43117,\"token_file\":\"credential.hex\"}"),
            Encoding.ASCII.GetBytes("{\"schema_version\":\"room_flows_v1_transport_config_v1\",\"enabled\":true,\"flow_kind\":\"shop\",\"bind_address\":\"127.0.0.1\",\"port\":43117,\"token_file\":\"credential.hex\"}"),
            ...
            Enabled.Concat(new byte[] { (byte)'\n' }).ToArray(),
        };
        foreach (byte[] value in rejected)
        {
            Write(ConfigPath, value);
            Require(ShopDiagnosticOperatorFiles.OpenEnabledForTests(_root) is null);
            Array.Clear(value);
        }
        ...
        Write(Path.Combine(item, "config.json"), Enabled);
    ...
    {
        using ItemOperatorConfiguration invalid = Present(ShopDiagnosticOperatorFiles.OpenEnabledForTests(_root));
        ...
        Reset();
        ItemOperatorConfiguration replaced = Present(ShopDiagnosticOperatorFiles.OpenEnabledForTests(_root));
        string old = ConfigPath + ".old";
        File.Move(ConfigPath, old); Write(ConfigPath, Enabled);
        ...
        Require(retained.SequenceEqual(Enabled));
        Array.Clear(retained); replaced.Dispose();
    }

    private static void LeaseCleanupAndSurface()
    {
        ItemOperatorConfiguration lease = Present(ShopDiagnosticOperatorFiles.OpenEnabledForTests(_root));
        byte[] retained = Present(lease.RetainedConfigurationForTests);
        lease.Dispose(); lease.Dispose();
        Require(retained.All(value => value == 0));
        string[] methods = typeof(ShopDiagnosticOperatorFiles).GetMethods(
            BindingFlags.Static | BindingFlags.Public | BindingFlags.NonPublic | BindingFlags.DeclaredOnly)
            .Select(method => method.Name).OrderBy(name => name, StringComparer.Ordinal).ToArray();
        Require(methods.SequenceEqual(new[] { "CloseAndNull", "FixedEquals", "IsExactEnabled", "IsSyntheticHome", "IsSyntheticHomeForTests", "OpenBelowHome", "OpenEnabled", "OpenEnabledForTests", "OpenFromAbsoluteHome", "get_EnabledDocument" }));
        Require(!ShopDiagnosticOperatorFiles.IsSyntheticHomeForTests("/private/tmp/a/b") &&
            !ShopDiagnosticOperatorFiles.IsSyntheticHomeForTests("/tmp/room-flow"));
    }
    ...
    private static void Reset()
    {
        ...
        foreach (string path in new[] { _root, Path.Combine(_root, "Library"),
                     Path.Combine(_root, "Library", "Application Support"), BridgePath, DiagnosticPath })
            File.SetUnixFileMode(path, DirectoryMode);
        Write(ConfigPath, Enabled); Write(CredentialPath, Credential);
    }
    ...
    private static string DiagnosticPath => Path.Combine(BridgePath, "shop_diagnostic_v1");
    private static string ConfigPath => Path.Combine(DiagnosticPath, "config.json");
    private static string CredentialPath => Path.Combine(DiagnosticPath, "credential.hex");
