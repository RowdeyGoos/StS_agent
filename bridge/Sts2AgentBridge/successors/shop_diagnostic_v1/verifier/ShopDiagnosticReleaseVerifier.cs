...
internal sealed record ShopDiagnosticReleaseVerificationReport(
    string AssemblySha256,
    string SourceProjectionSha256,
    string MetadataProjectionSha256,
    int CheckedMethodBodies);

internal static class ShopDiagnosticReleaseVerifier
{
    private const string ExpectedAssemblyName = "Sts2AgentBridgeShopDiagnosticV1";
    ...
    private const string ExpectedInitializer =
        "type:Sts2AgentBridge.Successors.ShopDiagnosticV1.ShopDiagnosticModEntry|" +
    ...
    private const string ExpectedEntryType =
        "type_def|Sts2AgentBridge.Successors.ShopDiagnosticV1.ShopDiagnosticModEntry|1048961|System.Object|0|0";
    private const string ExpectedEntryMethod =
        "method_def|Sts2AgentBridge.Successors.ShopDiagnosticV1.ShopDiagnosticModEntry.Initialize()|System.Void|0|0|0|150|0";

    internal static ShopDiagnosticReleaseVerificationReport Verify(
        byte[] candidate,
        string sourceRoot,
        ShopDiagnosticReleasePolicy policy,
        bool enforceArtifactIdentity)
    {
        ...
        return new ShopDiagnosticReleaseVerificationReport(
    ...
    internal static void VerifySemanticRules(ShopDiagnosticReleaseProjection projection)
    {
        ...
        if (projection.ActionReferences.Length != 0)
        {
            throw new VerificationException("action_surface_forbidden");
        }
        ...
        if (!projection.InitializerAttributes.SequenceEqual(new[] { ExpectedInitializer }, StringComparer.Ordinal) ||
            ...
            projection.Inventory.Count(value => value.StartsWith(
                "method_def|Sts2AgentBridge.Successors.ShopDiagnosticV1.ShopDiagnosticModEntry.",
    ...
    private static string VerifySources(string sourceRoot, IReadOnlyList<PolicySourceFile> sources)
    {
        ...
        foreach (PolicySourceFile source in sources)
        {
            ...
            string sha = ShopDiagnosticReleaseProjectionBuilder.Sha(bytes);
        ...
        return ShopDiagnosticReleaseProjectionBuilder.Sha(Encoding.UTF8.GetBytes(projection.ToString()));
    }
