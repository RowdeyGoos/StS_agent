...
internal static class Program
{
    public static int Main(string[] args)
    {
        try
        {
            ...
            byte[] policyBytes = SafeFiles.ReadBoundedRegularFile(parsed.Policy, 1024 * 1024, "policy_boundary");
            string policySha = ShopDiagnosticReleaseProjectionBuilder.Sha(policyBytes);
            if (policySha != ShopDiagnosticReleasePolicy.ExpectedPolicySha256)
            {
                throw new VerificationException("policy_identity_mismatch");
            }
            ShopDiagnosticReleasePolicy policy = ShopDiagnosticReleasePolicy.Parse(policyBytes);
            byte[] candidate = SafeFiles.ReadBoundedRegularFile(parsed.Assembly, 64 * 1024 * 1024, "candidate_boundary");
            ShopDiagnosticReleaseVerificationReport report = ShopDiagnosticReleaseVerifier.Verify(
            ...
            Emit(new SortedDictionary<string, object?>(StringComparer.Ordinal)
            {
                ...
                ["status"] = "passed",
                ["suite"] = "shop_diagnostic_v1_release_surface",
            });
            return 0;
        }
