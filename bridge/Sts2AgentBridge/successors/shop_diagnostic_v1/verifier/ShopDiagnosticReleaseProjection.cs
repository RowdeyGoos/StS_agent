...
internal sealed record ShopDiagnosticReleaseProjection(
    string AssemblyName,
...
internal static partial class ShopDiagnosticReleaseProjectionBuilder
{
    private static readonly Regex RoutePattern = new(
        @"/probe/[A-Za-z0-9_./-]*",
        RegexOptions.CultureInvariant);

    internal static ShopDiagnosticReleaseProjection Build(byte[] image)
    {
        try
        {
            ...
            string latin = Encoding.Latin1.GetString(image);
            string unicode = Encoding.Unicode.GetString(image);
            AddRoutes(latin, routes);
            AddRoutes(unicode, routes);
            AddForbiddenActionLiterals(latin, actions);
            AddForbiddenActionLiterals(unicode, actions);
            ...
            foreach ((MethodDefinitionHandle handle, string identity) in methods)
            {
                ...
                MethodSignature<string> signature = method.DecodeSignature(names, null);
                inventory.Add(FormattableString.Invariant(
                    $"method_def|{identity}|{signature.ReturnType}|{signature.GenericParameterCount}|{signature.RequiredParameterCount}|{(signature.Header.IsInstance ? 1 : 0)}|{(int)method.Attributes}|{(int)method.ImplAttributes}"));
                ...
                foreach (ParameterHandle parameterHandle in method.GetParameters())
                {
                    ...
                    inventory.Add(FormattableString.Invariant(
                        $"parameter|{identity}|{parameter.SequenceNumber}|{reader.GetString(parameter.Name)}|{(int)parameter.Attributes}"));
                }
            }
            ...
            foreach ((MethodDefinitionHandle handle, string owner) in methods)
            {
                ...
                inventory.Add($"body|{owner}|{Sha(Encoding.UTF8.GetBytes(string.Join('\n', bodyLines)))}");
            }

            string[] ordered = inventory.OrderBy(value => value, StringComparer.Ordinal).ToArray();
            actions.AddRange(ordered.Where(IsForbiddenActionIdentity));
            return new ShopDiagnosticReleaseProjection(
                assemblyName,
                assemblyVersion,
                Sha(image),
                image.LongLength,
                ...
                actions.OrderBy(value => value, StringComparer.Ordinal).ToArray(),
        ...
        SortedSet<string> routes)
    {
        ...
        if (instruction.OpCode == 0x0072)
        {
            ...
            foreach (Match match in RoutePattern.Matches(value)) routes.Add(match.Value);
    ...
    private static void AddRoutes(string value, SortedSet<string> routes)
    {
        foreach (Match match in RoutePattern.Matches(value)) routes.Add(match.Value);
    }

    private static void AddForbiddenActionLiterals(string image, List<string> actions)
    {
        foreach ((string value, string label) in new[]
        {
            ("X-Probe-Decision", "decision_header"),
            ("X-Probe-Action", "action_header"),
            ("POST ", "post_method"),
            ("buy:card:", "buy_action"),
            ("inventory:close", "close_action"),
            ("collect:", "collect_action"),
            ("choose:", "choice_action"),
        })
        {
            if (image.Contains(value, StringComparison.Ordinal))
                actions.Add("literal|" + label);
        }
    }

    private static bool IsForbiddenActionIdentity(string row)
    {
        if (row.Contains(".Apply(", StringComparison.Ordinal) ||
            row.Contains(".CapturePending(", StringComparison.Ordinal))
            return true;
        return row.Contains("IShopV1NativeDispatch", StringComparison.Ordinal) ||
            row.Contains("ShopV1Session", StringComparison.Ordinal) ||
            row.Contains("PinnedPurchaseDispatch", StringComparison.Ordinal) ||
            row.Contains("ShopV1Pending", StringComparison.Ordinal) ||
            row.Contains("ShopV1Completion", StringComparison.Ordinal) ||
            row.Contains("ShopV1ActionKind", StringComparison.Ordinal) ||
            row.Contains("RoomFlowDispatchReceipt", StringComparison.Ordinal) ||
            row.Contains("PurchaseStatus", StringComparison.Ordinal) ||
            row.Contains("Godot.InputEventAction", StringComparison.Ordinal) ||
            row.Contains("MegaCrit.Sts2.Core.ControllerInput.MegaInput", StringComparison.Ordinal) ||
            row.Contains(".ForceClick(", StringComparison.Ordinal) ||
            row.Contains("._GuiInput(", StringComparison.Ordinal) ||
            row.Contains(".PurchaseCompleted", StringComparison.Ordinal) ||
            row.Contains(".add_PurchaseCompleted(", StringComparison.Ordinal) ||
            row.Contains(".remove_PurchaseCompleted(", StringComparison.Ordinal);
    }

    private static bool IsForbiddenRuntimeReference(string owner, string identity) =>
        identity.StartsWith("member:System.Reflection.", StringComparison.Ordinal) &&
            !(owner == "Sts2AgentBridge.Successors.ItemBootstrapV1.PinnedItemBuildGuard.Verify()" &&
