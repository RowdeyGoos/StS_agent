#!/usr/bin/env python3
"""Static regressions for the shop diagnostic project's build-input boundary."""
...
def _components() -> dict[str, dict[str, bytes]]:
    ...
    production = components["shop_diagnostic_v1"]["production/Sts2AgentBridgeShopDiagnosticV1.csproj"]
    import xml.etree.ElementTree as ET

    source_paths = []
    for node in ET.fromstring(production).iter("Compile"):
        relative = node.attrib["Include"]
        normalized = os.path.normpath("shop_diagnostic_v1/production/" + relative)
        source_paths.append("bridge/Sts2AgentBridge/successors/" + normalized)
    components["shop_diagnostic_v1"]["policy/shop_diagnostic_policy.json"] = json.dumps(
...
def main() -> int:
    ...
    for name in (
        ...
        changed["shop_diagnostic_v1"][name] = b"<Project />\n"
    ...
    for before, after in (
        ...
        changed = {component: dict(files) for component, files in base.items()}
        changed["shop_diagnostic_v1"][props_path] = _replace(
            changed["shop_diagnostic_v1"][props_path], before, after)
        _expect_rejected(changed)
        checks += 1

    runtime_path = "runtime/Sts2AgentBridge.ShopDiagnosticV1.Transport.csproj"
    changed = {component: dict(files) for component, files in base.items()}
    changed["shop_diagnostic_v1"][runtime_path] = _replace(
        changed["shop_diagnostic_v1"][runtime_path],
    ...
    production_path = "production/Sts2AgentBridgeShopDiagnosticV1.csproj"
    for before, after in (
        ...
        changed = {component: dict(files) for component, files in base.items()}
        changed["shop_diagnostic_v1"][production_path] = _replace(
            changed["shop_diagnostic_v1"][production_path], before, after)
        _expect_rejected(changed)
        checks += 1
    ...
    for injected in (
        ...
        changed = {component: dict(files) for component, files in base.items()}
        changed["shop_diagnostic_v1"][runtime_path] = _replace(
            changed["shop_diagnostic_v1"][runtime_path], b"</Project>", injected + b"</Project>")
        _expect_rejected(changed)
        checks += 1
    ...
    print(json.dumps({
        "schema_version": 1,
        "status": "passed",
        "suite": "shop_diagnostic_project_boundary",
        "check_count": checks,
