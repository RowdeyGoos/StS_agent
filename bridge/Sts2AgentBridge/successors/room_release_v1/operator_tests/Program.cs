...
{
    private static readonly byte[] Shop = Encoding.ASCII.GetBytes(
    ...
    private static readonly byte[] Event = Encoding.ASCII.GetBytes(
    ...
    internal static int Main(string[] arguments)
    {
        if (arguments.Length != 2 || arguments[0] != "--fixture-root" ||
            !RoomFlowOperatorFiles.IsSyntheticHomeForTests(arguments[1]) ||
            Directory.Exists(arguments[1]) || File.Exists(arguments[1]))
            return 2;
        _root = arguments[1];
        try
        {
            Run(() => ExactEnabled(Shop));
            Run(() => ExactEnabled(Event));
            Run(DefaultDenyAndIsolation);
            Run(CredentialReservationAndBinding);
            Run(LeaseCleanupAndSurface);
            Console.WriteLine("{\"schema_version\":1,\"status\":\"passed\",\"suite\":\"room_flows_v1_operator\",\"check_count\":" + _checks + "}");
            return 0;
        }
        catch
        {
            Console.Error.WriteLine("room_flows_v1_operator_tests_failed");
            return 1;
        }
        finally
        {
            Array.Clear(Shop); Array.Clear(Event); Array.Clear(Credential);
    ...
    private static void ExactEnabled(byte[] expected)
    {
        ...
        using ItemOperatorConfiguration lease = Present(RoomFlowOperatorFiles.OpenEnabledForTests(_root));
    ...
    private static void DefaultDenyAndIsolation()
    {
        byte[][] rejected =
        {
            Encoding.ASCII.GetBytes("{\"schema_version\":\"room_flows_v1_transport_config_v1\",\"enabled\":false,\"flow_kind\":\"shop\",\"bind_address\":\"127.0.0.1\",\"port\":43117,\"token_file\":\"credential.hex\"}"),
            Encoding.ASCII.GetBytes("{\"schema_version\":\"room_flows_v1_transport_config_v1\",\"enabled\":true,\"flow_kind\":\"rest\",\"bind_address\":\"127.0.0.1\",\"port\":43117,\"token_file\":\"credential.hex\"}"),
            ...
            Shop.Concat(new byte[] { (byte)'\n' }).ToArray(),
        };
        foreach (byte[] value in rejected)
        {
            Write(ConfigPath, value);
            Require(RoomFlowOperatorFiles.OpenEnabledForTests(_root) is null);
            Array.Clear(value);
        }
        ...
        Write(Path.Combine(item, "config.json"), Shop);
    ...
    {
        using ItemOperatorConfiguration invalid = Present(RoomFlowOperatorFiles.OpenEnabledForTests(_root));
        ...
        Reset();
        ItemOperatorConfiguration replaced = Present(RoomFlowOperatorFiles.OpenEnabledForTests(_root));
        string old = ConfigPath + ".old";
        File.Move(ConfigPath, old); Write(ConfigPath, Shop);
        ...
        Require(retained.SequenceEqual(Shop));
        Array.Clear(retained); replaced.Dispose();
    }

    private static void LeaseCleanupAndSurface()
    {
        ItemOperatorConfiguration lease = Present(RoomFlowOperatorFiles.OpenEnabledForTests(_root));
        byte[] retained = Present(lease.RetainedConfigurationForTests);
        lease.Dispose(); lease.Dispose();
        Require(retained.All(value => value == 0));
        string[] methods = typeof(RoomFlowOperatorFiles).GetMethods(
            BindingFlags.Static | BindingFlags.Public | BindingFlags.NonPublic | BindingFlags.DeclaredOnly)
            .Select(method => method.Name).OrderBy(name => name, StringComparer.Ordinal).ToArray();
        Require(methods.SequenceEqual(new[] { "CloseAndNull", "FixedEquals", "IsExactEnabled", "IsSyntheticHome", "IsSyntheticHomeForTests", "OpenBelowHome", "OpenEnabled", "OpenEnabledForTests", "OpenFromAbsoluteHome", "get_EventEnabled", "get_ShopEnabled" }));
        Require(!RoomFlowOperatorFiles.IsSyntheticHomeForTests("/private/tmp/a/b") &&
            !RoomFlowOperatorFiles.IsSyntheticHomeForTests("/tmp/room-flow"));
    }
    ...
    private static void Reset()
    {
        ...
        Write(ConfigPath, Shop); Write(CredentialPath, Credential);
    }
    ...
    private static string RoomPath => Path.Combine(BridgePath, "room_flows_v1");
