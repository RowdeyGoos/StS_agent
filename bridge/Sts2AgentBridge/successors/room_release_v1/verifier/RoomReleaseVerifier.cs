...
internal sealed record RoomReleaseVerificationReport(
    string AssemblySha256,
    string SourceProjectionSha256,
    string MetadataProjectionSha256,
    int CheckedMethodBodies);

internal static class RoomReleaseVerifier
{
    ...
    internal static RoomReleaseVerificationReport Verify(
        byte[] candidate,
        string sourceRoot,
        RoomReleasePolicy policy,
        bool enforceArtifactIdentity)
    {
        ...
        return new RoomReleaseVerificationReport(
    ...
    internal static void VerifySemanticRules(RoomReleaseProjection projection)
    {
    ...
    private static string VerifySources(string sourceRoot, IReadOnlyList<PolicySourceFile> sources)
    {
        ...
        foreach (PolicySourceFile source in sources)
        {
            ...
            string sha = RoomReleaseProjectionBuilder.Sha(bytes);
        ...
        return RoomReleaseProjectionBuilder.Sha(Encoding.UTF8.GetBytes(projection.ToString()));
    }
