...
internal sealed record RoomReleaseProjection(
    string AssemblyName,
...
internal static partial class RoomReleaseProjectionBuilder
{
    private static readonly Regex RoutePattern = new(
        @"/probe/[A-Za-z0-9_./-]*",
        RegexOptions.CultureInvariant);

    internal static RoomReleaseProjection Build(byte[] image)
    {
        try
        {
            ...
            return new RoomReleaseProjection(
                assemblyName,
                assemblyVersion,
                Sha(image),
                image.LongLength,
