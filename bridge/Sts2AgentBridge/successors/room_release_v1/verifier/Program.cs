...
internal static class Program
{
    public static int Main(string[] args)
    {
        try
        {
            ...
            string policySha = RoomReleaseProjectionBuilder.Sha(policyBytes);
            if (policySha != RoomReleasePolicy.ExpectedPolicySha256)
            {
                throw new VerificationException("policy_identity_mismatch");
            }
            RoomReleasePolicy policy = RoomReleasePolicy.Parse(policyBytes);
            byte[] candidate = SafeFiles.ReadBoundedRegularFile(parsed.Assembly, 64 * 1024 * 1024, "candidate_boundary");
            RoomReleaseVerificationReport report = RoomReleaseVerifier.Verify(
            ...
            Emit(new SortedDictionary<string, object?>(StringComparer.Ordinal)
            {
                ...
                ["status"] = "passed",
                ["suite"] = "room_flows_v1_release_surface",
            });
            return 0;
        }
