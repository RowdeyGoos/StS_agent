...
internal static class RoomFlowOperatorFiles
{
    ...
    private static ItemOperatorConfiguration? OpenBelowHome(DarwinDescriptorLease descriptors)
    {
        ...
        try
        {
            if (!descriptors.TryOpenDirectory("Library", DarwinDescriptorPolicy.OwnedDirectory) ||
                ...
                !descriptors.TryOpenDirectory("room_flows_v1", DarwinDescriptorPolicy.PrivateDirectory) ||
    ...
    private static bool IsExactEnabled(byte[] bytes) =>
        FixedEquals(bytes, ShopEnabled) || FixedEquals(bytes, EventEnabled);

    private static ReadOnlySpan<byte> ShopEnabled =>
    ...
    private static ReadOnlySpan<byte> EventEnabled =>
