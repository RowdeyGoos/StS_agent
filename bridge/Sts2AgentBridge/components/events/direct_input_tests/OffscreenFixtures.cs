...
internal static class OffscreenTests
{
    ...
    internal static int Main()
    {
        try
        {
            Success(20,0);Success(20,15);Success(20,19);Success(2,1);Missing();Disabled();Reassigned();Deferred();
            Console.WriteLine("{\"status\":\"passed\",\"suite\":\"direct_transform_holders\",\"check_count\":"+_checks+"}");return 0;
        }
        catch(Exception error){Console.Error.WriteLine(error);return 1;}
    }
    private static void Success(int domain, int slot)
    {
        ...
        Check(observation.LegalActions.SequenceEqual(Enumerable.Range(0,domain).Select(i=>"select:"+i)),"all allocated eligible holders offered");
        f.Act(child,"select:"+slot);
        Check(target.DispatchCount==1&&target.DispatchedSlots.SequenceEqual(new[]{slot}),"actual native input on holder16");
        Check(f.Preview.Visible&&f.Selected.Count==1&&ReferenceEquals(f.Selected[0],f.Cards[slot]),"card16 opens preview");
        Check(f.Before.Children.Count==1&&f.Before.Children[0] is NPreviewCardHolder holder&&ReferenceEquals(holder.CardNode.Model,f.Cards[slot]),"preview exact card16 original");
        f.Act(child,"confirm");
        Check(f.Child(child) is CardSelectionV1ResolvedResult&&f.CompletionValid&&ReferenceEquals(f.Originals.Single(),f.Cards[slot]),"card16 exact transformation");
    ...
    private static void Missing()
    {
        ...
        var child=f.Start();var observation=(CardSelectionV1Observation)f.Child(child);
        Check(!observation.LegalActions.Contains("select:15"),"unallocated holder not offered");
        ...
        Check(f.SelectCalls==0&&target.DispatchCount==0,"unallocated holder never clicks another");
    }
    private static void Disabled()
    {
        ...
        var child=f.Start();var observation=(CardSelectionV1Observation)f.Child(child);
        Check(!observation.LegalActions.Contains("select:15"),"disabled holder not offered");
        f.Session.ApplyCardChild(child.Child!.ParentDecisionId,child.Child.ParentActionId,child.Child.Ordinal,observation.DecisionId,"select:15");
        Check(f.SelectCalls==0&&target.DispatchCount==0,"disabled holder never clicks another");
    }
