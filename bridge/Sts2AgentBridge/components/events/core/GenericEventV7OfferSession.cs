...
public sealed record GenericEventV7OfferCapture(string Phase,IReadOnlyList<GenericEventV7Offer> Offers);
...
public sealed class GenericEventV7OfferSession:IGenericEventV7RewardChildSession {
    ...
    private readonly string _nonce;private readonly bool _bundle;private readonly int _count;
    ...
    public GenericEventV7OfferSession(string nonce,bool bundle,int count,IGenericEventV7OfferAdapter adapter) {
        if(count is <1 or >5)throw new ArgumentException("Offer count unavailable.");
        _nonce=nonce;_bundle=bundle;_count=count;_adapter=adapter;
    }
    public string ContractVersion=>_bundle?"bundle_offer_v1":"card_offer_v1";
    ...
    private string[] Actions()=>_phase=="choose"?Enumerable.Range(0,_count).Select(i=>"choose:"+i).ToArray():new[]{"confirm"};
    private GenericEventV7RewardRead Value(string status)=>new(_nonce,status,status=="ready"?_phase:status=="resolved"?"complete":status,
        ...
        _history.ToArray(),status is "ready" or "resolved"?_selected:null,Offers:status is "ready" or "resolved"?_offers:Array.Empty<GenericEventV7Offer>());
    public GenericEventV7RewardRead Read(){
        ...
        try {
            ...
            if(_pending!="") {
                ...
                _history.Add(new(_pendingDecision,_pending,_pending=="confirm"||!_bundle?"collected":"previewed"));_pending="";_phase=expected;
            }
            if(capture.Phase!=_phase)throw new InvalidOperationException("Unowned offer phase.");
            if(_phase=="complete")return Value("resolved");
