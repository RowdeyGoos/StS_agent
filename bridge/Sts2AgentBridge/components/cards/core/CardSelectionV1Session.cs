...
public sealed class CardSelectionV1Session : IDisposable
{
    ...
    public ICardSelectionV1ApplyValue Apply(string? decisionId, string? actionId)
    {
        lock (_gate)
        {
            ...
            if (_resolved is not null || _dispatching || _pending is not null ||
                ...
                _acceptedActions >= (_context.AllowOptionalSelection ? 16 : CardSelectionV1Limits.MaximumAcceptedActions))
            {
                return Failure("rejected");
            }
    ...
    private static int SelectionLimit(CardSelectionV1ParentContext context) => context.AllowOptionalSelection && context.Operation==CardSelectionV1Operation.Add ? 15 : CardSelectionV1Limits.MaximumSelectedCards;
    private static bool OptionalContextValid(CardSelectionV1ParentContext context) => !context.AllowOptionalSelection ||
        ...
        context.Operation==CardSelectionV1Operation.Add && context.CommitMode==CardSelectionV1CommitMode.ExplicitConfirm;
...
internal static class CardSelectionV1Identity
{
        ...
        IReadOnlyList<CardSelectionV1ActionResult> history)
    {
        ...
        if(context.AllowOptionalSelection) Append(builder, "card_add_v2");
