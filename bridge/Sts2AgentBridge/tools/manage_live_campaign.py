...
ARTIFACT_ROOT = Path("/private/tmp/sts-r0i-v5-package.AgboAp")
CONFIG_BYTES = (
    b'{"schema_version":"live_probe_v0_config_v1","enabled":true,'
    b'"bind_address":"127.0.0.1","port":43117,"token_file":"credential.hex"}'
)
CONFIG_SHA256 = "f8c6ff9592fa330cc9317cd63200c9ee5b0f8023c23efc328f04eea57d6dffea"
...
OVERLAY_DLL_NAME = "Sts2AgentBridge.dll"
OVERLAY_MANIFEST_NAME = "Sts2AgentBridge.json"
...
CANONICAL_ARTIFACTS = ArtifactPolicy(
    dll=ArtifactSpec(
        OVERLAY_DLL_NAME,
        195_584,
        "3d44887ac8935a1cbe8920e610db4a7d1209556242f555fce524e22c5f77b748",
    ),
    manifest=ArtifactSpec(
    ...
    archive=ArtifactSpec(
        ...
        196_223,
        "dfc4a85877aa9488584ee116959657dc114ccc3f458ee0c44b0a6e725799b385",
    ),
)
...
def _production_layout(user_profile: Path) -> CampaignLayout:
    ...
    overlay_root = mods_parent / "Sts2AgentBridge"
    operator_parent = application_support / "Sts2AgentBridge"
    config_root = operator_parent / "r0a"
...
def _validate_operator_unit(root: Path, uid: int) -> dict[Path, os.stat_result]:
    records: dict[Path, os.stat_result] = {}
    records[root] = _require_directory(root, uid, 0o700)
    _list_exact(root, {"r0a"})
    records.update(_validate_config_tree(root / "r0a", uid))
    return records
...
def _validate_overlay_unit(
    ...
    _list_exact(unit_root, {"Sts2AgentBridge"})
    records.update(
        _validate_overlay_tree(unit_root / "Sts2AgentBridge", uid, policy)
    )
    return records
...
def _require_state_records(
    ...
    config_root = operator_root / "r0a"
    overlay_root = (
        overlay_unit_root / "Sts2AgentBridge"
        if state.mods_parent_created
...
def _install(
    ...
    try:
        ...
        with _private_umask():
            ...
            staged_config = layout.staged_operator / "r0a"
            ...
            if mods_parent_created:
                staged_overlay_root = layout.staged_overlay / "Sts2AgentBridge"
...
def _validate_purge_remainder(
    ...
    if progress < 4:
        ...
        expected_operator = {"r0a"} if progress < 3 else set()
    ...
    if progress < overlay_removed_at:
        ...
        if state.mods_parent_created:
            expected_unit = {"Sts2AgentBridge"} if progress < 7 else set()
