...
class FixtureTree:
    def __init__(self, root: Path, *, preexisting_mods: bool = False) -> None:
        ...
        self.overlay_root = self.mods_parent / "Sts2AgentBridge"
        self.operator_parent = self.application_support / "Sts2AgentBridge"
        self.config_root = self.operator_parent / "r0a"
...
def _verify_production_bindings() -> None:
    if manager.CAMPAIGN_ID != "R0I-BATCHED-BRIDGE-SMOKE-V1":
    ...
    if (
        ...
        != Path("/private/tmp/sts-r0i-v5-package.AgboAp")
...
def operation() -> dict[str, object]:
    ...
    with tempfile.TemporaryDirectory(
        prefix="sts-r0a-campaign-fixtures-",
        dir="/private/tmp",
    ) as temporary:
        root = Path(temporary)
