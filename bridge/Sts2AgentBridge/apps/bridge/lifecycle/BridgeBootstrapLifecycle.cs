...
internal interface IBridgeBootstrapOperatorLease : IDisposable
{
...
internal interface IBridgeBootstrapFrameAttachment
{
...
internal interface IBridgeBootstrapFrameConnector
{
    IBridgeBootstrapFrameAttachment? Attach(Action onFrame);
}

internal interface IBridgeBootstrapRuntime
{
    bool Start();
...
internal interface IBridgeBootstrapRuntimeFactory
{
    // Ownership of both arrays transfers at invocation, including failure paths.
    IBridgeBootstrapRuntime? Create(byte[] configuration, byte[] credential);
}

internal interface IBridgeBootstrapClock
{
    long Timestamp { get; }
...
internal interface IBridgeBootstrapTimer : IDisposable
{
}

internal interface IBridgeBootstrapTimerFactory
{
    IBridgeBootstrapTimer Arm(TimeSpan dueTime, Action callback);
}

internal sealed class BridgeBootstrapLifecycle
{
    internal const int PendingLifetimeSeconds = 5;

    private readonly object _gate = new();
    private readonly Func<IBridgeBootstrapOperatorLease?> _openEnabled;
    private readonly Func<bool> _verifyBuild;
    private readonly IBridgeBootstrapFrameConnector _frames;
    private readonly IBridgeBootstrapRuntimeFactory _runtimeFactory;
    private readonly IBridgeBootstrapClock _clock;
    private readonly IBridgeBootstrapTimerFactory _timers;
    ...
    private IBridgeBootstrapTimer? _timer;
    private IBridgeBootstrapFrameAttachment? _attachment;
    private IBridgeBootstrapRuntime? _runtime;
    private bool _stopCallAttempted;
    private bool _stopCallInProgress;
    private bool _stopCallRetryRequired;

    internal BridgeBootstrapLifecycle(
        Func<IBridgeBootstrapOperatorLease?> openEnabled,
        Func<bool> verifyBuild,
        IBridgeBootstrapFrameConnector frames,
        IBridgeBootstrapRuntimeFactory runtimeFactory,
        IBridgeBootstrapClock clock,
        IBridgeBootstrapTimerFactory timers)
    {
    ...
    internal bool Initialize()
    {
        ...
        IBridgeBootstrapOperatorLease? lease = null;
        byte[]? configuration = null;
        byte[]? credential = null;
        IBridgeBootstrapTimer? timer = null;
        IBridgeBootstrapFrameAttachment? attachment = null;
        try
        {
            lease = _openEnabled();
            ...
            IBridgeBootstrapOperatorLease leaseToDispose = lease;
            lease = null;
    ...
    internal bool RequestStop()
    {
        byte[]? configuration;
        byte[]? credential;
        IBridgeBootstrapTimer? timer;
        IBridgeBootstrapRuntime? runtime;
        lock (_gate)
        {
            _stopRequested = true;
    ...
    private void OnFrame()
    {
        ...
        IBridgeBootstrapRuntime? runtimeToDrain = null;
        byte[]? configuration = null;
        byte[]? credential = null;
        IBridgeBootstrapTimer? timer = null;
    ...
    {
        IBridgeBootstrapRuntime? runtime = null;
        try
        {
    ...
    {
        IBridgeBootstrapFrameAttachment? attachment;
        IBridgeBootstrapRuntime? runtime;
        IBridgeBootstrapTimer? timer;
        lock (_gate)
        {
    ...
    private bool FailInitialization(
        IBridgeBootstrapOperatorLease? lease,
        byte[]? configuration,
        byte[]? credential,
        IBridgeBootstrapTimer? timer,
        IBridgeBootstrapFrameAttachment? attachment = null)
    {
        ...
        Zero(configuration);
        Zero(credential);
        IBridgeBootstrapTimer? failedTimer = timer is not null && !TryDispose(timer) ? timer : null;

        byte[]? ownedConfiguration;
        byte[]? ownedCredential;
        IBridgeBootstrapTimer? ownedTimer;
        lock (_gate)
        {
            _stopRequested = true;
            _initializationInProgress = false;
    ...
    private static bool IsTransportStopped(IBridgeBootstrapRuntime runtime)
    {
        try
        {
    ...
    private static bool IsServiceDisposed(IBridgeBootstrapRuntime runtime)
    {
        try
        {
    ...
    private void IssueRuntimeStop(IBridgeBootstrapRuntime runtime)
    {
        lock (_gate)
        {
