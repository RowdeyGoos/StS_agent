...
internal static class BridgeOperatorFiles
{
    ...
    private static ItemOperatorConfiguration? OpenBelowHome(DarwinDescriptorLease descriptors)
    {
        ...
        try
        {
            if (!descriptors.TryOpenDirectory("Library", DarwinDescriptorPolicy.OwnedDirectory) ||
                ...
                !descriptors.TryOpenDirectory("unified", DarwinDescriptorPolicy.PrivateDirectory) ||
    ...
    private static ReadOnlySpan<byte> GenericEnabled =>
        "{\"schema_version\":\"bridge_transport_config_v1\",\"enabled\":true,\"bind_address\":\"127.0.0.1\",\"port\":43117,\"token_file\":\"credential.hex\"}"u8;
