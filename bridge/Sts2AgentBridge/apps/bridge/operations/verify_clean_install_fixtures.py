...
_MANIFEST_SHA256 = hashlib.sha256(common.CANONICAL_MANIFEST).hexdigest()
...
def _write_package(path: Path, dll: bytes, manifest: bytes) -> bytes:
    with zipfile.ZipFile(
        ...
        for name, data in (
            (
                "Sts2AgentBridgeUnified/Sts2AgentBridgeUnified.dll",
                dll,
            ),
            (
                "Sts2AgentBridgeUnified/Sts2AgentBridgeUnified.json",
                manifest,
            ),
        ):
...
class _Fixture:
    ...
    def install_overlay(self, manifest: bytes | None = None) -> None:
        ...
        (directory / "Sts2AgentBridgeUnified.dll").write_bytes(self.dll)
        (directory / "Sts2AgentBridgeUnified.json").write_bytes(
...
def operation() -> dict[str, object]:
    ...
    _require(
        ...
        and len(release_manifest) == len(common.CANONICAL_MANIFEST)
    ...
    with tempfile.TemporaryDirectory(
        ...
        mutated[mutated.index(b"STS2 Agent Bridge")] ^= 1
