...
CAMPAIGN_ID = "UNIFIED-BRIDGE-V1"
STATE_ROOT_NAME = "Sts2AgentBridgeCampaign-unified-bridge-v1"
...
CONFIGURATIONS = {
    "unified": b'{"schema_version":"bridge_transport_config_v1","enabled":true,"bind_address":"127.0.0.1","port":43117,"token_file":"credential.hex"}',
}
...
GENERIC_V10_STATE_ROOT_NAME = "Sts2AgentBridgeCampaign-generic-event-v10-smoke-v1"
GENERIC_V9_STATE_ROOT_NAME = "Sts2AgentBridgeCampaign-generic-event-v9-smoke-v1"
GENERIC_V10_OVERLAY_ROOT_NAME = "Sts2AgentBridgeGenericEventV10"
GENERIC_V9_OVERLAY_ROOT_NAME = "Sts2AgentBridgeGenericEventV9"
GENERIC_V8_STATE_ROOT_NAME = "Sts2AgentBridgeCampaign-generic-event-v8-smoke-v1"
GENERIC_V8_OVERLAY_ROOT_NAME = "Sts2AgentBridgeGenericEventV8"
GENERIC_V7_STATE_ROOT_NAME = "Sts2AgentBridgeCampaign-generic-event-v7-smoke-v1"
GENERIC_V7_OVERLAY_ROOT_NAME = "Sts2AgentBridgeGenericEventV7"
GENERIC_V6_STATE_ROOT_NAME = "Sts2AgentBridgeCampaign-generic-event-v6-smoke-v1"
GENERIC_V6_OVERLAY_ROOT_NAME = "Sts2AgentBridgeGenericEventV6"
...
OVERLAY_DLL_NAME = "Sts2AgentBridgeUnified.dll"
OVERLAY_MANIFEST_NAME = "Sts2AgentBridgeUnified.json"
...
sys.path.insert(0, str(Path(__file__).absolute().parents[1] / "package"))
from release_package import IDENTITIES, DLL_NAME, MANIFEST_NAME, ZIP_NAME
CANONICAL_ARTIFACTS = ArtifactPolicy(*(ArtifactSpec(name, *IDENTITIES[name])
    for name in (DLL_NAME, MANIFEST_NAME, ZIP_NAME)))
...
def parse_args(
    ...
    flow_kind = None
    if mode == "install":
        if len(values) != 10 or values[8] != "--flow-kind" or values[9] not in ("unified",):
            fail(EXIT_INVALID_INVOCATION, "invalid_invocation")
        flow_kind = values[9]
    elif len(values) != 8:
        fail(EXIT_INVALID_INVOCATION, "invalid_invocation")
...
def _production_layout(user_profile: Path) -> CampaignLayout:
    ...
    overlay_root = mods_parent / "Sts2AgentBridgeUnified"
    operator_parent = application_support / "Sts2AgentBridge"
    config_root = operator_parent / "unified"
...
def _validate_operator_unit(root: Path, uid: int, flow_kind: str) -> dict[Path, os.stat_result]:
    records: dict[Path, os.stat_result] = {}
    records[root] = _require_directory(root, uid, 0o700)
    _list_exact(root, {"unified"})
    records.update(_validate_config_tree(root / "unified", uid, flow_kind))
    return records
...
def _validate_operator_unit_metadata(
    ...
    records[root] = _require_directory(root, uid, 0o700)
    _list_exact(root, {"unified"})
    records.update(_validate_config_tree_metadata(root / "unified", uid, flow_kind))
    return records
...
def _validate_overlay_unit(
    ...
    _list_exact(unit_root, {"Sts2AgentBridgeUnified"})
    records.update(
        _validate_overlay_tree(unit_root / "Sts2AgentBridgeUnified", uid, policy)
    )
    return records
...
def _require_state_records(
    ...
    config_root = operator_root / "unified"
    overlay_root = (
        overlay_unit_root / "Sts2AgentBridgeUnified"
        if state.mods_parent_created
...
def _validate_live_parents(
    ...
    _require_absent(layout.application_support / GENERIC_V10_STATE_ROOT_NAME)
    _require_absent(layout.application_support / GENERIC_V9_STATE_ROOT_NAME)
    _require_absent(layout.application_support / GENERIC_V8_STATE_ROOT_NAME)
    _require_absent(layout.application_support / GENERIC_V7_STATE_ROOT_NAME)
    _require_absent(layout.application_support / GENERIC_V6_STATE_ROOT_NAME)
    ...
    if not state.mods_parent_created:
        ...
        _require_absent(layout.mods_parent / GENERIC_V10_OVERLAY_ROOT_NAME)
        _require_absent(layout.mods_parent / GENERIC_V9_OVERLAY_ROOT_NAME)
        _require_absent(layout.mods_parent / GENERIC_V8_OVERLAY_ROOT_NAME)
        _require_absent(layout.mods_parent / GENERIC_V7_OVERLAY_ROOT_NAME)
        _require_absent(layout.mods_parent / GENERIC_V6_OVERLAY_ROOT_NAME)
...
def _install(
    ...
    try:
        ...
        with _private_umask():
            ...
            staged_config = layout.staged_operator / "unified"
            ...
            if mods_parent_created:
                staged_overlay_root = layout.staged_overlay / "Sts2AgentBridgeUnified"
...
def _validate_purge_remainder(
    ...
    if progress < 4:
        ...
        expected_operator = {"unified"} if progress < 3 else set()
    ...
    if progress < overlay_removed_at:
        ...
        if state.mods_parent_created:
            expected_unit = {"Sts2AgentBridgeUnified"} if progress < 7 else set()
