...
_EXPECTED_CANONICAL_ARTIFACTS = tuple((name, *manager.IDENTITIES[name])
    for name in (manager.DLL_NAME, manager.MANIFEST_NAME, manager.ZIP_NAME))
...
_POLICY = manager.ArtifactPolicy(
    ...
    archive=manager.ArtifactSpec(
        "Sts2AgentBridgeUnified-0.1.0.zip",
        len(_ARCHIVE_DATA),
...
class FixtureTree:
    def __init__(self, root: Path, *, preexisting_mods: bool = False, flow_kind: str = "unified") -> None:
        ...
        self.overlay_root = self.mods_parent / "Sts2AgentBridgeUnified"
        self.operator_parent = self.application_support / "Sts2AgentBridge"
        self.config_root = self.operator_parent / "unified"
...
def _assert_recorded_retention(
    ...
    for overlay_unit in present_overlays:
        overlay_root = (
            overlay_unit / "Sts2AgentBridgeUnified"
...
def _verify_production_bindings() -> None:
    if manager.CAMPAIGN_ID != "UNIFIED-BRIDGE-V1":
    ...
    expected_configs = {"unified": b'{"schema_version":"bridge_transport_config_v1","enabled":true,"bind_address":"127.0.0.1","port":43117,"token_file":"credential.hex"}'}
    if manager.CONFIGURATIONS != expected_configs or manager.CONFIG_HASHES != {"unified": "bcff0b563a27d6ee91f4c4ddd663f637aba3eecd27f70abdd4aa3ff00d109b9f"}:
    ...
    if (
        ...
        != Path("/private/tmp/sts-unified-bridge-release")
...
def _exercise_flow_and_config_immutability(root: Path) -> None:
    ...
    config_path.write_bytes(manager.configuration_bytes("unified").replace(b'"enabled":true', b'"enabled":false'))
    config_path.chmod(0o600)
    _expect_failure(
...
def _exercise_client_validation_api(root: Path) -> None:
    ...
    config_path.unlink()
    config_path.write_bytes(manager.configuration_bytes("unified"))
    config_path.chmod(0o600)
    with (
        mock.patch.object(manager.sys, "platform", "darwin"),
        mock.patch.object(manager.os, "geteuid", return_value=501),
