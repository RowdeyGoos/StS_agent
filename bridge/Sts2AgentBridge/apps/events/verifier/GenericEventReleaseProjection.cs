...
internal static partial class GenericEventReleaseProjectionBuilder
{
    private static readonly Regex RequestLinePattern = new(
        @"(?:GET|POST) /probe/generic-event-v7/[a-z0-9/-]+ HTTP/1\.1",
    ...
    internal static GenericEventReleaseProjection Build(byte[] image)
    {
        try
        {
            ...
            foreach (TypeDefinitionHandle handle in reader.TypeDefinitions)
            {
                ...
                GenericEventDiagnosticSurface.AddEnum(reader, value, name, inventory);
                inventory.Add(FormattableString.Invariant(
            ...
            foreach ((MethodDefinitionHandle handle, string identity) in methods)
            {
                ...
                MethodSignature<string> signature = method.DecodeSignature(names, null);
                inventory.Add(FormattableString.Invariant(
                    $"method_def|{Token(handle)}|{MetadataCell(identity)}|{MetadataCell(signature.ReturnType)}|{signature.GenericParameterCount}|{signature.RequiredParameterCount}|{(signature.Header.IsInstance ? 1 : 0)}|{(int)method.Attributes}|{(int)method.ImplAttributes}"));
                ...
                foreach (ParameterHandle parameterHandle in method.GetParameters())
                {
                    ...
                    inventory.Add(FormattableString.Invariant(
                        $"parameter|{Token(handle)}|{parameter.SequenceNumber}|{MetadataCell(reader.GetString(parameter.Name))}|{(int)parameter.Attributes}"));
                }
            }
            ...
            foreach ((MethodDefinitionHandle handle, string owner) in methods)
            {
                ...
                GenericEventDiagnosticSurface.AddMethod(owner, instructions, reader, token => DescribeEntity(reader, IlDecoder.RequireEntityHandle(token), names, methods, fields), body.ExceptionRegions.Length, inventory);
                GenericEventHitboxSurface.AddMethod(owner, instructions, token => DescribeEntity(reader, IlDecoder.RequireEntityHandle(token), names, methods, fields), body.ExceptionRegions.Length, inventory);
                ...
                for (int instructionIndex = 0; instructionIndex < instructions.Count; instructionIndex++)
                {
                    ...
                    if (owner.StartsWith("Sts2AgentBridge.Successors.GenericEventReleaseV10.ProductionGenericEventRuntimeFactory.CreateVerifiedService(",StringComparison.Ordinal) &&
                        instruction.OpCode == 0x007d && instructionIndex > 0 && LoadedLocal(instructions[instructionIndex-1]) >= 0)
                        inventory.Add($"construction_alias|{owner}|{instruction.Offset}|{LoadedLocal(instructions[instructionIndex-1])}|{instruction.MetadataToken}");
                    if (instruction.MetadataToken.HasValue && instruction.OpCode != 0x0072)
                    {
                        ...
                        if (instruction.IsCall) GenericEventDiagnosticSurface.AddCall(owner, targetIdentity, instruction.Offset, inventory);
                        if (instruction.IsCall) GenericEventHitboxSurface.AddCall(owner, targetIdentity, instruction.Offset, instruction.OpCode, inventory);
                        if (instruction.IsCall) GenericEventOffscreenSurface.AddCall(owner, targetIdentity, instruction.Offset, instruction.OpCode, inventory);
    ...
    private static bool IsCriticalReference(string identity) =>
        ...
        identity.Contains(".BindDiagnostic(", StringComparison.Ordinal) ||
        identity.Contains(".ProductionGenericEventRuntimeFactory.CreateVerifiedService(", StringComparison.Ordinal) ||
        identity.Contains(".PinnedGenericEventV7NativeAdapter..ctor(", StringComparison.Ordinal) ||
        identity.Contains(".GenericEventV7Session..ctor(", StringComparison.Ordinal) ||
        identity.Contains(".GenericEventV7WireService..ctor(", StringComparison.Ordinal) ||
        identity.Contains(".GenericEventV7WireService.Handle(", StringComparison.Ordinal) ||
        ...
        IReadOnlyDictionary<FieldDefinitionHandle, string> fields)
    {
        ...
        int inputIndex = target == "method:Sts2AgentBridge.Successors.GenericEventV7.GenericEventV7Session..ctor(Sts2AgentBridge.Successors.GenericEventV7.IGenericEventV7NativeAdapter,System.String)"
            ? index - 2 : index - 1;
    ...
    // Compiler-generated local functions contain a literal pipe. Escape free
    // metadata cells before putting them into the strict linkage row format;
    // escaping percent first keeps literal escape-looking names distinct.
    internal static string MetadataCell(string value) => value
    ...
    {
        const string hook = "Sts2AgentBridge.Successors.GenericEventV7.Native.GenericEventV7Hooks";
        ...
        bool guardOwner = owner.StartsWith(
            "Sts2AgentBridge.Successors.GenericEventReleaseV10.PinnedGenericEventHarmonyGuard.",
        ...
        bool allowedBoundHook = owner == hook + "..ctor(System.Action<System.Int32>,System.Action)" &&
            identity is "member:System.Type.GetMethod(System.String,System.Reflection.BindingFlags,System.Reflection.Binder,System.Type[],System.Reflection.ParameterModifier[])" or
                "member:System.Reflection.MethodBase.get_IsPublic()" or
                "member:System.Reflection.MethodBase.get_IsPrivate()" or
                "member:System.Reflection.MethodBase.get_IsStatic()" or
                "member:System.Reflection.MethodBase.get_IsGenericMethod()" or
        ...
        bool allowedExit = owner == "Sts2AgentBridge.Successors.GenericEventReleaseV10.GenericEventBootstrapHost.Initialize()" &&
        ...
        return GenericEventOffscreenSurface.Forbidden(owner, identity) ||
            sensitive && !(allowedHook || allowedBoundHook || allowedGuard || allowedBuild || allowedExit) ||
