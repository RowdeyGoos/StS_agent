...
public sealed class GenericEventV7TransformAdapter : ICardTransformV2NativeAdapter
{
        ...
        else
        {
            if (!ReferenceEquals(_screen.GetNodeOrNull<NCardGrid>("%CardGrid"), _grid) ||
                ...
                !TryCaptureBindings(holders, _candidates, out candidates,
                    out bool allSettled, restrictTarget: true))
                return Unsupported();
    ...
    private bool FreshSelection(out CardSelectionV1NativeCandidate[] candidates)
    {
        ...
        if(!FreshContext()||_expectedPreview is not null||_previewContainer.Visible||
            !ReferenceEquals(_screen.GetNodeOrNull<NCardGrid>("%CardGrid"),_grid)||_grid.IsAnimatingOut||
            !TrySnapshotHolders(_grid,out var holders)||!TryCaptureBindings(holders,_candidates,out candidates,out bool settled,restrictTarget:true)||!settled)return false;
    ...
    private void Select(CandidateBinding candidate)
    {
        try
        {
            if(candidate.Slot!=TestTargetSlot||!FreshSelection(out var current)||!current[candidate.Slot].Visible||!current[candidate.Slot].Enabled||current[candidate.Slot].Selected||
        ...
        out GenericEventDiagnosticCode diagnostic)
    {
        ...
        diagnostic = GenericEventDiagnosticCode.CandidateDomainBounds;
        if (candidates.Length <= TestTargetSlot) return false;
        candidates=TargetCard16(candidates);
    ...
    // Controlled operator test: the sixteenth allocated holder, without a
    // clipping, viewport, scroll-layout or geometry admission requirement.
    private const int TestTargetSlot = 15;
    private static CardSelectionV1NativeCandidate[] TargetCard16(CardSelectionV1NativeCandidate[] candidates) =>
        candidates.Select(c => new CardSelectionV1NativeCandidate(c.Slot, c.StableKey,
            c.HolderIdentity, c.ModelIdentity, c.CardNodeIdentity, c.UpgradeLevel,
            c.Visible, c.Enabled && c.Slot == TestTargetSlot, c.Selected,
            c.SelectionSettled, c.SelectDispatch)).ToArray();

    private static bool TryCaptureBindings(
        IReadOnlyList<NGridCardHolder> holdersSnapshot,
        CandidateBinding[] bindings,
        out CardSelectionV1NativeCandidate[] candidates,
        out bool allSettled, bool restrictTarget = false)
    {
        ...
        if(restrictTarget)candidates=TargetCard16(candidates);
        return true;
    }
