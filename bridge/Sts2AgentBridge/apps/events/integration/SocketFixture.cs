...
{
    internal static readonly byte[] Configuration=Encoding.UTF8.GetBytes("{\"schema_version\":\"generic_event_v10_transport_config_v1\",\"enabled\":true,\"flow_kind\":\"generic\",\"bind_address\":\"127.0.0.1\",\"port\":43117,\"token_file\":\"credential.hex\"}");
    private static int Main(string[] args)
    {
        ...
        var runtime=GenericEventTransportRuntime.Create((byte[])Configuration.Clone(),()=>Encoding.ASCII.GetBytes(new string('c',64)),
            (_,nonce)=>new GenericEventV7WireService(nonce,new GenericEventV7Session(adapter,nonce)),()=>{
