"""Descriptor-bound read lease for the one fixed room operator tree."""
...
def read_credential(home: Path, uid: int, state: Any,
    ...
    try:
        ...
        item = open_entry('room_flows_v1', private=True, owned=True)
        ...
        for entry in fds:
            validate(entry)
        # Both leaves are relative to the retained room directory, never the config FD.
        parent = item[0]
