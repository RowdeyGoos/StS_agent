...
from test_shop_map_transport import room_frames, http

class Fixture:
    def __init__(self, flow_kind="shop"):
    ...
    def __enter__(self):
        ...
        self.item = self.home / 'Library' / 'Application Support' / 'Sts2AgentBridge' / 'room_flows_v1'
...
class ClientTests(unittest.TestCase):
    ...
    def test_config_failures_no_credential_read(self):
        for flow in ("shop", "event"):
            other = "event" if flow == "shop" else "shop"
            values=[reader.CONFIGURATIONS[flow].replace(b'true',b'false'),b'{}',reader.CONFIGURATIONS[flow]+b'\n',reader.CONFIGURATIONS[flow].replace(b'room_flows_v1',b'live_probe_v0'),reader.CONFIGURATIONS[other]]
            for data in values:
    ...
    def test_state_flow_and_config_binding_before_credential(self):
        for flow, config_hash in (("event", reader.CONFIG_HASHES["shop"]),
                                  ("shop", reader.CONFIG_HASHES["event"]),
                                  ("event", reader.CONFIG_HASHES["event"])):
            with self.subTest(flow=flow,config_hash=config_hash), Fixture("shop") as f:
                ...
                def change(fd,count):
                    ...
                    if mutation=='replace_config' or len(f.read_buffers)>2:
                        ...
                        if mutation=='in_place': path.write_bytes(b'b'*64)
                            ...
                            path.rename(f.item/'moved'); path.write_bytes(reader.CONFIGURATIONS["shop"] if mutation=='replace_config' else b'a5'*32); path.chmod(0o600)
                f.read_hook=change
                with self.assertRaises(reader.OperatorFailure): f.read()
    ...
    def test_preflight_failure_never_calls_reader_or_transport(self):
        for where in ('validate','read'):
            ...
            def validate(value):
                ...
                return SimpleNamespace(user_profile=Path('/synthetic')),SimpleNamespace(flow_kind='shop')
    ...
    def test_cli_invalid_inputs_sanitized(self):
        for args in ([],['--path','/synthetic-secret'],['--expected-state-sha256','BAD']):
            ...
            self.assertEqual(json.loads(result.stdout),{'schema_version':1,'status':'failed','code':'room_client_preflight_failed'})
...
if __name__=='__main__':
    ...
    if result.wasSuccessful(): print(json.dumps({'schema_version':1,'status':'passed','suite':'room_release_client','check_count':result.testsRun},separators=(',',':')))
