...
internal interface IRoomFlowBootstrapOperatorLease : IDisposable
{
...
internal interface IRoomFlowBootstrapFrameAttachment
{
...
internal interface IRoomFlowBootstrapFrameConnector
{
    IRoomFlowBootstrapFrameAttachment? Attach(Action onFrame);
}

internal interface IRoomFlowBootstrapRuntime
{
    bool Start();
...
internal interface IRoomFlowBootstrapRuntimeFactory
{
    // Ownership of both arrays transfers at invocation, including failure paths.
    IRoomFlowBootstrapRuntime? Create(byte[] configuration, byte[] credential);
}

internal interface IRoomFlowBootstrapClock
{
    long Timestamp { get; }
...
internal interface IRoomFlowBootstrapTimer : IDisposable
{
}

internal interface IRoomFlowBootstrapTimerFactory
{
    IRoomFlowBootstrapTimer Arm(TimeSpan dueTime, Action callback);
}

internal sealed class RoomFlowBootstrapLifecycle
{
    internal const int PendingLifetimeSeconds = 5;

    private readonly object _gate = new();
    private readonly Func<IRoomFlowBootstrapOperatorLease?> _openEnabled;
    private readonly Func<bool> _verifyBuild;
    private readonly IRoomFlowBootstrapFrameConnector _frames;
    private readonly IRoomFlowBootstrapRuntimeFactory _runtimeFactory;
    private readonly IRoomFlowBootstrapClock _clock;
    private readonly IRoomFlowBootstrapTimerFactory _timers;
    ...
    private IRoomFlowBootstrapTimer? _timer;
    private IRoomFlowBootstrapFrameAttachment? _attachment;
    private IRoomFlowBootstrapRuntime? _runtime;
    private bool _stopCallAttempted;
    private bool _stopCallInProgress;
    private bool _stopCallRetryRequired;

    internal RoomFlowBootstrapLifecycle(
        Func<IRoomFlowBootstrapOperatorLease?> openEnabled,
        Func<bool> verifyBuild,
        IRoomFlowBootstrapFrameConnector frames,
        IRoomFlowBootstrapRuntimeFactory runtimeFactory,
        IRoomFlowBootstrapClock clock,
        IRoomFlowBootstrapTimerFactory timers)
    {
    ...
    internal bool Initialize()
    {
        ...
        IRoomFlowBootstrapOperatorLease? lease = null;
        byte[]? configuration = null;
        byte[]? credential = null;
        IRoomFlowBootstrapTimer? timer = null;
        IRoomFlowBootstrapFrameAttachment? attachment = null;
        try
        {
            lease = _openEnabled();
            ...
            IRoomFlowBootstrapOperatorLease leaseToDispose = lease;
            lease = null;
    ...
    internal bool RequestStop()
    {
        byte[]? configuration;
        byte[]? credential;
        IRoomFlowBootstrapTimer? timer;
        IRoomFlowBootstrapRuntime? runtime;
        lock (_gate)
        {
            _stopRequested = true;
    ...
    private void OnFrame()
    {
        ...
        IRoomFlowBootstrapRuntime? runtimeToDrain = null;
        byte[]? configuration = null;
        byte[]? credential = null;
        IRoomFlowBootstrapTimer? timer = null;
    ...
    {
        IRoomFlowBootstrapRuntime? runtime = null;
        try
        {
    ...
    {
        IRoomFlowBootstrapFrameAttachment? attachment;
        IRoomFlowBootstrapRuntime? runtime;
        IRoomFlowBootstrapTimer? timer;
        lock (_gate)
        {
    ...
    private bool FailInitialization(
        IRoomFlowBootstrapOperatorLease? lease,
        byte[]? configuration,
        byte[]? credential,
        IRoomFlowBootstrapTimer? timer,
        IRoomFlowBootstrapFrameAttachment? attachment = null)
    {
        ...
        Zero(configuration);
        Zero(credential);
        IRoomFlowBootstrapTimer? failedTimer = timer is not null && !TryDispose(timer) ? timer : null;

        byte[]? ownedConfiguration;
        byte[]? ownedCredential;
        IRoomFlowBootstrapTimer? ownedTimer;
        lock (_gate)
        {
            _stopRequested = true;
            _initializationInProgress = false;
    ...
    private static bool IsTransportStopped(IRoomFlowBootstrapRuntime runtime)
    {
        try
        {
    ...
    private static bool IsServiceDisposed(IRoomFlowBootstrapRuntime runtime)
    {
        try
        {
    ...
    private void IssueRuntimeStop(IRoomFlowBootstrapRuntime runtime)
    {
        lock (_gate)
        {
